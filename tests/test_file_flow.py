...
class FileFlow(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.artifacts = Path(tempfile.mkdtemp(prefix="nano-file-flow-"))
        print(f"I retain File flow artifacts at {cls.artifacts}", flush=True)
        cls.compiler = shlex.split(os.environ.get("NANO_FILE_FLOW_CC", "cc"))
        cls.flags = ["-std=c11", "-D_DEFAULT_SOURCE", "-g", "-O1", "-Wall", "-Wextra", "-Werror",
                     "-fsanitize=address,undefined", "-fno-omit-frame-pointer"]
        cls.flags += shlex.split(os.environ.get("NANO_FILE_FLOW_CFLAGS", ""))
        cls.objects = shlex.split(os.environ["FILE_FLOW_OBJECTS"])
        cls.ldflags = shlex.split(os.environ.get("FILE_FLOW_LDFLAGS", "-lm -lcrypto"))

    def qualify(self, name, instrument):
        exe = self.artifacts / name
        sources = ["tests/nanoisa/test_file_flow.c", "src/nanoisa/service_file_nominal.c",
                   "src/nsi_file_plan.c"]
        if not instrument:
    ...
    def test_instrumented_flow_transactions_and_allocation(self):
        self.qualify("instrumented", True)

    def test_linked_flow_lifetimes_and_old_boundaries(self):
