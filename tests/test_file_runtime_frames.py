"""I qualify manual private frame transfers, not an opcode dispatcher."""
try:
...
class FileRuntimeFrames(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.artifacts = Path(tempfile.mkdtemp(prefix='nano-file-frames-'))
    ...
    def qualify(self, name, instrument):
        ...
        self.command(f'{name}-link', [*self.compiler, *self.flags,
                     *(['-DHOSTED_INSTRUMENT'] if instrument else []),
                     'tests/nanoisa/test_file_runtime_frames.c', *objects, *self.objects,
                     *self.ldflags, '-o', str(exe)])
        self.command(f'{name}-run', [str(exe)], run=True)

    def test_instrumented_frames_and_preserved_carrier(self):
        self.qualify('instrumented', True)

    def test_linked_frames_and_preserved_carrier(self):
