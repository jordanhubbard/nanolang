...
int main(int argc,char **argv){
 ...
 ctx_push_scope(&context);ctx_add_sym(&context,"print",TYPE_FUNCTION);expression(&context,&call,text,sizeof text);assert(strstr(text,"print(")==text);ctx_pop_scope(&context);
 call.as.call.func_expr=&callee;expression(&context,&call,text,sizeof text);assert(strstr(text,"(callback)(")==text);call.as.call.func_expr=NULL;
