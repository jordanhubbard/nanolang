...
class ConsumingCalls(unittest.TestCase):
    ...
    def test_consuming_calls_and_cleanup(self):
        with tempfile.TemporaryDirectory(prefix='nano-consuming-calls-') as name:
            tmp = Path(name)
            run = self.checked([os.environ.get('NANO_CONSUMING_CALL_TEST', ROOT/'obj/test_consuming_calls'), tmp])
            ...
            for _, index, status, value in cases:
                with self.subTest(case=index):
                    ...
                    self.assertEqual(vm.returncode, (int(value) if int(index)<5 else 0) if succeeds else 1)
