"""I qualify non-admitting indirect File ownership facts; no File module executes."""
import json
import os
...
class FileIndirectFlow(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.artifacts = Path(tempfile.mkdtemp(prefix="nano-file-indirect_flow-"))
        print(f"I retain indirect ownership query artifacts at {cls.artifacts}", flush=True)
        cls.compiler = shlex.split(os.environ.get("NANO_FILE_INDIRECT_FLOW_CC", "cc"))
        cls.flags = ["-std=c11", "-D_DEFAULT_SOURCE", "-g", "-O1", "-Wall", "-Wextra", "-Werror"]
        cls.flags += shlex.split(os.environ.get("NANO_FILE_INDIRECT_FLOW_CFLAGS", ""))
        cls.objects = shlex.split(os.environ["FILE_INDIRECT_FLOW_OBJECTS"])
        cls.ldflags = shlex.split(os.environ.get("FILE_INDIRECT_FLOW_LDFLAGS", "-lm -lcrypto"))
    ...
    def qualify(self, name, instrument):
        ...
        sources = ["tests/nanoisa/test_file_indirect_flow.c", "src/nanoisa/service_file_nominal.c", "src/nsi_file_plan.c"]
        if not instrument:
        ...
        self.assertIn(b"private indirect ownership checks; no File execution", stdout)
        print(stdout.decode(errors="replace").strip(), flush=True)

    def test_instrumented_indirect_flow_query(self):
        self.qualify("instrumented", True)

    def test_linked_indirect_flow_query(self):
