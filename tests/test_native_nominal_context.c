...
static void emit_checked(const char *source, bool declared_union) {
    int count = 0;
    ...
    assert(text);
    if (declared_union) {
        assert(strstr(text, "typedef struct nl_T"));
...
int main(void) {
    ...
    emit_checked(declared, true);
    assert(!strcmp(get_prefixed_type_name("T"), "void*"));
    emit_checked(generic, false);
    assert(!strcmp(get_prefixed_type_name("T"), "void*"));

    /* An enclosing emission's snapshot survives an inner success or refusal. */
    native_union_letters = UINT32_C(1) << ('U' - 'A');
    emit_checked(declared, true);
