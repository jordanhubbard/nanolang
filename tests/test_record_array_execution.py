"""I qualify copied non-admitting execution plans without runtime admission."""
import json
import os
...
class RecordArrayExecution(unittest.TestCase):
    command = test_file_cyclic.FileCyclic.command

    @classmethod
    def setUpClass(cls):
        cls.artifacts = Path(tempfile.mkdtemp(prefix='nano-record-array-execution-'))
    ...
    def test_complete_plan_and_allocation_controls(self):
        for observed in (False, True):
            ...
            self.command(mode + '-build', [*self.cc, *self.flags,
                *(['-DRA_WHITEBOX'] if observed else []),
                'tests/nanoisa/test_record_array_execution.c',
