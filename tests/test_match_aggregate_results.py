...
import unittest
from tests.test_affine_generic_identity import GenericAffineIdentity

class MatchAggregateResults(unittest.TestCase):
    def test_generic_union_result(self):
        GenericAffineIdentity().check('''union Box<T> { Some { value: T }, None {} }
...
''', True)
    ...
    def test_integer_selector_union_result(self):
        GenericAffineIdentity().check('''union Answer { Value { value: int }, Empty {} }
...
''', True)
    ...
    def test_scalar_match_result(self):
        GenericAffineIdentity().check('''fn choose(value: int) -> int { return match value { 1 => 7, _ => 8 } }
