...
class FlatRecordEmitter(unittest.TestCase):
    ...
    def test_string_to_int_matches_and_executes(self):
        ...
        with tempfile.TemporaryDirectory(prefix="nano-string-to-int-") as tmp:
            work = Path(tmp)
            ...
            result = self.run_checked(ROOT / "tests/nanoisa/test_nanoisa_src_nano", seed, assembly,
                                     "parse_number", "literal_number", "main")
    ...
    def test_string_to_int_wrong_arguments_are_refused(self):
        programs = [
            'fn bad() -> int { return (string_to_int) }',
            'fn bad() -> int { return (string_to_int "1" "2") }',
            'fn bad() -> int { return (string_to_int 1) }',
            'fn bad() -> int { return (string_to_int true) }',
        ]
        with tempfile.TemporaryDirectory(prefix="nano-string-to-int-refusal-") as tmp:
            work = Path(tmp)
