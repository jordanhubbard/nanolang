"""I qualify copied cyclic hosted facts without runtime or service execution."""
import os
...
class FileCyclicHosted(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.artifacts = Path(tempfile.mkdtemp(prefix="nano-file-cyclic-hosted-"))
        print(f"I retain cyclic hosted artifacts at {cls.artifacts}", flush=True)
        cls.compiler = shlex.split(os.environ.get("NANO_FILE_CYCLIC_HOSTED_CC", "cc"))
        cls.flags = shlex.split(os.environ.get("NANO_FILE_CYCLIC_HOSTED_CFLAGS", ""))
        cls.flags += ["-std=c11", "-D_DEFAULT_SOURCE", "-g", "-O1", "-Wall", "-Wextra", "-Werror"]
        if os.environ.get("NANO_FILE_CYCLIC_HOSTED_SANITIZERS", "0") != "0":
            cls.flags += ["-fsanitize=address,undefined", "-fno-omit-frame-pointer"]
        cls.objects = [p for p in shlex.split(os.environ["FILE_CYCLIC_HOSTED_OBJECTS"])
                       if Path(p).stem not in hosted_providers.PROVIDERS]
        cls.ldflags = shlex.split(os.environ.get("FILE_CYCLIC_HOSTED_LDFLAGS", "-lm -lcrypto"))
    ...
    def qualify(self, name, instrument):
        ...
        self.command(f"{name}-link", [*self.compiler, *self.flags,
                     *(["-DHOSTED_INSTRUMENT"] if instrument else []),
                     "tests/nanoisa/test_file_cyclic_hosted.c", *objects, *self.objects,
                     *self.ldflags, "-o", str(exe)])
        stdout = self.command(f"{name}-run", [str(exe)])
        self.assertIn(b"private cyclic hosted checks; no runtime or service execution", stdout)
    ...
    def test_linked_copied_cyclic_hosted_facts(self):
