/* I retain complete record snapshots independently of their original owners. */
...
static void metadata_complete_tuple(void) {
    char name[]="ExactOwner.Child";
    TypeInfo child={.base_type=TYPE_STRUCT,.generic_name=name};
    TypeInfo array={.base_type=TYPE_ARRAY,.element_type=&child};
    TypeInfo *parameters[]={&array}; Type parameter_types[]={TYPE_ARRAY};
    FunctionSignature signature={.param_count=1,.param_types=parameter_types,
        ...
        .return_type_info=&child};
    TypeInfo callback={.base_type=TYPE_FUNCTION,.fn_sig=&signature};
    TypeInfo *children[]={&array,&callback,&child};
    Type flat_types[]={TYPE_ARRAY,TYPE_FUNCTION,TYPE_STRUCT};
    char *flat_names[]={NULL,NULL,name};
    TypeInfo tuple={.base_type=TYPE_TUPLE,.type_param_count=3,.type_params=children,
        .tuple_element_count=3,.tuple_types=flat_types,.tuple_type_names=flat_names};
    assert(type_info_tuple_valid(&tuple));
    TypeInfo *annotations[]={&tuple};
    Environment *env=create_environment(); assert(env);
    StructDef record={.name=strdup("TupleHolder"),.field_count=1,.field_type_info=annotations,
        .field_names=calloc(1,sizeof(char *)),.field_types=calloc(1,sizeof(Type))};
    assert(record.name && record.field_names && record.field_types);
    record.field_names[0]=strdup("members"); assert(record.field_names[0]);
    record.field_types[0]=TYPE_TUPLE; env_define_struct(env,record);
    ModuleMetadata *snapshot=extract_module_metadata(env,"ExactOwner"); assert(snapshot);
    TypeInfo *copy=snapshot->structs[0].field_type_info[0];
    assert(copy!=&tuple && type_info_tuple_valid(copy));
    assert(copy->type_params!=children && copy->tuple_types!=flat_types && copy->tuple_type_names!=flat_names);
    for (int i=0;i<3;++i) assert(copy->type_params[i]!=children[i]);
    assert(copy->tuple_type_names[2]!=name && copy->type_params[2]->generic_name!=name);
    assert(copy->tuple_type_names[2]!=copy->type_params[2]->generic_name);
    assert(copy->type_params[1]->fn_sig!=&signature);
    assert(copy->type_params[1]->fn_sig->param_type_info[0]!=&array);
    free_environment(env);
    name[0]='X'; child.base_type=TYPE_BOOL; flat_types[0]=TYPE_INT; children[1]=NULL;
    assert(type_info_tuple_valid(copy));
    assert(copy->type_param_count==3 && copy->tuple_element_count==3);
    assert(copy->type_params[0]->element_type->base_type==TYPE_STRUCT);
    assert(!strcmp(copy->type_params[0]->element_type->generic_name,"ExactOwner.Child"));
    assert(copy->type_params[1]->fn_sig->param_type_info[0]->element_type->base_type==TYPE_STRUCT);
    assert(!strcmp(copy->type_params[1]->fn_sig->return_struct_name,"ExactOwner.Child"));
    assert(copy->type_params[1]->fn_sig->return_type_info->base_type==TYPE_STRUCT);
    assert(!strcmp(copy->tuple_type_names[2],"ExactOwner.Child"));
    assert(!strcmp(copy->type_params[2]->generic_name,"ExactOwner.Child"));
    free_module_metadata(snapshot);
    assert(tuple.type_params==children && tuple.tuple_types==flat_types && callback.fn_sig==&signature);
}
