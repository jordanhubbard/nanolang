...
class OwnedUnionSource(unittest.TestCase):
    ...
    def test_c_frontend_refuses_an_unknown_selected_arm_name(self):
        source_text = selected.PREFIX + selected.CONSUME.replace(
            'let Result.Ok { value }', 'let Result.Missing { value }', 1) + selected.MAIN
        with tempfile.TemporaryDirectory(prefix='nano-owned-union-unknown-arm-') as directory:
            source, module = Path(directory) / 'source.nano', Path(directory) / 'source.nvm'
            ...
            self.assertIn('I cannot resolve a qualified type in this source: Result.Missing',
