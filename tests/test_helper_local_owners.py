"""I keep caller and helper roots distinct through normal and terminal cleanup."""
try:
...
class HelperLocalOwners(unittest.TestCase):
    ...
    def test_paired_origins_and_cleanup(self):
        with tempfile.TemporaryDirectory(prefix='nano-helper-owners-') as name:
            tmp = Path(name)
            run = self.checked([os.environ.get('NANO_HELPER_OWNER_TEST', ROOT/'obj/test_helper_local_owners'), tmp])
