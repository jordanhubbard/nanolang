"""I qualify precise private global facts without executing a module."""
import json
import os
...
class RecordArrayGlobalFlow(unittest.TestCase):
    command = test_file_cyclic.FileCyclic.command

    @classmethod
    def setUpClass(cls):
        cls.artifacts = Path(tempfile.mkdtemp(prefix='nano-record-array-global-flow-'))
    ...
    def test_complete_global_flow_and_allocation_controls(self):
        for observed in (False, True):
            ...
            self.command(mode + '-build', [*self.cc, *self.flags,
                *(['-DRA_WHITEBOX'] if observed else []),
                'tests/nanoisa/test_record_array_global_flow.c',
                'tests/nanoisa/record_array_alloc.c', *products, *self.objects,
                *self.ldflags, '-o', str(exe)])
            output = self.command(mode + '-run', [str(exe)])
            self.assertIn(b'private global-flow checks; no runtime admission', output)
            if observed:
                self.assertIn(b'every exact MEMORY refusal recovered', output)
