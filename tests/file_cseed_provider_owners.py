        ...
        for name, order in cases:
            ...
            if compiler == 'nanoc_c':
                ...
                for module, obj in module_objects.items():
                    ...
                    # Each wrapper loads alone in a fresh process, matching the runtime's lazy binding.
                    test.command(label+'-'+module+'-dynamic-run',
                                 [sys.executable, '-m', 'tests.file_cseed_provider_owners', '--dynamic', module, library, forth_binary])
...
def dynamic(module, path, forth_binary):
    ...
    if module in ('cjson_provider', 'std_json'):
        ...
        value = fn('cJSON_Parse' if direct else 'nl_json_parse', pointer, [text])(b'{}')
        assert value
    ...
    print('PASS isolated dynamic wrapper', module, flush=True)


if __name__ == '__main__':
    assert len(sys.argv) == 5 and sys.argv[1] == '--dynamic'
    dynamic(sys.argv[2], Path(sys.argv[3]), Path(sys.argv[4]))
