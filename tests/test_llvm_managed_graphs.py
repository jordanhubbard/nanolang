...
class ManagedGraphs(unittest.TestCase):
    ...
    def test_preparation_growth_failures_and_nested_entry(self):
        ...
        extra=('static long budget=-1;'
               'void *nano_test_malloc(size_t n){if(!budget)return 0;if(budget>0)--budget;return malloc(n);}')
        ...
        for b in (0,1,2,3,4,6,10,15,25,45,80):
            self.native_harness(ir,f'budget={b};uint64_t s=nano_try_entry();if(s && s!=((uint64_t)3<<32))return 1;if({b}==0 && s!=((uint64_t)3<<32))return 8;if(nms_module_live_objects())return 2;budget=-1;if(nano_try_entry()||nms_module_live_objects())return 3;return nano_dispose();',extra,allocation_control=True)
