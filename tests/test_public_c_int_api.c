/* I preserve binding identity and publication when integer conversion refuses. */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc,char **argv){
 ...
 ASTNode *args[]={&integer};ASTNode call={.type=AST_CALL};
 call.as.call.name="int_to_string";call.as.call.args=args;call.as.call.arg_count=1;
 CBCtx context={0};strcpy(context.prefix,"nano_fixture_");char text[4096];
 expression(&context,&call,text,sizeof text);assert(strstr(text,"nano_fixture_int_text_new(")==text);
 assert(infer_expr_type(&context,&call)==TYPE_STRING);
 ctx_push_scope(&context);ctx_add_sym(&context,"int_to_string",TYPE_FUNCTION);
 ...
 expression(&context,&call,text,sizeof text);assert(strstr(text,"int_to_string(")==text);
 ...
 ASTNode declared={.type=AST_FUNCTION};declared.as.function.name="int_to_string";declared.as.function.return_type=TYPE_STRING;
 ...
 context.root=&declaration_root;expression(&context,&call,text,sizeof text);assert(strstr(text,"int_to_string(")==text);context.root=NULL;
 ASTNode wrong_float={.type=AST_FLOAT},wrong_bool={.type=AST_BOOL},wrong_string={.type=AST_STRING};
 wrong_string.as.string_val="ordinary";
 ASTNode *wrong[]={&wrong_float,&wrong_bool,&wrong_string};
 for(size_t i=0;i<sizeof wrong/sizeof wrong[0];++i){
  args[0]=wrong[i];assert(infer_expr_type(&context,&call)==TYPE_UNKNOWN);
  context.out=tmpfile();assert(context.out);context.error=NULL;
  assert(emit_expr(&context,&call)!=0);assert(strstr(context.error,"exact INT"));
  const char *first=context.error;ctx_error(&context,"I retain my first diagnostic.");assert(context.error==first);
  assert(fclose(context.out)==0);
 }
 args[0]=&integer;call.as.call.arg_count=0;context.out=tmpfile();assert(context.out);context.error=NULL;
 ...
 args[0]=&unknown;call.as.call.name="int_to_string";
 ...
 args[0]=&integer;assert(c_backend_emit(&root,argv[1],"ordinary.nano",&options)==0);
 return 0;
}
