...
def workspace():
    target = os.environ.get('OWNED_ARRAY_ORIGIN_ARTIFACTS')
    if target:
...
class OwnedArrayOrigins(unittest.TestCase):
    ...
    def test_origins_and_failure_atomicity(self):
        with workspace() as temp:
            work = Path(temp)
            self.artifacts = work
            self.command_index = 0
            compiler = shlex.split(os.environ.get('CC', 'cc'))
            flags = shlex.split(os.environ.get('OWNED_ARRAY_ORIGIN_CFLAGS', ''))
