...
extern DynArray* process_run(const char* command);
...
static int64_t nl_test_process_run();
static int64_t nl_test_system();
static int64_t nl_main();

static int64_t nl_test_process_run() {
    nl_println_string("Testing process_run:");
    DynArray* raw = dyn_array_new(ELEM_STRING);
    /* unsafe */ {
        raw = nl_os_process_run("echo hello world");
    }
    const char* code_s = nl_array_at_string(raw, 0LL);
    const char* stdout = nl_array_at_string(raw, 1LL);
    const char* stderr = nl_array_at_string(raw, 2LL);
    nl_print_string("  Exit code: ");
    nl_println_string(code_s);
    nl_print_string("  Stdout: ");
    nl_println_string(stdout);
    nl_print_string("  Stderr: ");
    nl_println_string(stderr);
    return 0LL;
}

static int64_t nl_test_system() {
    nl_println_string("Testing system (skipped - requires stdlib.h)");
    return 0LL;
}

static int64_t nl_main() {
    nl_println_string("=== Testing std::process ===");
    nl_println_string("");
    nl_test_process_run();
    nl_println_string("");
    nl_test_system();
    nl_println_string("");
    nl_println_string("All tests passed!");
    return 0LL;
}
