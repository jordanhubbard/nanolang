...
class MultipleConsumingCalls(unittest.TestCase):
    ...
    def test_multiple_consuming_calls_and_cleanup(self):
        with tempfile.TemporaryDirectory(prefix='nano-consuming-calls-') as name:
            tmp = Path(name)
            run = self.checked([os.environ.get('NANO_MULTIPLE_CONSUMING_CALL_TEST', ROOT/'obj/test_multiple_consuming_calls'), tmp])
            print(run.stdout, end='')
            ...
            for _, index, status, value in cases:
                with self.subTest(case=index):
                    ...
                    self.assertEqual(vm.returncode, (int(value) if int(index)<6 else 0) if succeeds else 1)
