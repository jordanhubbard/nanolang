...
'''+main,['wrap','probe','main'])
        ...
        positives['global-string']=('let words: array<string> = (str_split "a,b" ",")\nfn probe() -> int { assert (== (at words 1) "b") return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['probe', 'main'])
        positives['global-declared']=('fn str_split(value: int) -> array<int> { return [value] }\nshadow str_split { assert (== (at (str_split 41) 0) 41) }\nlet words: array<int> = (str_split 41)\nfn probe() -> int { assert (== (at words 0) 41) return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['str_split', 'probe', 'main'])
        positives['nested-store']=('fn probe() -> int { let mut rows: array<array<string>> = [["seed"]] set rows (array_push rows (str_split "a,b" ",")) (array_set rows 0 (str_split "c,d" ",")) let first: array<string> = (at rows 0) let second: array<string> = (at rows 1) assert (== (at first 1) "d") assert (== (at second 1) "b") return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['probe', 'main'])
        positives['declared-insert']=('fn array_push(rows: array<array<int>>, words: array<string>) -> int { return (+ 40 (array_length words)) }\nshadow array_push { assert (== (array_push [[1]] ["a", "b"]) 42) }\nfn probe() -> int { assert (== (array_push [[1]] (str_split "a,b" ",")) 42) return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['array_push', 'probe', 'main'])
        positives['local-store']=('fn chosen_store(rows: array<array<int>>, index: int, words: array<string>) -> int { return (+ index (array_length words)) }\nshadow chosen_store { assert (== (chosen_store [[1]] 40 ["a", "b"]) 42) }\nfn probe() -> int { let array_set: fn(array<array<int>>,int,array<string>)->int = chosen_store assert (== (array_set [[1]] 40 (str_split "a,b" ",")) 42) return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['chosen_store', 'probe', 'main'])
        positives['local-insert']=('fn chosen(rows: array<array<int>>, words: array<string>) -> int { return (array_length words) }\nshadow chosen { assert (== (chosen [[1]] ["a", "b"]) 2) }\nfn probe() -> int { let array_push: fn(array<array<int>>,array<string>)->int = chosen assert (== (array_push [[1]] (str_split "a,b" ",")) 2) return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['chosen', 'probe', 'main'])
        positives['union-payload']=('union Words { Some { items: array<string> } }\nfn make_words() -> Words { return Words.Some { items: (str_split "a,b" ",") } }\nshadow make_words { assert true }\nfn probe() -> int { let words: Words = (make_words) match words { Some(value) => { assert (== (at value.items 1) "b") } } return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['make_words', 'probe', 'main'])
        argument_provider=self.work/'string-destination-provider.nano'
        argument_provider.write_text('module string_destination_provider\npub fn strings(words: array<string>) -> int { return (array_length words) }\nshadow strings { assert (== (strings ["a", "b"]) 2) }\npub fn integers(words: array<int>) -> int { return (array_length words) }\nshadow integers { assert (== (integers [1, 2]) 2) }\npub fn str_split(value: int) -> array<int> { return [value] }\nshadow str_split { assert (== (at (str_split 41) 0) 41) }\n')
        argument_import=f'module {json.dumps(str(argument_provider))} as Destination\n'
        positives['qualified-array-argument']=(argument_import+'fn probe() -> int { assert (== (Destination.strings (str_split "a,b" ",")) 2) return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['strings','integers','str_split','probe','main'])
        positives['global-qualified']=(argument_import+'let words: array<int> = (Destination.str_split 41)\nfn probe() -> int { assert (== (at words 0) 41) return 0 }\nshadow probe { assert (== (probe) 0) }\n'+main,['strings','integers','str_split','probe','main'])
        negatives={
        ...
        boundary_refusals['global-reverse']='let words: array<string> = [41]\nfn main() -> int { return 0 }\nshadow main { assert true }\n'
        boundary_refusals['nested-push']='fn main() -> int { let mut rows: array<array<int>> = [[1]] set rows (array_push rows (str_split "a,b" ",")) return 0 }\nshadow main { assert true }\n'
        boundary_refusals['nested-set']='fn main() -> int { let rows: array<array<int>> = [[1]] (array_set rows 0 (str_split "a,b" ",")) return 0 }\nshadow main { assert true }\n'
        boundary_refusals['nested-set-reverse']='fn main() -> int { let rows: array<array<string>> = [["a"]] (array_set rows 0 [41]) return 0 }\nshadow main { assert true }\n'
        boundary_refusals['union-local']='union Words { Some { items: array<int> } }\nfn main() -> int { let words: Words = Words.Some { items: (str_split "a,b" ",") } return 0 }\nshadow main { assert true }\n'
        boundary_refusals['union-return']='union Words { Some { items: array<int> } }\nfn make_words() -> Words { return Words.Some { items: (str_split "a,b" ",") } }\nshadow make_words { assert true }\nfn main() -> int { return 0 }\nshadow main { assert true }\n'
        boundary_refusals['union-record']='union Words { Some { items: array<int> } }\nstruct Outer { words: Words }\nfn main() -> int { let value: Outer = Outer { words: Words.Some { items: (str_split "a,b" ",") } } return 0 }\nshadow main { assert true }\n'
        boundary_refusals['union-context']='union Box<T> { Some { items: T } }\nfn main() -> int { let value: Box<array<int>> = Box.Some { items: (str_split "a,b" ",") } return 0 }\nshadow main { assert true }\n'
        boundary_refusals['union-explicit']='union Box<T> { Some { items: T } }\nfn main() -> int { let value: Box<array<int>> = Box<array<int>>.Some { items: (str_split "a,b" ",") } return 0 }\nshadow main { assert true }\n'
        boundary_refusals['qualified-array-argument']=argument_import+'fn main() -> int { return (Destination.integers (str_split "a,b" ",")) }\nshadow main { assert true }\n'
        boundary_refusals['qualified-array-reverse']=argument_import+'fn main() -> int { return (Destination.strings [41]) }\nshadow main { assert true }\n'
