...
class ConditionalFieldTypes(unittest.TestCase):
    def check(self, field_type, left, right, assertion, reject=False, extra_clause=""):
        if reject:
            assertion = "let checked: Holder = (choose true) assert true"
        source = f"""struct Holder {{ value: {field_type} }}
fn choose(flag: bool) -> Holder {{
 return Holder {{ value: (cond (flag {left}) {extra_clause} (else {right})) }}
}}
...
"""
            ...
            for compiler in ("nanoc_c", "nano_virt", "nanoc_stage1", "nanoc_stage2"):
                with self.subTest(compiler=compiler):
                    ...
                    if reject:
                        ...
                        run_command = [output] if compiler == "nanoc_c" else [ROOT / "bin/nano_vm", output]
                        run = subprocess.run(run_command, cwd=ROOT, capture_output=True, text=True, timeout=15)
    ...
    def test_mismatched_middle_arm_preserves_output(self):
        self.check("string", '"first"', '"last"', "assert true", reject=True, extra_clause="(false 7)")
