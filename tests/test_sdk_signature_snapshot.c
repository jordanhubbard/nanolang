...
#undef calloc
int main(void) {
    uint8_t *tags=malloc(4);assert(tags);
    tags[0]=TAG_OPAQUE;tags[1]=TAG_OPAQUE;tags[2]=TAG_ENUM;tags[3]=TAG_INT;
    NvmV2Signature signatures[]={
        {1,1,tags,tags+1},{1,1,tags,tags+1},
        {1,1,tags+2,tags+3}}; /* row2 is deliberately unused. */
    ...
    NvmV2Module m={0};m.signatures=(NvmV2Signatures){signatures,3};
    ...
    assert(nvm_sdk_signature_snapshot_prepare(&m,NVM_SDK_GENERATION_MAX_BYTES,&p)==NVM_SDK_OK);
    memset(tags,0,4);free(tags);memset(signatures,0,sizeof signatures);memset(functions,0,sizeof functions);
    memset(imports,0,sizeof imports);memset(callbacks,0,sizeof callbacks);memset(links,0,sizeof links);
    const NvmV2Signatures *copy=nvm_sdk_signature_snapshot_rows(p);assert(copy&&copy->count==3);
