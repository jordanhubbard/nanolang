...
static void test_logic_refused_until_implemented(void) {
    ...
    r = nl_frontend_accept(m, &f);
    CHECK(!r.ok, "Logic is not implemented");
    CHECK(strstr(r.error, "not implemented") != NULL,
          "Logic error names the unpublished implementation");
    nvm_module_free(m);
}
...
int main(void) {
    ...
    test_logic_refused_until_implemented();
