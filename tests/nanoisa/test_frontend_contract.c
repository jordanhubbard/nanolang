...
static void test_goals_published_before_implementation(void) {
    ...
    CHECK(nl_frontend_implemented(NL_FE_SHELL), "Shell is implemented");
    CHECK(nl_frontend_implemented(NL_FE_LOGIC), "Logic is implemented");
...
static void test_scheme_accepts_when_implemented(void) {
    ...
    CHECK(r.ok, "Scheme accepts a verified module");
    if (!r.ok) printf("    %s\n", r.error);
    nvm_module_free(m);
}
...
static void test_actor_accepts_when_implemented(void) {
    ...
    r = nl_frontend_accept(m, &f);
    CHECK(r.ok, "Actor accepts a verified module");
    if (!r.ok) printf("    %s\n", r.error);
    nvm_module_free(m);
}

static void test_dataflow_accepts_when_implemented(void) {
    NvmModule *m = assemble_ok(k_add, "dataflow add");
    NlFrontendFacts f;
    NlFrontendResult r;
    if (!m) return;
    attach_debug(m);
    f = facts_for(NL_FE_DATAFLOW, "add.df");
    r = nl_frontend_accept(m, &f);
    CHECK(r.ok, "Dataflow accepts a verified module");
    if (!r.ok) printf("    %s\n", r.error);
    nvm_module_free(m);
}
...
static void test_shell_accepts_when_implemented(void) {
    ...
    r = nl_frontend_accept(m, &f);
    CHECK(r.ok, "Shell accepts a verified module");
    if (!r.ok) printf("    %s\n", r.error);
    nvm_module_free(m);
}

static void test_logic_accepts_when_implemented(void) {
    NvmModule *m = assemble_ok(k_add, "logic add");
    NlFrontendFacts f;
    NlFrontendResult r;
    if (!m) return;
    attach_debug(m);
    f = facts_for(NL_FE_LOGIC, "add.dl");
    r = nl_frontend_accept(m, &f);
    CHECK(r.ok, "Logic accepts a verified module");
    if (!r.ok) printf("    %s\n", r.error);
    nvm_module_free(m);
}
...
int main(void) {
    ...
    test_shell_accepts_when_implemented();
    test_logic_accepts_when_implemented();
