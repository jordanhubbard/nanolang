/* I validate exact scalar globals and their checked initialization flows. */
...
static void analysis_many(const char *prefix,const uint8_t *tags,uint32_t count,const char *callee,bool accepted) {
    ...
    CHECK(nvm_verify(m).ok==accepted);
    nvm_module_free(m);
}
...
int main(void) {
    ...
    /* Declared but uninitialized globals still refuse public execution. */
