...
static NvmModule *call_fixture(const char *call,const char *helper) {
    ...
    m->ownership_data=realloc(m->ownership_data,124);CHECK(m->ownership_data);
    memset(m->ownership_data+112,0,12);m->ownership_size=124;
    word(m->ownership_data,2);word(m->ownership_data+112,1);m->ownership_data[116]=1;
...
int main(void) {
    ...
    for(unsigned i=0;i<2;i++) {
        ...
        CHECK(result.ok);
        CHECK(!nvm_verify_owned_module(m).ok); /* Runtime remains held. */
        nvm_module_free(m);
    }
