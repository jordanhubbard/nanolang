...
static int64_t nl_main() {
    nl_println_string("Testing fs_walkdir:");
    DynArray* files = dyn_array_new(ELEM_STRING);
    /* unsafe */ {
        files = nl_os_walkdir("modules/std");
    }
    int64_t n = dyn_array_length(files);
    nl_print_string("Found ");
    nl_print_string(int_to_string(n));
    nl_println_string(" files in modules/std");
    int64_t i = 0LL;
    while (i < n && i < 10LL)     {
        const char* basename = "";
        const char* path = nl_array_at_string(files, i);
        /* unsafe */ {
            basename = nl_os_path_basename(path);
        }
        nl_print_string("  ");
        nl_print_string(int_to_string((i + 1LL)));
        nl_print_string(". ");
        nl_println_string(basename);
        i = (i + 1LL);
    }
    return 0LL;
}
