...
I record notable changes here using [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).
...
- I make `return` exit the enclosing function inside match arms; expressions
  supply match values separately.
- I run transitive dependency shadows by default. `--root-shadows-only`
  opts out; source-only C emission does not execute shadows.
- I retain original source-line diagnostics through self-hosted import merging
  without replacing module identity with textual alias rewriting.
...
- I publish native module artifacts in immutable generations, retain supported
...
- I expand NanoISA verification and native AOT aggregate/control-flow coverage.
  Passing supported cases does not establish full backend parity.
