...
test-nanoisa-src-nano: nanoisa_emit nano_virt nanoisa_dump $(NANOISA_OBJECTS) $(NANOISA_UTF8)
	...
	@echo "Checking C seed --emit-nvm of tokenize_result.nano..."
	@$(TIMEOUT_CMD) ./bin/nano_virt src_nano/compiler/tokenize_result.nano \
	...
	@echo "Checking nanoc_integrated.nano emits and assembles..."
	@$(TIMEOUT_CMD) ./bin/nanoisa_emit src_nano/nanoc_integrated.nano \
		-o /tmp/nanolang_cut_a_nanoc_integrated.nasm
	@test -s /tmp/nanolang_cut_a_nanoc_integrated.nasm
	@grep -F ".function main" /tmp/nanolang_cut_a_nanoc_integrated.nasm >/dev/null
	@if grep -F "I refused that program:" /tmp/nanolang_cut_a_nanoc_integrated.nasm >/dev/null; then exit 1; fi
	@$(TIMEOUT_CMD) ./bin/nanoisa asm /tmp/nanolang_cut_a_nanoc_integrated.nasm \
		-o /tmp/nanolang_cut_a_nanoc_integrated.nvm
	@test -s /tmp/nanolang_cut_a_nanoc_integrated.nvm
	@echo "Checking nanoc_v06.nano emits and assembles..."
	@$(TIMEOUT_CMD) ./bin/nanoisa_emit src_nano/nanoc_v06.nano \
		-o /tmp/nanolang_cut_a_nanoc_v06.nasm
	@test -s /tmp/nanolang_cut_a_nanoc_v06.nasm
	@grep -F ".function main" /tmp/nanolang_cut_a_nanoc_v06.nasm >/dev/null
	@if grep -F "I refused that program:" /tmp/nanolang_cut_a_nanoc_v06.nasm >/dev/null; then exit 1; fi
...
.PHONY: test-nanoc-v06-nvm-backend
test-nanoc-v06-nvm-backend: $(COMPILER_C) nanoisa_emit nanoisa_dump nvm2c
	@echo "Running self-hosted nanoc_v06 NanoISA backend tests..."
	@chmod +x tests/test_nanoc_v06_nvm_backend.sh
	@$(TIMEOUT_CMD) bash tests/test_nanoc_v06_nvm_backend.sh $(COMPILER_C)
