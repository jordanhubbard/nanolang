...
- **Shadow Tests** - My project policy requires useful shadows. Missing-shadow enforcement is not universal. My C seed executes selected shadows; my self-hosted native driver and `nano_virt` run root-file shadows before publishing executable output, with a ten-second execution deadline. Source-only C emission does not execute shadows. C-seed FFI exemptions and imported-shadow policy remain open. `make test-language-claims` and `make test-native-shadows` check these boundaries.
