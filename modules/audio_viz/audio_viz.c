...
void nl_audio_viz_init(int64_t format, int64_t channels) {
...
static float nl_audio_viz_get_channel_volume(int64_t channel) {
    if (channel < 0 || channel >= 4) return 0.0f;
    return smoothed_volumes[channel];
}

// Get volume as integer (0-100) for easier use in nanolang
int64_t nl_audio_viz_get_channel_volume_int(int64_t channel) {
    return (int64_t)(nl_audio_viz_get_channel_volume(channel) * 100.0f);
}
...
float nl_audio_viz_get_waveform_sample(int64_t channel, int64_t index) {
...
int64_t nl_audio_viz_get_waveform_size(void) {
    return (int64_t)WAVEFORM_BUFFER_SIZE;
}
