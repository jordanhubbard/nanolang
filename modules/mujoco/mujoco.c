...
static int load_api(void) {
    ...
    const char *candidates[] = {
        ...
        "libmujoco.so",
        "libmujoco.so.3",
        ".nanolang/native/mujoco/lib/libmujoco.so",
        ".nanolang/native/mujoco/lib/libmujoco.so.3",
        ".nanolang/native/mujoco/bin/libmujoco.so",
...
int64_t nl_mj_body_qpos_addr(void *ptr, int64_t id) {
    ...
    int joint_addr = sim->model->body_jntadr[id];
    if (sim->model->body_jntnum[id] <= 0 || joint_addr < 0 || joint_addr >= sim->model->njnt) return -1;
    return (int64_t)sim->model->jnt_qposadr[joint_addr];
}

int64_t nl_mj_body_qvel_addr(void *ptr, int64_t id) {
    NLMjSim *sim = as_sim(ptr);
    if (!valid_sim(sim) || id < 0 || id >= sim->model->nbody) return -1;
    int joint_addr = sim->model->body_jntadr[id];
    if (sim->model->body_jntnum[id] <= 0 || joint_addr < 0 || joint_addr >= sim->model->njnt) return -1;
    return (int64_t)sim->model->jnt_dofadr[joint_addr];
}
