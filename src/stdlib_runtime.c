...
void generate_profiling_system(StringBuilder *sb) {
    ...
    sb_append(sb, "        fprintf(stderr, \"[profile] macOS requires special permissions for 'sample'.\\n\");\n");
    sb_append(sb, "        fprintf(stderr, \"[profile] Try: sudo ./your_binary (or use Instruments.app)\\n\");\n");
    sb_append(sb, "        fprintf(stderr, \"[profile] Alternatively, run: sample <pid> 10 -file /tmp/out.txt\\n\\n\");\n");
    sb_append(sb, "        return;\n");
    sb_append(sb, "    }\n");
    sb_append(sb, "    \n");
    ...
    sb_append(sb, "    \n");
    sb_append(sb, "    /* Start sampling in background */\n");
    sb_append(sb, "    pid_t sampler = fork();\n");
    sb_append(sb, "    if (sampler == 0) {\n");
    sb_append(sb, "        /* Sampler child: run sample command */\n");
