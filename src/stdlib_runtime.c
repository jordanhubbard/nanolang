...
void generate_profiling_system(StringBuilder *sb, const char *profile_output_path) {
    ...
    sb_append(sb, "/* Parse macOS xctrace XML output: count name= attributes in frame elements */\n");
    ...
    sb_append(sb, "    struct NlProfEnt { char name[256]; int count; };\n");
    sb_append(sb, "    int max_funcs = 2048;\n");
    sb_append(sb, "    struct NlProfEnt *entries = (struct NlProfEnt*)calloc((size_t)max_funcs, sizeof(struct NlProfEnt));\n");
    sb_append(sb, "    int num_entries = 0, total_samples = 0;\n");
    sb_append(sb, "    if (entries) {\n");
    sb_append(sb, "        char line[8192];\n");
    sb_append(sb, "        while (fgets(line, sizeof(line), f)) {\n");
    sb_append(sb, "            char *p = line;\n");
    sb_append(sb, "            while ((p = strstr(p, \"name=\\\"\")) != NULL) {\n");
    sb_append(sb, "                p += 6;\n");
    sb_append(sb, "                char *end = strchr(p, '\"');\n");
    sb_append(sb, "                if (!end) break;\n");
    sb_append(sb, "                size_t len = (size_t)(end - p);\n");
    sb_append(sb, "                if (len == 0 || len >= 256) { p = end + 1; continue; }\n");
    sb_append(sb, "                int found = 0;\n");
    sb_append(sb, "                for (int i = 0; i < num_entries; i++) {\n");
    sb_append(sb, "                    if (strncmp(entries[i].name, p, len) == 0 && entries[i].name[len] == '\\0') {\n");
    sb_append(sb, "                        entries[i].count++; found = 1; total_samples++; break;\n");
    sb_append(sb, "                    }\n");
    sb_append(sb, "                }\n");
    sb_append(sb, "                if (!found && num_entries < max_funcs) {\n");
    sb_append(sb, "                    strncpy(entries[num_entries].name, p, len);\n");
    sb_append(sb, "                    entries[num_entries].name[len] = '\\0';\n");
    sb_append(sb, "                    entries[num_entries].count = 1;\n");
    sb_append(sb, "                    num_entries++; total_samples++;\n");
    sb_append(sb, "                }\n");
    sb_append(sb, "                p = end + 1;\n");
    sb_append(sb, "            }\n");
    sb_append(sb, "        }\n");
    sb_append(sb, "        /* Insertion sort by count descending */\n");
    sb_append(sb, "        for (int i = 1; i < num_entries; i++) {\n");
    sb_append(sb, "            struct NlProfEnt tmp = entries[i]; int j = i - 1;\n");
    sb_append(sb, "            while (j >= 0 && entries[j].count < tmp.count) { entries[j+1] = entries[j]; j--; }\n");
    sb_append(sb, "            entries[j+1] = tmp;\n");
    sb_append(sb, "        }\n");
    sb_append(sb, "        /* Emit top 20 nl_ prefixed hotspots */\n");
    sb_append(sb, "        int count = 0;\n");
    sb_append(sb, "        if (total_samples == 0) total_samples = 1;\n");
    sb_append(sb, "        for (int i = 0; i < num_entries && count < 20; i++) {\n");
    sb_append(sb, "            if (strncmp(entries[i].name, \"nl_\", 3) == 0) {\n");
    sb_append(sb, "                double pct = (100.0 * entries[i].count) / total_samples;\n");
    sb_append(sb, "                _nl_profile_json_entry(&count, entries[i].name, entries[i].count, pct);\n");
    sb_append(sb, "            }\n");
    sb_append(sb, "        }\n");
    sb_append(sb, "        free(entries);\n");
    sb_append(sb, "    }\n");
    sb_append(sb, "    fclose(f);\n");
    ...
    sb_append(sb, "    \n");
    sb_append(sb, "    /* Re-entry guard: xctrace --launch re-invokes this binary; restore CWD and\n");
    sb_append(sb, "     * run the program directly without trying to profile again. */\n");
    sb_append(sb, "    if (getenv(\"_NL_PROFILING_ACTIVE\")) {\n");
    sb_append(sb, "        const char *saved_cwd = getenv(\"_NL_PROFILING_CWD\");\n");
    sb_append(sb, "        if (saved_cwd && saved_cwd[0]) { chdir(saved_cwd); }\n");
    ...
    sb_append(sb, "        \n");
    sb_append(sb, "        /* Save CWD and set re-entry guard before forking */\n");
    sb_append(sb, "        char saved_cwd[4096] = {0};\n");
    sb_append(sb, "        if (getcwd(saved_cwd, sizeof(saved_cwd))) {\n");
    sb_append(sb, "            setenv(\"_NL_PROFILING_CWD\", saved_cwd, 1);\n");
    sb_append(sb, "        }\n");
    sb_append(sb, "        setenv(\"_NL_PROFILING_ACTIVE\", \"1\", 1);\n");
    sb_append(sb, "        \n");
    sb_append(sb, "        /* Run program with xctrace, passing all original args */\n");
    sb_append(sb, "        pid_t child = fork();\n");
    sb_append(sb, "        if (child == 0) {\n");
    sb_append(sb, "            /* Child: run xctrace (inherits _NL_PROFILING_ACTIVE + _NL_PROFILING_CWD) */\n");
    sb_append(sb, "            freopen(\"/dev/null\", \"w\", stdout);\n");
    sb_append(sb, "            freopen(\"/dev/null\", \"w\", stderr);\n");
    sb_append(sb, "            /* Build xctrace argv: xctrace record --template ... --output ... --launch -- argv[0..n] */\n");
    sb_append(sb, "            int xc_base = 8; /* fixed args before user argv */\n");
    sb_append(sb, "            char **xc_argv = (char**)malloc(sizeof(char*) * (size_t)(xc_base + argc + 1));\n");
    sb_append(sb, "            if (!xc_argv) { _exit(1); }\n");
    sb_append(sb, "            xc_argv[0] = \"xctrace\";\n");
    sb_append(sb, "            xc_argv[1] = \"record\";\n");
    sb_append(sb, "            xc_argv[2] = \"--template\";\n");
    sb_append(sb, "            xc_argv[3] = \"Time Profiler\";\n");
    sb_append(sb, "            xc_argv[4] = \"--output\";\n");
    sb_append(sb, "            xc_argv[5] = trace_file;\n");
    sb_append(sb, "            xc_argv[6] = \"--launch\";\n");
    sb_append(sb, "            xc_argv[7] = \"--\";\n");
    sb_append(sb, "            for (int i = 0; i < argc; i++) { xc_argv[xc_base + i] = argv[i]; }\n");
    sb_append(sb, "            xc_argv[xc_base + argc] = NULL;\n");
    sb_append(sb, "            execvp(\"xctrace\", xc_argv);\n");
    ...
    sb_append(sb, "        \n");
    sb_append(sb, "        /* Clean up: trace_file is a .trace directory, table_file is a plain file */\n");
    sb_append(sb, "        { char rm_cmd[320]; snprintf(rm_cmd, sizeof(rm_cmd), \"rm -rf %s\", trace_file); system(rm_cmd); }\n");
    ...
    sb_append(sb, "    fflush(stderr);\n");
    sb_append(sb, "    {\n");
    sb_append(sb, "        char saved_cwd2[4096] = {0};\n");
    sb_append(sb, "        if (getcwd(saved_cwd2, sizeof(saved_cwd2))) { setenv(\"_NL_PROFILING_CWD\", saved_cwd2, 1); }\n");
    sb_append(sb, "    }\n");
    sb_append(sb, "    setenv(\"_NL_PROFILING_ACTIVE\", \"1\", 1);\n");
    sb_append(sb, "    \n");
