...
Value builtin_str_index_of(Value *args) {
    ...
    return create_int(nl_cstr_index_of(args[0].as.string_val,
                                       args[1].as.string_val));
}

Value builtin_str_last_index_of(Value *args) {
    if (args[0].type != VAL_STRING || args[1].type != VAL_STRING) {
        fprintf(stderr, "Error: str_last_index_of requires two string arguments\n");
        return create_void();
    }
    return create_int(nl_cstr_last_index_of(args[0].as.string_val,
                                            args[1].as.string_val));
}
