...
bool effect_register(EffectRegistry *reg, const char *name,
    ...
    d->name     = estrdup(name);
    d->op_count = op_count;
    d->ops      = calloc(op_count, sizeof(EffectSysOp));
...
bool effect_register_from_ast(EffectRegistry *reg, ASTNode *node) {
    ...
    EffectSysOp *ops = calloc(count, sizeof(EffectSysOp));
...
bool effect_check_perform(EffectCtx *ctx, ASTNode *node) {
    ...
    EffectSysOp *op = effect_op_lookup(decl, op_name);
    if (!op) {
        char msg[256];
        snprintf(msg, sizeof(msg),
