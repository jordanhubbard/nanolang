...
static unsigned build_ordered_call_args(WorkList *list, ASTNode **args, int arg_count,
                                        Environment *env) {
    static _Thread_local unsigned call_id_counter = 0;
    unsigned call_id = call_id_counter++;
    ...
    switch (expr->type) {
        ...
        case AST_CALL: {
            ...
            /* Special handling for array_get - needs type-specific accessor */
                ...
                snprintf(mapped_name_storage, sizeof(mapped_name_storage), "%s", mapped_name);
                mapped_name = mapped_name_storage;
                ...
                /* C does not specify argument evaluation order. Bind ordinary
                 * call inputs before the call so my source order is observable. */
                emit_literal(list, "({ ");
        ...
        case AST_MODULE_QUALIFIED_CALL: {
            ...
            char c_name_storage[512];
            snprintf(c_name_storage, sizeof(c_name_storage), "%s", c_name);
            c_name = c_name_storage;
            
            emit_literal(list, "({ ");
            unsigned call_id = build_ordered_call_args(list, expr->as.module_qualified_call.args,
                                                       expr->as.module_qualified_call.arg_count, env);
