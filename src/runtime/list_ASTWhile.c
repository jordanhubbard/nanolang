...
static void ensure_capacity_ASTWhile(List_ASTWhile *list, int min_capacity) {
    ...
    struct nl_ASTWhile *new_data = realloc(list->data, sizeof(struct nl_ASTWhile) * new_capacity);
    if (!new_data) {
...
List_ASTWhile* nl_list_ASTWhile_with_capacity(int capacity) {
    nl_list_validate_capacity(capacity, sizeof(*((List_ASTWhile *)0)->data));
...
void nl_list_ASTWhile_push(List_ASTWhile *list, struct nl_ASTWhile value) {
    ensure_capacity_ASTWhile(list, nl_list_next_length(list->length));
...
void nl_list_ASTWhile_insert(List_ASTWhile *list, int index, struct nl_ASTWhile value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(struct nl_ASTWhile) * (list->length - index));
...
struct nl_ASTWhile nl_list_ASTWhile_remove(List_ASTWhile *list, int index) {
    ...
    struct nl_ASTWhile value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(struct nl_ASTWhile) * (list->length - index - 1));
