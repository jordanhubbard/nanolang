...
static void ensure_capacity_ASTCall(List_ASTCall *list, int min_capacity) {
    ...
    struct nl_ASTCall *new_data = realloc(list->data, sizeof(struct nl_ASTCall) * new_capacity);
    if (!new_data) {
...
List_ASTCall* nl_list_ASTCall_with_capacity(int capacity) {
    nl_list_validate_capacity(capacity, sizeof(*((List_ASTCall *)0)->data));
...
void nl_list_ASTCall_push(List_ASTCall *list, struct nl_ASTCall value) {
    ensure_capacity_ASTCall(list, nl_list_next_length(list->length));
...
void nl_list_ASTCall_insert(List_ASTCall *list, int index, struct nl_ASTCall value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(struct nl_ASTCall) * (list->length - index));
...
struct nl_ASTCall nl_list_ASTCall_remove(List_ASTCall *list, int index) {
    ...
    struct nl_ASTCall value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(struct nl_ASTCall) * (list->length - index - 1));
