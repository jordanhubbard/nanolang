#include "list_ASTMatchClause.h"
#include "../generated/compiler_schema.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>

/* Note: The actual struct nl_ASTMatchClause definition must be included */
...
static void ensure_capacity_ASTMatchClause(List_ASTMatchClause *list, int min_capacity) {
    ...
    struct nl_ASTMatchClause *new_data = realloc(list->data, sizeof(struct nl_ASTMatchClause) * new_capacity);
    if (!new_data) {
...
List_ASTMatchClause* nl_list_ASTMatchClause_new(void) {
    return nl_list_ASTMatchClause_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTMatchClause* nl_list_ASTMatchClause_with_capacity(int capacity) {
    List_ASTMatchClause *list = malloc(sizeof(List_ASTMatchClause));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = malloc(sizeof(struct nl_ASTMatchClause) * capacity);
    if (!list->data) {
...
void nl_list_ASTMatchClause_push(List_ASTMatchClause *list, struct nl_ASTMatchClause value) {
    ensure_capacity_ASTMatchClause(list, list->length + 1);
...
void nl_list_ASTMatchClause_insert(List_ASTMatchClause *list, int index, struct nl_ASTMatchClause value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(struct nl_ASTMatchClause) * (list->length - index));
...
struct nl_ASTMatchClause nl_list_ASTMatchClause_remove(List_ASTMatchClause *list, int index) {
    ...
    struct nl_ASTMatchClause value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(struct nl_ASTMatchClause) * (list->length - index - 1));
