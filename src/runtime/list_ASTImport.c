...
static void ensure_capacity_ASTImport(List_ASTImport *list, int min_capacity) {
    ...
    struct nl_ASTImport *new_data = realloc(list->data, sizeof(struct nl_ASTImport) * new_capacity);
    if (!new_data) {
...
List_ASTImport* nl_list_ASTImport_with_capacity(int capacity) {
    nl_list_validate_capacity(capacity, sizeof(*((List_ASTImport *)0)->data));
...
void nl_list_ASTImport_push(List_ASTImport *list, struct nl_ASTImport value) {
    ensure_capacity_ASTImport(list, nl_list_next_length(list->length));
...
void nl_list_ASTImport_insert(List_ASTImport *list, int index, struct nl_ASTImport value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(struct nl_ASTImport) * (list->length - index));
...
struct nl_ASTImport nl_list_ASTImport_remove(List_ASTImport *list, int index) {
    ...
    struct nl_ASTImport value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(struct nl_ASTImport) * (list->length - index - 1));
