 ...
 * ARR_LITERAL, ARR_GET, ARR_LEN, AGG_PACK, AGG_GET. Anything else is refused
 * with an error. CALL_EXTERN is refused because it is the VM FFI / co-process
 * path, not a host C ABI. Embedded NULs, nested arrays, nested records,
 * variants, tuples, and the rest of the string and array libraries stay
 * refused.
 */
