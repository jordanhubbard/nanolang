 ...
 * ARR_LITERAL, ARR_GET, ARR_LEN, ARR_PUSH, AGG_PACK, AGG_GET, bool results
 * as i64, PUSH_BOOL, BOOL_NOT, BOOL_AND, BOOL_OR, PRINT, PRINTLN, ASSERT.
 * Anything else is refused with an error. CALL_EXTERN is refused because it
 * is the VM FFI / co-process path, not a host C ABI. Embedded NULs, nested
 * arrays, nested records, variants, tuples, printing arrays/records, string
 * arrays, and the rest of the string and array libraries stay refused.
 */
