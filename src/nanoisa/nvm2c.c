...
static void emit_function_body(Nvm2cBuf *b, const NvmModule *mod, uint32_t idx,
    ...
    for (i = 0; i < fn->local_count; i++) {
        ...
        if (lk == NVM2C_VK_INT) {
            nvm2c_printf(b, "    uint8_t lt%u = TAG_INT;\n", (unsigned)i);
        }
        nvm2c_printf(b, "    (void)l%u;\n", (unsigned)i);
    }
    nvm2c_printf(b, "    int64_t t[%d] = {0};\n", NVM2C_MAX_TEMPS);
    nvm2c_puts(b, "    (void)t;\n");
    nvm2c_printf(b, "    uint8_t tt[%d] = {0};\n", NVM2C_MAX_TEMPS);
    nvm2c_puts(b, "    (void)tt;\n");
    ...
    while (pc < remaining) {
        ...
        case OP_DUP: {
            ...
            }
            {
                ...
                if (k == NVM2C_VK_STR) {
                    ...
                    int t = stack_push_temp(b, &st, rhs);
                    if (t >= 0) nvm2c_printf(b, "    tt[%d] = tt[%d];\n", t, src);
                }
            }
            break;
        }
        ...
        case OP_LOAD_LOCAL: {
            ...
            if (fn_local_kind(kinds, idx, slot) == NVM2C_VK_STR) {
                ...
                int t = stack_push_temp(b, &st, rhs);
                if (t >= 0) nvm2c_printf(b, "    tt[%d] = lt%u;\n", t, (unsigned)slot);
            }
            break;
        }
        case OP_STORE_LOCAL: {
        ...
        case OP_NE: {
            ...
            if (lk == NVM2C_VK_INT && rk == NVM2C_VK_INT) {
                char expr[64];
                snprintf(expr, sizeof expr, "(tt[%d] == tt[%d] && t[%d] %s t[%d])",
                          lhs, rhs, lhs, ins.opcode == OP_EQ ? "==" : "!=", rhs);
                if (ins.opcode == OP_NE) {
                    snprintf(expr, sizeof expr, "(tt[%d] != tt[%d] || t[%d] != t[%d])",
                             lhs, rhs, lhs, rhs);
                }
                stack_push_temp(b, &st, expr);
        ...
        case OP_HM_GET: {
            int key = stack_pop_expect(b, &st, NVM2C_VK_INT, "HM_GET key");
            int map = stack_pop_expect(b, &st, NVM2C_VK_HM, "HM_GET map");
            if (b->failed) goto done;
            int t = stack_push_temp(b, &st, "0");
            nvm2c_printf(b, "    tt[%d] = nhmap_get(h[%d], (nscalar_t){tt[%d], t[%d]}, &t[%d]);\n",
                         t, map, key, key, t);
            break;
        }
        case OP_HM_SET: {
            int value = stack_pop_expect(b, &st, NVM2C_VK_INT, "HM_SET value");
            int key = stack_pop_expect(b, &st, NVM2C_VK_INT, "HM_SET key");
            int map = stack_pop_expect(b, &st, NVM2C_VK_HM, "HM_SET map");
            if (b->failed) goto done;
            nvm2c_printf(b, "    nhmap_set(h[%d], (nscalar_t){tt[%d], t[%d]}, (nscalar_t){tt[%d], t[%d]});\n",
                         map, key, key, value, value);
            stack_push_hm(b, &st, "h[0]");
            nvm2c_printf(b, "    h[%d] = h[%d];\n", st.slots[st.sp - 1], map);
            break;
        }
        case OP_I64_NE:
            emit_binop(b, &st, "!=");
...
char *nvm2c_emit(const NvmModule *mod, char *err, size_t err_len) {
...

    {
        ...
        int need_hm = module_has_opcode(mod, OP_HM_NEW);
        uint32_t i;
        ...
        nvm2c_puts(&b,
            ...
            "#include <stddef.h>\n"
            "#include <stdint.h>\n"
            "enum { TAG_VOID = 0, TAG_INT = 1, TAG_BOOL = 3 };\n");
