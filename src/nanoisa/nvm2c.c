...
static const char *c_result_type(const NvmFunctionEntry *fn, uint8_t array_kind) {
    ...
    if (fn->result_tag == TAG_ARRAY)
        return array_kind == NVM2C_VK_ARR ? "narr_t" :
               array_kind == NVM2C_VK_SARR ? "nsarr_t" : "nrarr_t";
        ...
        case OP_TAIL_CALL: {
            ...
            if (ins.opcode == OP_CALL) {
                if (cf->result_count == 1 && cf->result_tag == TAG_STRING) {
                    ...
                    result.kind = resolved_shape_kind(b, b->shape_results[callee]);
                    result.origin = -1;
        ...
        case OP_HALT: {
            if (fn->result_count == 1 &&
                ...
                if (fn->result_tag == TAG_STRING) {
                    ...
                    uint8_t result_kind = resolved_shape_kind(b, v.shape);
                    if (result_kind != NVM2C_VK_UNK)
                        mark_origin(local_kind, nloc, v.origin, result_kind);
                    if (v.kind == NVM2C_VK_RARR &&
...
static int build_direct_call(Nvm2cBuf *b, Nvm2cStack *st, const NvmModule *mod,
    ...
    const NvmFunctionEntry *cf = &mod->functions[callee];
    if (c_result_type(cf, resolved_shape_kind(b, b->shape_results[callee])) == NULL) {
    ...
    while (pc < remaining) {
        ...
        case OP_CALL: {
            ...
            if (result_is_i64(cf)) {
                ...
                uint8_t result_kind = resolved_shape_kind(b, b->shape_results[callee]);
                if (result_kind == NVM2C_VK_ARR) stack_push_arr(b, &st, call);
                else if (result_kind == NVM2C_VK_SARR) stack_push_sarr(b, &st, call);
                else {
