...
static int mark_origin(Nvm2cBuf *b, uint8_t *local_kind, uint16_t nloc,
                        uint8_t *global_kind, uint32_t nglobal,
    ...
    if (origin <= -2) {
        ...
        if (slot < nglobal) return merge_kind(b, &global_kind[slot], kind, "global");
    }
    return 1;
}

static int mark_slot_origin(Nvm2cBuf *b, uint8_t *local_kind, uint8_t *rec_fields,
                            uint16_t nloc, uint8_t *global_kind,
                            uint8_t *global_rec_fields, uint32_t nglobal,
                            Nvm2cSimSlot slot, uint8_t kind) {
    if (slot.origin_field >= 0) {
        if (slot.origin >= 0 && (uint16_t)slot.origin < nloc) {
            return merge_kind(b, rec_fields +
                              (size_t)slot.origin * NVM2C_MAX_REC_FIELDS +
                              slot.origin_field, kind, "record field");
        }
        if (slot.origin <= -2) {
            uint32_t global = (uint32_t)(-slot.origin - 2);
            if (global < nglobal) {
                return merge_kind(b, global_rec_fields +
                                  (size_t)global * NVM2C_MAX_REC_FIELDS +
                                  slot.origin_field, kind, "global record field");
            }
        }
    }
    return mark_origin(b, local_kind, nloc, global_kind, nglobal,
                       slot.origin, kind);
}
...
static int mark_slot_str_origin(Nvm2cBuf *b, uint8_t *local_kind,
                                ...
                                uint8_t *global_rec_fields, uint32_t nglobal,
                                Nvm2cSimSlot slot) {
    return mark_slot_origin(b, local_kind, rec_fields, nloc, global_kind,
                            global_rec_fields, nglobal, slot, NVM2C_VK_STR);
}
...
static int classify_function(Nvm2cBuf *b, const NvmModule *mod, uint32_t idx,
                              ...
                              uint8_t *global_kind, uint8_t *global_rec_fields,
    ...
    while (pc < remaining) {
        ...
        case OP_LOAD_GLOBAL: {
            ...
            memset(&loaded, 0, sizeof loaded);
            loaded.kind = global_kind[slot];
            loaded.origin = -(int)slot - 2;
            loaded.origin_field = -1;
            if (loaded.kind == NVM2C_VK_REC || loaded.kind == NVM2C_VK_RARR) {
                memcpy(loaded.rec_k, global_rec_fields +
                       (size_t)slot * NVM2C_MAX_REC_FIELDS, NVM2C_MAX_REC_FIELDS);
            }
            if (!sim_push_slot(b, idx, stk, &sp, loaded)) return 0;
            break;
        }
        case OP_STORE_GLOBAL: {
            ...
            if (!sim_pop(b, idx, stk, &sp, &v)) return 0;
            before = global_kind[slot];
            if (!merge_kind(b, &global_kind[slot], v.kind, "global")) return 0;
            if (before != global_kind[slot]) *changed = 1;
            if (v.kind == NVM2C_VK_REC || v.kind == NVM2C_VK_RARR) {
                uint8_t *dst = global_rec_fields +
                    (size_t)slot * NVM2C_MAX_REC_FIELDS;
                for (i = 0; i < NVM2C_MAX_REC_FIELDS; i++) {
                    before = dst[i];
                    if (!merge_kind(b, &dst[i], v.rec_k[i], "global record field")) return 0;
                    if (before != dst[i]) *changed = 1;
                }
            }
            break;
        }
        ...
        case OP_STR_LEN: {
            Nvm2cSimSlot v;
            if (!sim_pop(b, idx, stk, &sp, &v)) return 0;
            if (!mark_slot_str_origin(b, local_kind, rec_fields, nloc,
                                      global_kind, global_rec_fields,
                                      global_count, v)) return 0;
            if (!sim_push(b, idx, stk, &sp, NVM2C_VK_INT, -1)) return 0;
            break;
        }
        case OP_STR_CONCAT: {
            ...
            if (!mark_slot_str_origin(b, local_kind, rec_fields, nloc,
                                      global_kind, global_rec_fields,
                                      global_count, rhs) ||
                !mark_slot_str_origin(b, local_kind, rec_fields, nloc,
                                      global_kind, global_rec_fields,
                                      global_count, lhs)) return 0;
            if (!sim_push(b, idx, stk, &sp, NVM2C_VK_STR, -1)) return 0;
            break;
        }
        case OP_STR_SUBSTR: {
        ...
        case OP_ARR_LEN: {
            ...
            if (v.kind == NVM2C_VK_RARR) {
                if (!mark_slot_origin(b, local_kind, rec_fields, nloc,
                                      global_kind, global_rec_fields, global_count,
                                      v, NVM2C_VK_RARR)) return 0;
            } else if (v.kind == NVM2C_VK_SARR) {
                if (!mark_slot_origin(b, local_kind, rec_fields, nloc,
                                      global_kind, global_rec_fields, global_count,
                                      v, NVM2C_VK_SARR)) return 0;
            } else {
                if (!mark_slot_origin(b, local_kind, rec_fields, nloc,
                                      global_kind, global_rec_fields, global_count,
                                      v, NVM2C_VK_ARR)) return 0;
            }
            if (!sim_push(b, idx, stk, &sp, NVM2C_VK_INT, -1)) return 0;
            break;
        }
        case OP_ARR_GET: {
            ...
            if (arr.kind == NVM2C_VK_RARR) {
                ...
                if (!mark_slot_origin(b, local_kind, rec_fields, nloc,
                                      global_kind, global_rec_fields, global_count,
                                      arr, NVM2C_VK_SARR)) return 0;
                if (!sim_push(b, idx, stk, &sp, NVM2C_VK_STR, -1)) return 0;
            } else {
                if (!mark_slot_origin(b, local_kind, rec_fields, nloc,
                                      global_kind, global_rec_fields, global_count,
                                      arr, NVM2C_VK_ARR)) return 0;
                if (!sim_push(b, idx, stk, &sp, NVM2C_VK_INT, -1)) return 0;
            }
            break;
        }
        case OP_ARR_PUSH: {
            ...
            if (val.kind == NVM2C_VK_REC) {
                ...
                } else if (arr.origin <= -2) {
                    uint32_t global = (uint32_t)(-arr.origin - 2);
                    if (global < global_count) {
                        memcpy(global_rec_fields +
                               (size_t)global * NVM2C_MAX_REC_FIELDS,
                               val.rec_k, NVM2C_MAX_REC_FIELDS);
                    }
                }
            ...
            if (fn->result_count == 1 &&
                ...
                if (fn->result_tag == TAG_STRING) {
                    ...
                    if (!merge_kind(b, &result_kinds[idx], v.kind, "array result") ||
                        !mark_slot_origin(b, local_kind, rec_fields, nloc,
                                          global_kind, global_rec_fields,
                                          global_count, v,
...
static void emit_function_body(Nvm2cBuf *b, const NvmModule *mod, uint32_t idx,
                               ...
                               const uint8_t *global_kind,
                               const uint8_t *global_rec_fields,
                               uint32_t global_count) {
    const NvmFunctionEntry *fn = &mod->functions[idx];
    const char *rt = c_result_type(fn, result_kinds[idx]);
    ...
    while (pc < remaining) {
        ...
        case OP_LOAD_GLOBAL: {
            ...
            if (kind == NVM2C_VK_STR) stack_push_str(b, &st, rhs);
                ...
                int r = stack_push_rec(b, &st, rhs);
                if (r >= 0) memcpy(st.rec_k[r], global_rec_fields +
                                   (size_t)slot * NVM2C_MAX_REC_FIELDS,
                                   NVM2C_MAX_REC_FIELDS);
            } else if (kind == NVM2C_VK_RARR) {
                int a = stack_push_rarr(b, &st, rhs);
                if (a >= 0) memcpy(st.rec_k[a], global_rec_fields +
                                   (size_t)slot * NVM2C_MAX_REC_FIELDS,
                                   NVM2C_MAX_REC_FIELDS);
...
static int module_has_global_op(const NvmModule *mod, const uint8_t *reachable,
                                uint8_t op, uint32_t slot) {
    uint32_t i;
    for (i = 0; i < mod->function_count; i++) {
        if (!reachable[i]) continue;
        const NvmFunctionEntry *fn = &mod->functions[i];
...
static int module_global_count(Nvm2cBuf *b, const NvmModule *mod,
                               const uint8_t *reachable, uint32_t *out) {
    uint32_t count = 0;
    uint32_t i;
    for (i = 0; i < mod->function_count; i++) {
        if (!reachable[i]) continue;
...
static void emit_globals(Nvm2cBuf *b, const NvmModule *mod,
    ...
    for (i = 0; i < count; i++) {
        if (!module_has_global_op(mod, reachable, OP_LOAD_GLOBAL, i) &&
            !module_has_global_op(mod, reachable, OP_STORE_GLOBAL, i)) continue;
...
char *nvm2c_emit(const NvmModule *mod, char *err, size_t err_len) {
    ...
    uint8_t *global_kind = NULL;
    uint8_t *global_rec_fields = NULL;
    ...
    if (!module_global_count(&b, mod, reachable, &global_count)) goto fail;
    global_kind = calloc(global_count ? global_count : 1, 1);
    global_rec_fields = calloc((global_count ? global_count : 1) *
                               NVM2C_MAX_REC_FIELDS, 1);
    if (!global_kind || !global_rec_fields) {
        nvm2c_fail(&b, "out of memory");
        goto fail;
    }
    memset(global_kind, NVM2C_VK_UNK, global_count);
    memset(global_rec_fields, NVM2C_VK_UNK,
           (size_t)global_count * NVM2C_MAX_REC_FIELDS);

    {
        uint32_t pass;
        uint32_t limit = mod->function_count * NVM2C_MAX_LOCALS + global_count + 1;
        ...
        if (pass > limit) {
            nvm2c_fail(&b, "parameter, global, and record-field classification did not converge");
            goto fail;
        }
        ...
        for (pass = 0; pass < global_count; pass++) {
            if (global_kind[pass] == NVM2C_VK_UNK) {
                if (module_has_global_op(mod, reachable, OP_LOAD_GLOBAL, pass)) {
                    nvm2c_fail(&b, "global %u is loaded but has no inferred stored value", pass);
                    goto fail;
                }
                global_kind[pass] = NVM2C_VK_INT;
            }
        }
    }
...

    {
        ...
        int need_iarr = need_iarr_lit ||
            ...
            module_has_global_kind(global_kind, global_count, NVM2C_VK_ARR);
        int need_sarr = need_sarr_lit ||
            ...
            module_has_global_kind(global_kind, global_count, NVM2C_VK_SARR);
        int need_rarr = module_has_local_kind(kinds, mod->function_count, NVM2C_VK_RARR) ||
            ...
            module_has_global_kind(global_kind, global_count, NVM2C_VK_RARR);
        ...
        emit_globals(&b, mod, reachable, global_kind, global_count);
    }
