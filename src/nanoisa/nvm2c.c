...
static int classify_function(Nvm2cBuf *b, const NvmModule *mod, uint32_t idx,
    ...
    while (pc < remaining) {
        ...
        case OP_DUP: {
            ...
            if (!sim_push(b, idx, stk, &sp, stk[sp - 1].kind, stk[sp - 1].origin)) return 0;
            break;
        }
        case OP_POP: {
            Nvm2cSimSlot dumped;
        ...
        case OP_SWAP: {
            ...
            if (!sim_push(b, idx, stk, &sp, x.kind, x.origin)) return 0;
            if (!sim_push(b, idx, stk, &sp, y.kind, y.origin)) return 0;
            break;
        }
        case OP_LOAD_LOCAL: {
            ...
            if (!sim_push(b, idx, stk, &sp, local_kind[slot], (int)slot)) return 0;
            break;
        }
        case OP_STORE_LOCAL: {
            uint16_t slot = ins.operands[0].u16;
...
static void emit_function_body(Nvm2cBuf *b, const NvmModule *mod, uint32_t idx,
    ...
    while (pc < remaining) {
        ...
        case OP_ARR_LITERAL: {
            ...
            int ei;
            if (tag != TAG_INT) {
                nvm2c_fail(b, "function %u: ARR_LITERAL only supports int elements", idx);
                goto done;
            }
            ...
            for (ei = (int)count - 1; ei >= 0; ei--) {
                elems[ei] = stack_pop_expect(b, &st, NVM2C_VK_INT, "ARR_LITERAL");
                if (b->failed) goto done;
            }
            if (count == 0) {
        ...
        case OP_ARR_LEN: {
            int arr = stack_pop_expect(b, &st, NVM2C_VK_ARR, "ARR_LEN");
            if (b->failed) goto done;
            char expr[64];
        ...
        case OP_ARR_GET: {
            ...
            int arr = stack_pop_expect(b, &st, NVM2C_VK_ARR, "ARR_GET array");
            if (b->failed) goto done;
            char expr[80];
        ...
        case OP_ARR_PUSH: {
            int val = stack_pop_expect(b, &st, NVM2C_VK_INT, "ARR_PUSH value");
            int arr = stack_pop_expect(b, &st, NVM2C_VK_ARR, "ARR_PUSH array");
            if (b->failed) goto done;
            char expr[80];
        ...
        case OP_AGG_PACK: {
            ...
            int ei;
            if (kind != AGG_RECORD) {
                nvm2c_fail(b, "function %u: AGG_PACK only supports int-field records", idx);
                goto done;
            }
            ...
            for (ei = (int)count - 1; ei >= 0; ei--) {
                elems[ei] = stack_pop_expect(b, &st, NVM2C_VK_INT, "AGG_PACK");
                if (b->failed) goto done;
            }
            if (st.next_rec >= NVM2C_MAX_TEMPS) {
                nvm2c_fail(b, "too many record temporaries");
...
static void emit_nstr_helpers(Nvm2cBuf *b) {
    nvm2c_puts(b,
...
char *nvm2c_emit(const NvmModule *mod, char *err, size_t err_len) {
...

    {
        ...
        if (need_concat || need_arr_lit || need_arr_get || need_arr_push ||
        ...
        if (need_concat) emit_nstr_helpers(&b);
        if (need_arr_lit || need_arr_push) {
            emit_narr_arena(&b);
        }
        if (need_arr_lit) emit_narr_lit(&b);
        if (need_arr_get) emit_narr_get(&b);
        if (need_arr_push) emit_narr_push(&b);
    }
...

    {
        uint32_t i;
        for (i = 0; i < mod->function_count; i++) {
            emit_function_body(&b, mod, i, kinds);
            if (b.failed) goto fail;
        }
    }
