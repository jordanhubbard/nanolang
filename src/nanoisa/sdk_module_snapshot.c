...
typedef struct { size_t bytes,payloads,signature_bytes; uint32_t work; } ModuleSnapshotPlan;
...
static bool module_work(ModuleSnapshotPlan *p,uint32_t count) {
    if(count>NVM_SDK_GENERATION_MAX_WORK-p->work)return false;
...
static NvmSdkResult module_plan(const NvmV2Module *m,size_t limit,ModuleSnapshotPlan *out) {
    if(!m||!out)return NVM_SDK_INVALID;
    ModuleSnapshotPlan p={.bytes=sizeof(NvmSdkModuleSnapshot)};
...
#undef PLAN_TABLE
    ...
    uint32_t signature_limit=((NVM_SDK_GENERATION_MAX_WORK-p.work)/3)*2;
...
NvmSdkResult nvm_sdk_module_snapshot_prepare(const NvmV2Module *source,size_t limit,
    ...
    NvmSdkResult result=module_plan(source,limit,&plan);
    if(result!=NVM_SDK_OK)return result;
