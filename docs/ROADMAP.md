...
- [ ] I lower exact tuple values through my canonical NanoISA source path (`task_6d2da9bf149349839fd544a5475d6efd`). I preserve ordered element types and left-to-right evaluation for literals, typed locals, parameters, calls, returns and numeric projections; I emit `AGG_TUPLE`/`TAG_TUPLE` rather than substituting an arbitrary record. My native translator must retain that distinct runtime kind while reusing its checked aggregate storage, and both VM and sanitized native products must pass positive mixed-scalar cases plus arity, element-type and projection-bound refusals. This bounded repair does not claim arrays, unions, resources or full product acceptance.
