...
- [ ] I qualify scalar callback source boundaries (`task_d0997d4a11184689ac99a91b430340de`, parent500906): direct reduce-result bit observation lacks selfhost FLOAT inference, and C-seed canonical map/reduce emits FUNCREF refused by native translation. I preserve the fresh checked-refusal log, retain exact refusal/output assertions, and distinguish direct arithmetic/optimized interpreter success from missing paired callable coverage. I do not widen native callable admission in the scalar helper patch. The first typed-local callback aggregate gate additionally reaches selfhost legacy nl_reduce's int-only ABI with a double callback and fails its generated shadow array-kind assertion; I preserve that failed gate and do not replay the legacy route. A separately named supported-route subset cannot close this callback task or parent500906. I now execute the [preimplementation contract](NANOISA_SCALAR_REDUCE_CONTRACT.md) from fresh main85fd6a52: exact reduce signature/result inference, existing homogeneous helper selection and source-order snapshots; fresh post-repair controls replace no historical failed artifact. Canonical FUNCREF remains separately refused, so the full callback parent stays open.
  - [ ] I specialize resolved same-module named scalar map/reduce callbacks in the C-seed canonical producer using existing direct CALL (task_fd15bbaf8acd4e448347ce3cce4e7a1d; [preimplementation contract](NANOISA_NAMED_SCALAR_CALLBACKS.md)). I preserve exact binding/body/signature identity, source order, empty/alias behavior and computed callback fallback; paired int/float/bool VM/native/Cseed/Stage1/Stage2 bit gates precede closure; U8 retains existing fallback until its selfhost functional ABI is qualified. Failed-worker d099 history, general callable acceptance and scalar5009 remain open.
    ...
    - [ ] I complete the C-seed exact reduce signature check under collection callback parent task_75b340982b6cf797f29b38c1a188aab3: the first named-callback negative fixture accepts `fn(int,float)->int` with a FLOAT initializer. No mismatched callback was executed; I retain the log and require checker/prior-output qualification before claiming this refusal.
