...
- [ ] I correct stale auxiliary LLVM/Wasm example help, retired AST runner references and missing browser-interpreter build sources (MAC `task_26dc7883efaf4b7a83e83128a10f99a7`). These are distinct from implementing NanoISA translators. My bounded contract removes retired direct runner modes and stale example help, makes the unmaintained browser-interpreter rebuild fail explicitly before output mutation, and documents the retained historical browser bundle. Existing direct C execution and RISC-V/PTX validation reporting remain unchanged; focused runner/build-contract checks must pass. I do not claim a current browser interpreter build or broader NanoISA backend support.
