...
- [ ] I separately qualify aggregate float operators against the selected scalar arithmetic result policy (`task_3717eda847a74122916571444478ee0f`). VM array arithmetic, interpreter array/broadcast loops, C array runtime helpers and selfhost element loops have independent implementations; scalar callback bodies remain included in task500906. I record an explicit aggregate policy and paired acceptance before claiming array coverage, preserve foreign math/transport contracts and keep full arithmetic/release obligations open.
