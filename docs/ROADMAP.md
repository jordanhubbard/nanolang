...
- [x] I recognize only an immediate same-block `PUSH_BOOL 0; ASSERT` as terminal in native classification/emission, reset the proof at branch entries/skipped instructions and resume independent incoming successors normally. I retain reachable HALT restrictions and never fabricate float/string defaults. The unchanged scalar match fixture passes VM but native refuses its unreachable empty-stack HALT; log `/tmp/nanolang-scalar-match-values-paired.log`. Contract: `docs/NATIVE_FALSE_ASSERT.md`; MAC `task_d86843ef53d343bd8881dd419b072532`, prerequisite of `task_b43095db71f74c0b8418c27530f1686a`. Four focused GCC/Clang sanitizer methods, 2,422 native/1,269 shape checks and the unchanged twelve-method source suite pass; `docs/evidence/native-false-assert.md` records the exact dependency tool. Canonical task closure awaits merge.
