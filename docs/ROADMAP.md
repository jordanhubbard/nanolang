...
- [ ] I preserve optional scalar payloads in native aggregate storage (`task_497e1ba5b9544b81b3614ec37da90e98`). Fresh product bootstrap9808acf6 passes Stage1 hello but refuses optional-to-int storage during Stage2 translation (retained `/tmp/nanolang-product-through597-bootstrap.log`). I extend only conversion-inferred scalar destination storage, preserve exact constraints and checked unboxing, and require focused mixed present/optional storage plus full fresh product bootstrap. Historical export-shadow aborts remain separate. My initial shape-only correction exposed missing int/bool projection acceptance in fresh mixed-record controls (`/tmp/nanolang-optional-scalar-focused.log`); I record this before completing scalar storage seeding and projection. My fresh `b8f3b842` product bootstrap builds Stage2 but hello stops at a native invariant in `parse_function_definition`, generated C line 46847. Independent source-only emission locates a strict string storage-kind guard. I preserve the failed artifact and review typed projection of boxed present scalars before further acceptance; no cause is attributed from the guard alone.
...
- [ ] I resume callers after verified nested implicit returns in NanoVM (MAC `task_4b3800f46af143fbb171f9565f92b8e0`). My ordinary scalar fixture verifies but exits with helper result 9 before its caller returns 0; I preserve `/tmp/nanolang-llvm-implicit-tests.log`. The scalar LLVM profile refuses non-entry implicit exits until this separate runtime boundary is repaired.
- [ ] I lower verified implicit returns in C AOT with declared result tags and existing root lifetimes (MAC `task_8a18a76c86884299ac3f7880ea617978`). Entry fallthrough already verifies and executes in VM but C translation refuses it; I preserve `/tmp/nanolang-llvm-profile-tests.log` and keep LLVM implicit exits outside the first common profile.
