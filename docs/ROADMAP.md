...
- [ ] I retain exclusive source endpoints for non-block match arm expressions so existing checker scope bounding restores outer/sibling bindings without discarding emission metadata. I require same-line, multiline, nested and escaped-binding controls through C-native/NanoVirt and selfhost native stages; no global lookup or ownership relaxation. Contract: `docs/MATCH_EXPRESSION_SCOPE.md`; MAC `task_2faf762553284557ad77a9f7f541801b`. This precedes scalar expression-match NanoISA admission.
- [ ] I restore exact checked native match-arm payload metadata before emission rechecks, retaining file/location/nominal identity, lexical endpoints and declared resource classification without copying transient ownership state. My ordinary multiline/nested source gate emits spurious E001 despite compiler success; I retain `/tmp/nanolang-match-expression-scope-tests.log`. MAC `task_109daee0b08147bc89d813211034d89f`; this companion precedes complete expression-scope acceptance.
