...
- [x] I preserve my strict Darwin formatting fault-injection harness (`task_8674e6954ba44d3da54ea5809d0bba9c`) before aggregate policy qualification. The current SDK already defines a fortified `snprintf` macro, so the substring test translation unit undefines only that existing macro before installing its private `format_snprintf` fault injector. I keep production source, fortification, flags, assertions and deadlines unchanged. Corrected Darwin and Linux substring/NanoVM gates each pass 274,541 VM checks; I retain the originating failed-gate boundary and my [bounded evidence](evidence/darwin-format-fault-harness.md). Aggregate policy and release publication remain open.
