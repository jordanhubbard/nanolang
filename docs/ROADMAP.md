...
- [ ] I lower ownership-preserving source if/else and while around checked borrowed calls (MAC `task_89c4d7131a664f5a8aadbbbbc4b5389e`). My [bounded contract](NANOISA_SOURCE_BORROW_CONTROL_FLOW.md) preserves exact owner/reference and initialized-scalar joins, balanced regions, real branch/backedge execution and all selected shadows in both producers. I require paired metadata/code/names, VM/sanitized native behavior and ordinary publication refusals; branch-local initialization, path-sensitive moves and early exits remain separate.
