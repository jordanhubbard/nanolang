...
- [ ] I match operation-specific enum coercion in typed integer instructions (`task_a77ca354773245f9a8b5f490fe336c06`). My native typed binary arithmetic/comparison boundary now normalizes checked int/enum operands while preserving original producer tags and all other exact integer guards. Paired GCC/Clang controls and 2,422 native/1,139 shape checks pass; LLVM/Wasm acceptance remains pending. I retain typed unary and generic MOD/NEG distinctions. [Contract and evidence](NATIVE_TYPED_ENUM_COERCIONS.md).
...
- [ ] I admit lexical int/bool declarations in borrowed-source control flow after definite-initialization analysis, preserving initializer order, lexical restoration, names, shadows and paired VM/native execution (MAC `task_d3fdac5a43784608851272407743977b`). Resource moves and early exits remain separate.
