...
- [ ] I preserve exact binary64 operand facts (`task_18f9dbfe2abf41fb9159ba9f8c84584d`) before admitting reconstructed float source. My additive `f64_bits` field is exactly 16 lowercase hexadecimal digits copied without numeric conversion; existing `arg`, signatures and executable admission remain unchanged. I check signed zeros, finite boundaries, infinities and NaN payload/signaling bits plus unchanged int/bool facts and output-preserving source refusal.
