...
- [ ] I retain my native Darwin linker for arithmetic LTO qualification (`task_39536e6a885e47b88bed4445bb289e5e`). My first explicit LLVM build passes, but unconditional Clang LLD selection fails parsing SDK27 arm64e.x1 TAPI targets. I preserve the failure, keep LTO/sanitizers/bit assertions unchanged and qualify Apple clang with its default Apple linker; non-Darwin Clang retains LLD. [Contract](DARWIN_ARITHMETIC_GRAPHS_732.md).

- [ ] I make my generated graph allocation controls portable (`task_f0a43b04e0c3444fb922e95084664bbb`). Apple ld rejects GNU malloc wrapping after five graph methods pass. I preserve that outcome and substitute only the generated test module malloc symbol before its existing ASan pass, keeping every budget, cleanup/recovery assertion and target route. [Contract](DARWIN_ARITHMETIC_GRAPHS_732.md).
