...
- [x] I restore integrated affine-resource example shadow compilation (`task_c4351c720aee424ea9b90187e51a08f2`). Frozen e9a5f55f passes canonical component builds/entries but fails 1 of 244 eligible VM examples at `nl_affine_resource_demo.nano`: the source borrow profile refuses its shadow helper topology. I retain [the failed gate](evidence/product-acceptance-e9a5f55f.md), qualified PR761 source/runtime controls and the [new installed-product244/244 example result](evidence/product-acceptance-cd72dab3.md), preserving eligibility and every original shadow. The independent documentation/full-product gate remains open.
...
- [x] I invoke my NanoCore reference evaluator through direct arguments and pipe transport, preserving literal expression bytes and compiler paths instead of constructing a shell command (`task_3d5bf23a0b40466aa8ba4e7c84e31e01`). I test valid quoted strings, spaced paths and long expressions with a benign evaluator; formal correspondence remains separate. I complete this bounded prerequisite through PR638; fresh combined product acceptance remains open.
...
- [ ] I restore NanoVirt match-arm lexical scope (`task_6d9b923cd7854003a8430dddde22d4b2`, required d00106). My [preimplementation contract](NANOVIRT_MATCH_BINDING_SCOPE_CONTRACT.md) separates lexical binding indexes from allocated slot highwater, preserves runtime ownership/control flow, and requires fresh corrected-source shadow/guard/loop acceptance. I retain the first frozen paired guarded failure without replay or a full-gate success claim.

- [ ] I qualify installed-product affine acceptance after frozen 950f36d1 (`task_e8d860a16da0464891dd32e91c42bef1`). My [first terminal evidence and complete55-subcase classification](evidence/product-acceptance-950f36d1.md) retains all244 example/docs158 acceptance followed by checked positive-source refusals. I require dependency-ordered exact source/runtime contracts and unchanged tests/shadows; no negative-phase failure or unrelated runner defect accounts for these55. Full product/ownership parents remain open.
