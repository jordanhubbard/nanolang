...
- [ ] I qualify the borrowed-source harness setup deadline (`task_ef9261a163f64e0397a93d377d623ed7`). Frozen product63c26ecd records a setUpClass error while compiling its large tools under the ordinary180-second limit. I preserve the exact traceback, measure that setup separately with source/tool hashes, and adjust only setup if justified. Ordinary assertion budgets remain unchanged; all source-borrow methods must pass before completion. [Contract](BORROW_CANONICAL_SETUP.md).
