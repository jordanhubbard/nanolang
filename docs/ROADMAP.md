...
- [ ] I lower my canonical `abs`, `map`, `reduce` and `filter` calls (`task_c83543611db54102b1f75f7a94f9e93d`, also required by parent2578). I resolve declared callback identity and exact scalar signatures before lowering, evaluate inputs once in source order, capture input length before callbacks, and preserve empty result element types and fresh output ownership. I retain explicit indirect/unresolved callback refusals and execute ordinary core examples through NanoISA with mandatory shadows; I do not use my legacy C route as a substitute. [Contract](NANOISA_FUNCTIONAL_ARRAYS.md).
