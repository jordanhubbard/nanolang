...
- [ ] I establish explicit underscore union-payload pattern semantics across producers (MAC `task_fc39603161314568929719b36d25f14b`). Static audit finds C native omission versus selfhost `nl__` declaration; wildcard arms are distinct. Ordinary let-name repair does not claim this pattern contract. My bounded contract makes `Variant(_)` a payload discard: it creates no binding, so an existing outer `_` remains visible. I retain ordinary `let _` names and wildcard-arm behavior, require checker/interpreter/native/VM consistency on supported routes, and reject reading a discarded payload when no outer name exists. I test normal and guarded arms, expression and statement contexts, outer-name preservation and ordinary named payload controls.
