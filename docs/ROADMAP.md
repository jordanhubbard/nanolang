...
- [ ] I apply the selected scalar binary64 policy to each existing floating array operation (`task_bd474b339063ea2fe3915b29f12f9c80`, parent3717). My [preimplementation contract](NANOISA_AGGREGATE_BINARY64_POLICY.md) covers VM/interpreter and paired legacy element loops without new shape/native/LLVM/Wasm admission, requires exact-bit supported-route qualification and preserves allocation/alias boundaries.
- [ ] I snapshot scalar-left broadcast operands in source order in both legacy producers (`task_30c04e6d0ef594ef20bf7f9fec161969`). Static audit finds array-first evaluation in C-seed and selfhost branches. I require left once, right once, then length capture, including typed and existing helper fallback paths; no unknown-to-INT substitution or old-artifact replay.

- [ ] I preserve exact paired array arithmetic result types and canonical opcode selection (`task_90b473627cffdb98e69d5a5641937d30`). Frozen3087067f passes Cseed/interpreter but both selfhost stages refuse eight broadcast subcases as scalar FLOAT; static canonical emission also lacks an array branch before typed scalar instructions. My [corrective contract](NANOISA_AGGREGATE_BINARY64_POLICY.md) requires exact flat int/float shapes, generic VM array operations, checked mixed/unknown refusal and a fresh bootstrap without executing invalid old outputs.

- [ ] I retain exact direct-call numeric array results in legacy arithmetic (`task_6ed0cd1295af4a42b04133ddc6d39d98`). Corrected985321f7 passes exact arithmetic and canonical ordering; two selfhost legacy ordering cases refuse generated C because identifier-only selection drops the declared array result. My [bounded corrective contract](NANOISA_AGGREGATE_BINARY64_POLICY.md) preserves lexical precedence, original fixtures and all prior failure evidence.

- [ ] I preserve strict Darwin formatting fault injection (`task_8674e6954ba44d3da54ea5809d0bba9c`). Fresh array-policy source acceptance passes, but the VM adjacent harness collides with the SDK snprintf macro. I retain its compile refusal and scope the [test-only repair](NANOISA_AGGREGATE_BINARY64_POLICY.md) to undefining the macro before the existing hook, with unchanged production flags and corrected platform gates.
