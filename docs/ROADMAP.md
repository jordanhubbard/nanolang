...
- [ ] I establish unsigned-byte scalar transport and exact conversions across VM/C/LLVM/Wasm (MAC `task_ad1c498801b34aa38d82f58a588f1d5a`). I first repair same-U8 numeric ordering and CAST_FLOAT, then retain TAG_U8 through scalar locals/calls/results and checked conversions. I test boundary values, exact tags and same-module backend parity; mixed-tag numeric policy remains task01d. My contract is in [scalar U8](evidence/scalar-u8-contract.md).
- [ ] I define and implement U8 string conversion consistently (MAC `task_d08968be827a4d26942123825229ac8e`). My VM prints bytes numerically but CAST_STRING currently takes the empty-string fallback. I retain that observation separately from scalar numeric/boolean casts.
