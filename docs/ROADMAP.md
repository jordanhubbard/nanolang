...
- [x] I gave each portable v2 instruction one comprehensible meaning and kept operand forms symmetric: every family instruction in `spec/nanoisa.yaml` carries a unique `meaning`, every operand names a declared kind in `operand_kinds` (immediate constants, the shared `mem.*` `[offset, align]` pair, and `trap-code`), and paired `get`/`set` and `load`/`store` instructions share identical operand lists, enforced by `gen_nanoisa_schema.validate` and `tests/test_nanoisa_schema.py`.
