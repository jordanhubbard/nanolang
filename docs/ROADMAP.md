...
- [ ] I restore numerical FOR compiler metadata scope under `task_e0cdaf25cbfe4a11b053a4fd408ab0e0` ([preimplementation contract](PUBLIC_C_FOR_SCOPE_CONTRACT.md)). Static final6ade context audit finds the iterator binding added outside the scope that is later popped, leaving outer STRING/INT shadows incorrectly typed after emitted C's iterator scope ends. I require bounded normal/error scope restoration, nested/control-exit observations and strict generated execution; no lifted-header admission or runtime loop semantics change. No old output was executed.
