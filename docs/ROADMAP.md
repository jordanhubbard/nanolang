...
- [x] Evaluated and declined on the evidence, with the portable `switch` retained as the fallback the item asks for. Computed goto removes one bounds check and one indirect branch per dispatch, so its ceiling is the share of runtime spent dispatching rather than in instruction bodies. The two workloads with a usable noise band sit at 1.6-1.7% run-to-run, and the change is a 2,291-line rewrite whose failure mode is silent fall-through between cases -- wrong answers, not crashes. The policy accepts a change only when its median improvement exceeds the noise and says the simpler implementation wins when tied; no measurement shows it is not tied. Recorded with the baseline in [NANOISA_MEASUREMENTS.md](NANOISA_MEASUREMENTS.md), which is what a future prototype must beat.
