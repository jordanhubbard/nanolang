...
- [x] I gave each portable instruction one comprehensible meaning and kept operand forms symmetric: every v2 family declares a unique `meaning`, every operand names a declared `operand_kind`, and `tests/test_nanoisa_schema.py` enforces the contract.
