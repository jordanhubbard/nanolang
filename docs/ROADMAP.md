...
- [ ] I provide the native invariant macro when compiling the standalone map runtime harness (`task_02182f5f162a4bbf9f14803f29d533f3`). After PR613 instrumented map-runtime text, four full native unit builds lack NVM2C_ABORT and fail strict C compilation; I retain 2,418 passed/four failed in `/tmp/nanolang-numeric-union-native.log`. I must repair the harness/prelude contract and rerun the gate without removing invariant checks.
