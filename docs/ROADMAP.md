...
- [ ] **Ordinary NanoISA local inference** (`task_fd96d33b7d404449aab1e74b2b636628`): I will derive a supported initializer type when an ordinary let omits its annotation, reuse typed lowering and evaluate once. I preserve explicit types and require annotations for ambiguous empty constructors. Acceptance pairs scalar/nominal/nonempty-array values and lexical scopes across both producers and VM/native, retains refusal controls, and runs fresh bootstrap plus existing emitter gates. This is independent of the retained export-shadow incident.
- [ ] **Checked map access inference** (`task_0c5f2c64cc824d34a2c452111f18fb32`): I will retain the concrete value type of a declared HashMap receiver for builtin map_get, preserve ordinary function identity and explicit unknown annotations, and check concrete keys. The unchanged inferred-local fixture must publish through canonical Stage1/Stage2 and execute in VM/native; string/int results and wrong-key refusals remain checked.
- [ ] **Checked enum-member inference** (`task_156884dbd35d476cbaea730ce3aa6152`): I will resolve declared enum members to their nominal type before record-field handling, retain local shadowing and reject missing members. The unchanged inferred-local fixture must retain both enum and array-of-enum types through canonical Stage1/Stage2 and VM/native.
- [ ] **Integer context for fresh enum array literals** (`task_b7c4f9a51c894ad5aeb8f1adf5447219`): I will apply existing enum/int scalar compatibility to fresh explicitly declared array<int> literals, checking every element and retaining container invariance for existing array<Enum> values. Positive compiler literals/bootstrap and mixed-element/alias refusals establish this narrow context.
- [ ] **Defensive lowering error state** (`task_43128761d0b94470bcc13e700768454f`, companion to `task_dd74b033c3984805bc27ce5017096c3c`): I will validate local types before initializer emission, avoid binding publication on failure, and stop later statements while retaining scope cleanup and the first diagnostic. Ordinary state/refusal shadows and fresh paired/bootstrap/canonical gates establish this bounded behavior; they do not attribute the preserved historical incident.
