# My Roadmap
- [ ] I correct the admission-counter refusal expectation (`task_6b0e7040a5534b1cb14da14f8291623b`): declaration validation may refuse before full owned admission. I preserve the first harness log and require exact refusal with no activation, zero-or-one full admission on that refusal, and exactly one admission on accepted/runtime-assertion paths across all four APIs.

- [ ] I qualify instrumented owned call-graph execution (`task_5b3e7272ec3f4ae9ad8eb8ffb15a3565`) before claiming the remaining PR725 gate: O0/O2 full runs reach600 seconds, partition preambles/cases0 and1 reach300 seconds, and current-process ptrace sampling is denied. I retain complete ordinary/native results separately, stop further launches, audit tool configuration and localize bounded ordinary execution with phase evidence before any repair. My [reuse contract](NANOISA_OWNED_VERIFICATION_REUSE.md) preserves mandatory admission versus non-authorizing metadata, standalone verifier checks and invocation-local proof only, including the zero-link nvm_verify_linked route used by vm_init. My timed diagnostic observes 17 completed runtime admissions consuming25.154 seconds within API0; I propose separately reviewed stack-local public-invocation reuse across internal true-ASSERT resumptions, excluding callbacks, tracing, mutation and separate public calls. Elapsed time alone establishes no runtime or instrumentation defect. [Evidence](evidence/owned-value-call-graphs.md).
...
- [ ] I qualify bounded value-only owned call graphs (`task_4ce5cfc5b8034949852255d7307c9f91`) as the first c435 prerequisite: at most eight functions/frames, acyclic exact direct calls, zero-argument intermediate functions, mode-zero scalar/resource parameters, scalar results and per-frame local reference provenance. I require full preflight/unwind and paired VM/native lifecycle/authority gates before source admission; borrowed call graphs, owned/void results and string effects remain separate.
