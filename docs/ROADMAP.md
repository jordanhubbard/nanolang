...
- [ ] I preserve every owned-string admission boundary before PR750 merge (`task_64394f8214c3eb7bd28ebc7e2333ce6d`). Independent static review found instantiated-constant readiness confined to caller checks and shared string-print opcode allowlists widening the separate borrowed profile. At `78bffdb2`, both shared allowlists require the value-graph profile and every entry path shares one validated required/owned-transfer classification. Instantiated constants are required only for positive owned execution; linked advisory fallback remains ordinary, the replaced fixture string pool is freed, and the helper-free four-trap sequence records exact cumulative admissions `3/5/7/9`. The clean focused Homebrew LLVM 23 LeakSanitizer gate, a separately sanitized leak check, and all adjacent ownership/advisory suites pass. My [evidence](evidence/owned-string-print.md) limits injected allocation claims to reached `heap.c` sites. I keep this item and the [review hold](https://github.com/jordanhubbard/nanolang/pull/750#issuecomment-5732726734) open for renewed independent review; source admission and product522 remain blocked.
...
- [ ] I restore NanoVirt match-arm binding scope (`task_6d9b923cd7854003a8430dddde22d4b2`, required guarded-match child d00106). My frozen public C guarded fixture at `66824443` passed public C/interpreter observations but failed its verified VM outer-payload assertion after a nested same-name binding; both first GCC/Clang logs remain `/tmp/nanolang-public-c-guarded-{gcc,clang}.log` (3/4 methods each). Static `AST_MATCH` indexes lexical bindings using allocated slot counts despite their separate lifetimes. I require binding-count restoration with retained slot highwater and fresh corrected-source paired acceptance; I do not replay failed artifacts or claim the full guarded gate passed.
