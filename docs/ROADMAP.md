...
- [ ] I produce nested resource-path borrows from declaration-ordered finite owned trees using existing checked runtime authority (MAC `task_91cb8c2db94941bc950d9e9fa597d77a`). My [bounded contract](NANOISA_NESTED_SOURCE_BORROWS.md) requires exact nominal child moves/destructuring/disposal, leaf-record formals, numeric path limits and caller-root prefix overlap checks. Paired metadata/code/names, mandatory shadows, ordinary refusal/publication and VM/sanitized native gates precede completion; control flow, recursive calls, imports and broader construction remain separate.
