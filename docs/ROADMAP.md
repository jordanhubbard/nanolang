...
- [ ] I run one clean native full-source acceptance from merged code `61b0aedc` after map-byte debt and builtin host-string ownership repairs (MAC `task_fc43d8d1923b40ebb343ae56da535dfc`). I freeze this source separately, retain helper/tool/host hashes, and require help plus verified hello before a full generation. Each generation retains 1800 seconds, 48 GiB owned RSS and a 32 GiB host reserve. Only a successful first generation permits the second and raw comparison; I preserve incomplete evidence without retry or budget extension.
...
- [x] I complete unchanged-calculator bytecode/native acceptance through both producers (`task_668e98f3e13e4fcebb3a2f92e671c713`, dependencies `task_b927827f37734658bce360d7ecf913aa` and bounded builtin child `task_7f2b7373646341d5a317f374d302e390`). Both retained bytecodes verify and execute in VM, then print `Result: 3.14159` with strict native GCC plus ASan/UBSan/LSan and strict Clang. My [host ownership evidence](evidence/native-host-string-ownership.md) records exact hashes. Generic artifact ownership remains open independently.
