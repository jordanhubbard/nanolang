...
- [ ] I isolate each owned string-output proof invalidation and report only reached allocation sites (`task_56529d8eef98479ab688d249237b0ccd`). A simple owned sequence must establish exact verifier-admission deltas at every `PRINT` or `PRINTLN` boundary; unrelated helper activations cannot satisfy the assertion. I retain explicit fast/fallback/public-core/resume constant refusals and borrowed `CALL_REF` string-output refusal. My allocation evidence names only the two reached `VmHeap` sites instrumented through `heap.c`, not call-frame allocation.
