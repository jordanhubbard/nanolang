...
- [x] I keep the owned-result generated-native sanitizer qualification portable on Darwin (`task_70893b77ecec8f6f8dfbc5a9be995116`). Linux retains leak detection; Apple Clang uses ASan/UBSan with leak detection disabled and the unchanged generated harness's explicit zero-live-allocation assertions. I preserve the original unsupported-option failure. At code checkpoint `497dd96d`, the exact focused gate passes 502 descriptor, 1,552 lifecycle, 3,485 allocation and 117 return-preflight checks plus all ten generated-native sanitizer cases without production changes. [Evidence](evidence/owned-value-results.md).
