...
- [ ] I preserve my strict Darwin formatting fault-injection harness (`task_8674e6954ba44d3da54ea5809d0bba9c`) before aggregate policy qualification. The current SDK already defines a fortified `snprintf` macro, so the substring test translation unit must undefine only that existing macro before installing its private `format_snprintf` fault injector. I keep production source, fortification, flags, assertions and deadlines unchanged, preserve the first failed Darwin log, and require corrected Linux and Darwin substring/NanoVM gates before closure.
