...
- [x] I gave each portable v2 instruction one comprehensible meaning and kept operand forms symmetric: every family instruction carries a unique `meaning`, every operand names a declared kind in `operand_kinds` (immediate constants, `mem.*` `[offset, align]`, and trap selectors included), and paired `get`/`set` and `load`/`store` instructions share identical operand lists, enforced by `tests/test_nanoisa_schema.py`.
