...
- [ ] I execute nested same-frame reference paths and reborrows (MAC `task_556d6702b3c74377b0cb830b6d988c68`, parents ed702/718). I first preserve format-1 ownership bytes and specify the bounded format-2 numeric path table, then connect four vacant opcodes to authoritative layout resolution, exact parent/region dataflow and matched VM/native owner access. I require [transport, authority, resume and lifetime gates](NANOISA_NESTED_REFERENCES.md); caller provenance and frontend production remain separate required work.
...
- [ ] I report allocation failure from existing scalar CAST_STRING arms (MAC `task_822750774e0040298e8297e96145f3d5`). Integer/float/bool and default empty-string helpers can return NULL, but their dispatcher arms currently push that result unchecked. I retain this static finding separately from the checked U8 conversion.
