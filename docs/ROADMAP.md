...
- [ ] I define explicit owned artifact-string results before freeing filesystem return buffers (MAC `task_bdc323f270d44f02b38ba728f1f93184`). My existing generic path adapters allow borrowed literals, and artifact `file_read` mixes owned empty text with a borrowed allocation-failure literal. I must audit callers and establish an explicit owned-or-NULL ABI/adapter contract; I never infer ownership from text contents, a symbol spelling or a library filename. My [static caller audit and proposed provider cleanup contract](NATIVE_ARTIFACT_STRING_LIFETIMES.md) preserve the legacy non-NULL interface; adapter implementation and lifetime acceptance remain open.
