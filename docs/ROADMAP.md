...
- [x] Cut A for-range: the same pin matches `upto` (`for` lowered
...
- [x] Cut A mixed record results: the same pin matches `make_tok`
      (`-> Tok` as `.function make_tok 0 0 0 struct 1`) and `via_tok`
      (`CALL make_tok` then `AGG_GET` of the string field) bytecode
      with the C seed. Nested records stay refused. I still
      pretty-print C to build the compiler.
      `make test-nanoisa-src-nano` (110 passed).
- [x] Cut A array results: the same pin matches `ones`
      (`-> array<int>` as `.function ones 0 0 0 array 1`) and
      `via_ones` (`CALL ones` then `ARR_LEN`) bytecode with the C
      seed. Nested arrays stay refused. I still pretty-print C to
      build the compiler.
      `make test-nanoisa-src-nano` (114 passed).
- [x] Cut A list results: the same pin matches `new_l`
      (`-> List<int>` as `.function new_l 0 0 0 array 1` and
      `ARR_NEW`) and `via_new_l` (`CALL new_l` then `ARR_LEN`)
      bytecode with the C seed. Nested arrays stay refused. I still
      pretty-print C to build the compiler.
      `make test-nanoisa-src-nano` (118 passed).
- [x] Cut A record-list results: the same pin matches `one_t`
      (`-> List<Tok>` as `.function one_t ... array 1` and
      `ARR_PUSH` of `nrec_t` before `RET`) and `via_one_t`
      (`CALL one_t` then `ARR_GET`/`AGG_GET`) bytecode with the C
      seed. Nested records stay refused. Empty `List<Tok>` `CALL`
      is a separate pin. I still pretty-print C to build the
      compiler.
...
- [x] `nvm2c` runs Cut A `via_tok` (`CALL` of a mixed int/string
      record) without `nano_vm`. Nested records stay refused.
      `make test-nvm2c` (383 passed).
- [x] `nvm2c` runs Cut A `via_ones` (`CALL` of `array<int>`) without
      `nano_vm`. Nested arrays stay refused.
      `make test-nvm2c` (389 passed).
- [x] `nvm2c` runs Cut A `via_new_l` (`CALL` of `List<int>`) without
      `nano_vm`. Nested arrays stay refused.
      `make test-nvm2c` (395 passed).
