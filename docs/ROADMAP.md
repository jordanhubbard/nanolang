...
- [ ] I preserve every owned-string admission boundary before PR750 merge (`task_64394f8214c3eb7bd28ebc7e2333ce6d`). Independent static review found instantiated-constant readiness confined to caller checks and shared string-print opcode allowlists widening the separate borrowed profile. At `d2aade9b`, every positive runtime predicate requires readiness and both shared allowlists require the value-graph profile; exact fast/trace/callback/reference/public-core and borrowed `PUSH_STR`/`PRINT`/`PRINTLN` controls pass with Homebrew LLVM 23 LeakSanitizer, as do all adjacent ownership suites. My [evidence](evidence/owned-string-print.md) retains precise per-trap proof counts and allocation claims limited to instrumented `heap.c` sites. I keep this item and the [review hold](https://github.com/jordanhubbard/nanolang/pull/750#issuecomment-5732726734) open for renewed independent review; source admission and product522 remain blocked.
