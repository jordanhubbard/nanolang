...
- [ ] I retain scalar `cast_int` in my canonical bytecode emitter (`task_1c172729873542a78d36fcd64ee7fda3`), including exact arity/type checks and declared-function identity. The unchanged calculator currently stops at `undefined function cast_int` before emission.
- [ ] I implement my NanoISA float-to-int contract (`task_b927827f37734658bce360d7ecf913aa`): truncate finite values in `[-2^63, 2^63)` toward zero; stop with an explicit conversion diagnostic for NaN, infinities or values outside that interval before any C integer cast. My VM and concrete/tagged native paths must agree. Legacy interpreter/C-emitter conversion policy remains separately audited; I do not infer it from NanoISA checks.
