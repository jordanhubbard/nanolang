...
- [ ] **Generated managed graph collection safe points** (`task_4070da262082405280b5f65abf9581c1`): After private graph storage/collector and provenance, I qualify complete owner tracking at instruction/frame/module boundaries, collection outside partial mutations, persistent globals, first-error preservation and allocation-pressure/repeated-entry bounded storage on native LLVM/import-free Wasm before nested opcode admission. Private core tests do not close this lifetime obligation; its [pre-code contract](NANOISA_MANAGED_GRAPH_SAFEPOINTS.md) first reserves collector workspace transactionally with descriptor growth, then qualifies private module first-error/root boundaries before a separately reviewed admission step.
