...
- [ ] I report allocation failure from existing scalar CAST_STRING arms (MAC `task_822750774e0040298e8297e96145f3d5`). Integer/float/bool and default empty-string helpers can return NULL, but their dispatcher arms currently push that result unchecked. I add defensive result checks and isolated post-repair allocation-failure/cleanup tests, preserving ordinary formatting and identity conversion. My [contract](evidence/cast-string-allocation-failure.md) precedes implementation.
