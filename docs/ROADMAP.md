...
- [ ] I preserve every owned-string admission boundary before PR750 merge (`task_64394f8214c3eb7bd28ebc7e2333ce6d`). Independent static review found instantiated-constant readiness confined to caller checks and shared string-print opcode allowlists widening the separate borrowed profile. At `d2aade9b`, both shared allowlists require the value-graph profile, but the readiness repair remains too broad: it changes advisory metadata fallback and leaves direct-core admission coupled to a module-only predicate. I require every entry path to share one validated required/owned-transfer classification, require instantiated constants only when that classification is positive, preserve advisory and non-standalone fallback, free replaced fixture string pools, and retain exact per-trap proof counts plus allocation claims limited to instrumented `heap.c` sites. I keep this item and the [review hold](https://github.com/jordanhubbard/nanolang/pull/750#issuecomment-5732726734) open for renewed independent review; source admission and product522 remain blocked.
