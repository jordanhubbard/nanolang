...
- [ ] I lower native scalar `CAST_FLOAT` with exact VM conversion semantics so unchanged `arctan_series` calls retain VM/native parity (`task_3d6c3314d8924dd2b0aca679647e7048`). The positive loop currently verifies and executes in VM, while native translation explicitly refuses; float-to-int task `task_b927827f37734658bce360d7ecf913aa` remains separate.
