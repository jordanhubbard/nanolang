...
- [ ] I finish the source inventory for my exact admitted native host-string results (MAC `task_d5f899966241452a900422938fff3265`), using merged PR553 builtin/facade adoption, PR583 file_read cleanup and PR589 six path-provider companions. I document every builtin/artifact table entry, preserve borrowed provider storage, and reconcile bounded repeated-result/alias/sanitizer evidence. Arrays, maps and callback/co-process/interpreter enrollment remain separate.
...
- [ ] I establish explicit underscore union-payload pattern semantics across producers (MAC `task_fc39603161314568929719b36d25f14b`). Static audit finds C native omission versus selfhost `nl__` declaration; wildcard arms are distinct. Ordinary let-name repair does not claim this pattern contract.
