...
- [ ] I keep the owned-result generated-native sanitizer qualification portable on Darwin (`task_70893b77ecec8f6f8dfbc5a9be995116`). I select leak detection from the actual sanitizer compiler runtime, not from the host OS: only identified Apple Clang uses its documented unsupported-LSan fallback, while Homebrew LLVM and Linux retain `detect_leaks=1`. I preserve the original Apple failure, require the same complete ten-case harness under both Darwin compilers, and keep ASan/UBSan plus explicit zero-live-allocation assertions in every run. [Evidence](evidence/owned-value-results.md).
