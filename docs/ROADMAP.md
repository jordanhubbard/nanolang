...
- [ ] **Passive owner calls outside node ranges** (`task_eccab63d320946d283342bb8fc8eb18f`): I will admit version-2 direct CALL outside marked nodes under ordinary target/stack/frame checks. I retain node purity, guarded parameter immutability and block ranges, plus version-1 and other owner-opcode restrictions. Acceptance checks effectful serial calls around multiple par/flow blocks, preserved caller locals, both frontend producers and VM/native output, with existing passive/roundtrip/verifier gates.
