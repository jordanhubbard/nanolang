...
- [ ] I match operation-specific enum coercion in typed integer instructions (`task_a77ca354773245f9a8b5f490fe336c06`). My native typed binary arithmetic/comparison boundary now normalizes checked int/enum operands while preserving original producer tags and all other exact integer guards. Paired GCC/Clang controls pass; full native and LLVM/Wasm acceptance remain pending. I retain typed unary and generic MOD/NEG distinctions. [Contract and evidence](NATIVE_TYPED_ENUM_COERCIONS.md).
