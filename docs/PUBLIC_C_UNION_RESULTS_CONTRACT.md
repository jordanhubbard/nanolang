...
## I retain the existing dotted constructor representation

My first frozen `863d156d` GCC/Clang gates each pass the API method and refuse both source methods before generating C. I retain `/tmp/nanolang-public-c-union-results-{gcc,clang}.log`. Static inspection shows my checker can retain a selected constructor as `AST_STRUCT_LITERAL` with the exact dotted `Union.Variant` name, and my existing public emitter already lowers that representation. My first resolver omitted it.

Before correction I require exact full spelling against a real local owner and its declared variant, followed by the same field count, distinct names and compatible scalar type validation as `AST_UNION_CONSTRUCT`. A temporary compiler-local AST view may reuse validation without mutating the source AST. Ordinary structs and unknown dotted/module names are not union identity. I preserve every original fixture/assertion and run only fresh corrected-source qualification; no refused artifact executes.
