...
For Darwin I build the borrowed-result fixture with the existing supported `-dynamiclib` form. My focused driver must select an explicit compiler wrapper through PATH for the old exact-contract method, whose unchanged compile command uses `cc` and HOST_RUNTIME. That wrapper supplies the same sanitizer and SDK flags as every fresh provider build; the new fixture-only NANO_ARTIFACT_CFLAGS variable is not evidence that the old method was instrumented.

My first b209 Darwin gate passes fresh providers, the exact-contract matrix and the mutable-root lifetime case. The real-provider fixture stops before assembly/execution because it incorrectly requires one library pathname for two distinct declaration owners. I retain owner0 ABI and owner1 root, each bound to the same actual source. `nlc_module_artifact` explicitly returns the immutable generation selected by each module build; `module_build_with_flags` can publish a fresh `.nano-gen-*` directory. Equal source paths do not authorize merging owners or requiring equal generation paths. The retained root provider hash is d6c70af4a81321870a5da58c08a62da691c234da263f1793bbb8d3e3938d6733; ABI provider hash is 11a09aeebabf41e47ff089e132e7ce0c1e09c3964ed9171dfeb69918dfa252a2, each303336bytes.

Before fixture correction, I require exact two symbol/import contracts and preserve their distinct owner rows. I will compare every selected provider's actual ABI/root against its own typed C oracle, require agreement on the selected SDK root and actual ABI, then execute the unchanged VM/native program against the original per-symbol imports. I retain and recheck every provider hash. I remove only the unsupported single-path assumption; production and declared owners remain unchanged.
