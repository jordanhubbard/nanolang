...
I qualify generated byte conversion at production `e0e85441f` and corrected
neighbor fixtures `163290dda`. All five conversion methods pass on Linux and
Darwin in ordinary and selected sanitizer runs. Generated C and native harnesses
use ASan/UBSan; emitted native LLVM uses ASan. LLVM verification, O0/O2,
Wasmtime and Node checks pass. The scalar neighbor suite passes all 36 tests,
and the managed neighbors pass on both hosts. Darwin neighbor and reconstruction
checks use the exact two-file `163290dda` fixture overlay on the frozen
`e0e85441f` tree; the evidence records that source state explicitly.

Both reconstruction languages still refuse CAST_U8 and preserve existing output.
I retain original branch/NUL, target-layout sanitizer and stale-neighbor failures
separately from their corrected runs. My sealed reports and artifact references
