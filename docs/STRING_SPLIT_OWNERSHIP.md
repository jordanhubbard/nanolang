...
My prerequisite fixture at `d1e41d0e9` passes on the current integrated C seed on Linux and both explicitly retained `1ee9b0f33` Nano refresh products on Darwin. Source, tool and provider endpoints remain unchanged. Linux also passes the corrected actual evaluator/native ownership and allocation-failure controls, including the ninth physical NUL buffer. These are bounded prerequisite results, not a fresh bootstrap or complete paired SDK acceptance.

My source checkpoint adds only two early returns to `split_lines`: zero length returns `[""]`, and lengths through 64MiB call the existing intrinsic with the literal newline. The entire existing scanner remains after those returns for larger inputs. All original shadows remain; additive newline-only and bare-CR cases check the empty tail and delimiter specificity. The complete fresh bootstrap, original merger selection/deadlines, paired corpus and installed source-hidden SDK acceptance remain required before I claim this source change qualified.
