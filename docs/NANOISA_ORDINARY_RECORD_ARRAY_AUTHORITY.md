...
The legacy borrowed-root validator scans reachable siblings and can allocate; its boolean failure cannot distinguish unsupported roots from allocation failure. My private query preserves that ambiguity as UNKNOWN and charges conservative full-field/root work per descriptor before calling the shared validator. It does not substitute a path-depth-only estimate. Public results remain unchanged.

## My fixture checkpoint before qualification

My direct C fixture checks all five flat element tags and both declaration orders, copied facts after source overwrite and heap-source free, public kind2 refusal, public v1/v2 and union-only-v3 controls, mixed union/array whole-envelope refusal, ordered complete bindings and distinct equal-shaped record identities. A real maximum table combines256 layouts,65536 fields/bindings and4096 type rows; each next-count limit refuses. Forward/backward chains check cached64-depth and65-limit behavior. I inject every measured preparation allocation as persistent and transient failure, preserve outputs, recover freshly and check tracked allocation baseline. Instrumentation covers the shared ownership/layout decoder translation units, not every linked provider or the legacy reference validator. The step boundary is additionally observed through the private counter. Private structural input bounds already have their separately sealed seven-configuration acceptance.

The shared runner imports a helper module, not another TestCase. Qualification must rebuild the two affected providers for each mode, retain exact provider inputs, clear LSAN_OPTIONS externally, and run public ownership/ordinary declaration and raw transport neighbors separately. Raw transport acceptance is not inferred from these in-memory query checks. Full array/source/runtime parents remain open.
