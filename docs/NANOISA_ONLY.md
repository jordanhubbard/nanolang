...
`list_string_set` as `ARR_SET` then `POP`, and `for` over
`array<int>` as `LT`). `STR_SPLIT`
and `STR_REPLACE`
...
native C), and Cut A `for` (`LT`; `upto` runs as native C). Nested arrays,
