...
Shadow tests are small assertions next to the function they test. My C seed checks dependency and root-file shadows, then executes them in a supervised interpreter child before publishing output. My bytecode CLI compiles the same selection into a separate verified NanoISA module and runs it in NanoVM; root-only selection requires an explicit opt-out. Both children have a ten-second parent deadline, not a security sandbox. My self-hosted native driver runs root shadows in a separate timed test executable. Source-only C emission does not execute tests. Dependency shadows by default is my 5.0 policy; self-hosted native parity remains work, as does full interpreted foreign ABI support. The requirement remains: if you write a function, you must test at least one observable property of it.
