...
Shadow tests are small assertions next to the function they test. My C seed checks dependency and root-file shadows, then executes them in a supervised interpreter child before publishing output. My bytecode CLI compiles the same selection into a separate verified NanoISA module and runs it in NanoVM. My self-hosted native driver runs the merged dependency and root shadows in a separate test executable. All three drivers select dependencies by default, retain an explicit root-only opt-out, and impose a ten-second parent deadline. This is not a security sandbox. Source-only C emission does not execute tests. Full backend semantic equivalence and interpreted foreign ABI support remain work. The requirement remains: if you write a function, you must test at least one observable property of it.
