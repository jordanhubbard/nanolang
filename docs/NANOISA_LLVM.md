...
I reject imports, linked modules, passive/reference contracts and unsupported instructions. Scalar/literal profiles also reject nominal layouts and ownership metadata. My separate managed profile supports qualified computed string operations and shape-checked arrays, including boxed nested/cyclic arrays with counted roots and prepared collection. `NANOISA_MANAGED_GRAPH_ADMISSION.md` defines this bounded graph lifetime; it does not itself admit other heap kinds. My separate checked ordinary-record extension requires COMPLETE acyclic ordinary authority plus exact field origins, immutable descriptor binding and matched counted-root lowering, as specified in `NANOISA_MANAGED_RECORD_EXECUTION.md`. Exact all-record forward DAGs follow `NANOISA_MANAGED_FORWARD_RECORDS.md`. Unknown/resource, mixed-kind forward and mixed record-array layouts remain refused. My bounded literal-string profile refuses ADD/CAST_INT/CAST_FLOAT throughout string-bearing modules until managed string operations are lowered; numeric-only modules retain their generic arithmetic. Explicit RET and verified code-end exits retain declared result-count and tag checks. I do not embed NanoVM or resurrect AST target paths. I publish named output only after verification, profile checks and complete emission succeed, using an exclusive temporary beside the destination. Existing output and source survive failures.
