...
At source checkpoint `59d28b33`, I also admit inferred homogeneous float literals and require exact nonliteral float-array context, including the opposite int-array direction. Eleven focused/driver methods pass GCC in 2.924 seconds; four float methods pass Clang in 2.355 seconds. My final broader emitter gate passes all 88 methods in 144.437 seconds. Parent2578 remains open for functional transformations.

After a clean restack onto main `41e652f0` (including filesystem adapters and managed substring), my rebuilt tools pass twelve focused float/driver/filesystem methods with GCC in 23.705 seconds and four float methods with Clang in 2.236 seconds. I strengthen the lifetime control by replacing the direct array local before further allocation churn, so only the record retains the grown float array; that final test passes GCC in 1.643 seconds and Clang in 1.272 seconds with sanitizers. The native implementation is unchanged from the reviewed full-gate checkpoint apart from unrelated merged main additions.

I keep the native gate log at `/tmp/nanolang-float-arrays-full.log`, corrected emitter log at `/tmp/nanolang-float-arrays-emitter-final.log`, and restack/sole-record-root logs under `/tmp/nanolang-float-arrays-restack-*` and `/tmp/nanolang-float-arrays-record-root-*`. These are local retained logs; my checked-in tests and pinned source define the repeatable acceptance. I have not rerun the full product bootstrap or claimed the filter example complete.
