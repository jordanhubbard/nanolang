...
save(base/'identity.json',dict(pin='69eeb6c2e',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable,shutil.which('pkg-config',path=env['PATH'])]}))
