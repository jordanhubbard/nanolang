...
root=Path('/home/jkh/Src/nanolang-cast-u8-backends');base=root/'docs/evidence/cast-u8-backends-integration';base.mkdir(parents=True,exist_ok=False)
objects={};summary={}
def sha(p):
 with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
for host in ['linux','puck']:
 src=Path('/tmp/nanolang-byte-integration-'+host+'-seal');dst=base/host
...
for name in ['seal-cast-u8-backends-integration.py','publish-cast-u8-backends-integration.py','collect-cast-u8-backends-inner.py']:shutil.copyfile('/tmp/'+name,base/name)
archive=Path('/tmp/nanolang-byte-integration-puck-evidence.tar.gz');(base/'transport.json').write_text(json.dumps({'archive':str(archive),'sha256':sha(archive),'bytes':archive.stat().st_size},indent=2)+'\n')
