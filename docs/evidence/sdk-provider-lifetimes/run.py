...
save(base/'identity.json',dict(pin='255f8b241f2a4ee50c20645b2686472e4ee1541d',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
