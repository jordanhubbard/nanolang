...
out=pathlib.Path('/tmp/nanolang-temporary-owner-937bb61c');out.mkdir(exist_ok=False)
def git(*a):return subprocess.check_output(['git',*a],cwd=root,text=True).strip()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
pin=git('rev-parse','HEAD');assert pin.startswith('937bb61c');assert not git('status','--porcelain','--untracked-files=no')
assert (root/'bin/nanoc_stage2').is_file()
