...
toolpaths=[pathlib.Path(shutil.which(p)).resolve() for p in ['python3','make','as','ld'] if shutil.which(p)]+[pathlib.Path(c).resolve() for _,c,_ in ccs]
...
steps=[(name,['python3','-m','unittest','-f','-v','tests.test_nsi_file'],{'NANO_FILE_TEST_CC':cc,**extra}) for name,cc,extra in ccs]
steps.append(('adjacent',['make','test-nsi-cap','test-nsi-shm','test-nsi-fabric','test-nsi-runtime','test-nsi-gen','test-nsi'],{}))
try:
