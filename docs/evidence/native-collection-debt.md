...
Both actual compiler-product gates pass together in 392.714 seconds. My translator passes 2386 checks, shape constraints pass 1092 checks, and returned array/string cleanup passes. My map/global/string/scaling regressions retain their existing lifetime, content, peak-owner and sanitizer assertions; 17 focused methods pass across the combined suite and deferred-drop control. My full compiler comparison uses a separate executable translated from the exact same pinned compiler bytecode; the original and indexed baseline executables remain unchanged. Full self-compilation and byte-identical bootstrap convergence are separate acceptance criteria.
...
My exact pinned full compiler advances past source merging and aborts after 9.92 seconds at `nrarr_push -> nl_push_tok -> nl_tokenize_string`. I isolated this to the existing inline `nrec_t data[256]` capacity. The committed `tests/nanoisa/fixtures/record_array_growth_257.nasm` passes VM execution; the unchanged merged-PR-477 translator aborts on the 257th record. My debt translator emits byte-identical C for that map-free fixture, so the capacity failure is independent of scheduling. I recorded checked dynamic growth as `task_617006c46f9746da9694c6a4e0a0ceaf` before implementation. This is progress to a new guard, not successful full compiler self-compilation. Exact evidence: `/tmp/nanolang-record-capacity-257-{baseline,debt}.c`, `/tmp/nanolang-record-capacity-257-stack.log`, and `/tmp/nanolang-bootstrap-8d769-stage1-debt-stack.log`.
