...
base=Path('/tmp/nanolang-file-cyclic-public-puck-package');base.mkdir(exist_ok=False)
...
def sha(p):
 ...
 return h.hexdigest()
groups={'darwin-first':'aff3','darwin-schema-first':'414c','darwin-header-corrected':'414c-config','darwin-native-corrected':'a11c'};index={};refs=0;current=[]
for label,suffix in groups.items():
 ...
 results=json.loads((src/'results.json').read_text());last=results[-1]['phase'];assert results[-1]['status']==(0 if suffix=='a11c' else 1)
...
for name in ('reports','artifacts'):
 dest=Path('/tmp/nanolang-file-cyclic-public-puck-'+name+'.tar.gz')
...
Path('/tmp/nanolang-file-cyclic-public-puck-packaging.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary,indent=2),flush=True)
