...
run('schema-consistency',[sys.executable,'scripts/gen_nanoisa_schema.py','--check'])
run('schema-tests',[sys.executable,'-m','unittest','-v','tests.test_nanoisa_schema'])
...
runtime('clang-ordinary','tests.test_file_cyclic_public',shutil.which('clang'),clang_flags,0)
runtime('sanitizer','tests.test_file_cyclic_public',os.environ['CARRIER_SAN_CC'],normal_flags,1)
if not darwin:runtime('clang-sanitizer','tests.test_file_cyclic_public',shutil.which('clang'),clang_flags,1)
