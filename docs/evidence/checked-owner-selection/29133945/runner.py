...
out=pathlib.Path('/tmp/nanolang-checked-owner-selection-29133945');out.mkdir(exist_ok=False)
def git(*a):return subprocess.check_output(['git',*a],cwd=root,text=True).strip()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
pin=git('rev-parse','HEAD');assert pin.startswith('29133945');assert not git('status','--porcelain','--untracked-files=no')
...
steps=[('tools',['make','-j2','nano_asm','nano_vm','nvm2c']),('checked-selection',['python3','-m','unittest','-v','tests.test_checked_owner_selection'])]
try:
