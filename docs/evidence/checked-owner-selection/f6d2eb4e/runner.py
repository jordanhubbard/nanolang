...
out=pathlib.Path('/tmp/nanolang-checked-owner-selection-f6d2eb4e');out.mkdir(exist_ok=False)
def git(*a):return subprocess.check_output(['git',*a],cwd=root,text=True).strip()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
pin=git('rev-parse','HEAD');assert pin.startswith('f6d2eb4e');assert not git('status','--porcelain','--untracked-files=no')
...
steps=[('source-borrows',['make','-o','bootstrap','test-source-borrow-emission']),('owned-graphs',['make','test-owned-value-graphs'])]
try:
