...
root=pathlib.Path('/home/jkh/Src/nanolang-checked-owner-selection-integrated')
out=pathlib.Path('/tmp/nanolang-checked-owner-selection-9b87b18c');out.mkdir(exist_ok=False)
def git(*a):return subprocess.check_output(['git',*a],cwd=root,text=True).strip()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
pin=git('rev-parse','HEAD');assert pin.startswith('9b87b18c');assert not git('status','--porcelain','--untracked-files=no')
...
steps=[('bootstrap',['make','-j2','bootstrap']),('tools',['make','-j2','nanoisa_dump','nano_vm','nvm2c']),('checked-selection',['python3','-m','unittest','-v','tests.test_checked_owner_selection.CheckedOwnerSelection']),('source-borrows',['make','-o','bootstrap','test-source-borrow-emission']),('owned-graphs',['make','test-owned-value-graphs'])]
try:
