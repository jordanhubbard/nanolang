...
source.append(pathlib.Path('/tmp/nanolang-scalar-u8-8caba.py'));save('tracked-paths.json',tracked);save('runner.json',entry(__file__))
...
fixture_runner=out/'run-corrected-guard.py'
fixture_runner.write_text("import importlib.util,sys,unittest\nsys.path.insert(0,sys.argv[1]);s=importlib.util.spec_from_file_location('corrected_scalar_u8',sys.argv[2]);m=importlib.util.module_from_spec(s);s.loader.exec_module(m);t=unittest.defaultTestLoader.loadTestsFromName('ScalarU8.test_declared_byte_result_checks_actual_return_tag',m);assert t.countTestCases()==1;r=unittest.TextTestRunner(verbosity=2).run(t);sys.exit(not r.wasSuccessful())\n")
if not command('corrected-byte-result-guard',[sys.executable,str(fixture_runner),str(root),'/tmp/nanolang-scalar-u8-8caba.py'],120):sys.exit(1)
if not darwin:
 save('terminal.json',{'status':'PASS','head':pin,'fixture_pin':'8caba839c','scope':'Corrected single byte result guard against preserved ordinary providers.'});sys.exit(0)
