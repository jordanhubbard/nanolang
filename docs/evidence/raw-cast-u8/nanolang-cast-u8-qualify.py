...
phases=[('discovery',[sys.executable,'-c',"import unittest;from tests import test_cast_u8;s=unittest.defaultTestLoader.loadTestsFromModule(test_cast_u8);assert s.countTestCases()==1;print('I discover exactly one raw CAST_U8 method.')"],30),('build',['make','-j2','nanoisa_dump','nano_vm','nvm2c','nvm2llvm','nvm2wasm',*flags],1800),('schema',[sys.executable,'-m','unittest','-v','tests.test_nanoisa_schema'],120),('raw-ordinary',['make','test-cast-u8',*flags],300),('byte-neighbors',[sys.executable,'-m','unittest','-v','tests.test_scalar_u8'],900)]
