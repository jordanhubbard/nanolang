...
configs=[('homebrew-ordinary',native,False),('homebrew-sanitizers',native,True)] if darwin else [('gcc-sanitizers',cc,True),('clang-ordinary',llvm+'/clang',False),('clang-sanitizers',llvm+'/clang',True)]
for label,compiler,sanitize in configs:
 flags=baseflags+(' -fsanitize=address,undefined -fno-omit-frame-pointer' if sanitize else '')
