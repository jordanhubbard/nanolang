...
root=pathlib.Path('/home/jkh/Src/nanolang-record-lists-qualified-50de')
source=pathlib.Path('/home/jkh/Src/nanolang-full-compiler-evaluator-timing')
prior=pathlib.Path('/tmp/nanolang-record-lists-50de-linux-prepare')
out=pathlib.Path('/tmp/nanolang-full-compiler-timing-280b-linux');out.mkdir(exist_ok=False)
toolroot=out/'compiler-root';toolroot.mkdir();(toolroot/'bin').mkdir()
for name in ('src','src_nano','modules','stdlib','std','include','lib','scripts','share','locales'):
...
assert subprocess.check_output(['git','rev-parse','HEAD'],cwd=source,text=True).strip()=='280bbff9a79587cf067370619aa0d21e53334e8a'
assert subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip()=='50de86e7570289e3e19d65e6edf76a27c9288308'
assert not subprocess.check_output(['git','status','--porcelain'],cwd=source,text=True).strip()
base=json.loads((prior/'source-initial.json').read_text());tools=json.loads((prior/'tools-initial.json').read_text());products=json.loads((prior/'bootstrap-products-after.json').read_text())
...
# I verify every diagnostic compile source against50de, allowing only reviewed observer/header dependency edits.
extra=[]
...
env.update({k:v for k,v in json.loads((prior/'environment.json').read_text()).items() if k in ('PATH','NANO_NATIVE_TEST_CC','NANOLANG_GUARD_SAN_CC','NMS_RUNTIME_CLANG','NMS_RUNTIME_OPT','NMS_NATIVE_CLANG_FLAGS')})
...
result=command('full-compiler',[compiler,root/'src_nano/nanoc_v06.nano','-o',out/'nanoc_stage1'],toolroot,120)
save('diagnostic-terminal.json',dict(status=result,scope='one full original nanoc_v06 graph, unchanged ten-second shadow supervision; timing is diagnostic only'))
