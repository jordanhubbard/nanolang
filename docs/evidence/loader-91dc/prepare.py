...
save(base/'identity.json',dict(pin='91dc9700d',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 ...
 save(base/'terminal.json',dict(status=0,scope='remaining prepared-loader and SDK probe controls, Darwin sanitizer explicit child lifecycle; prior original8c93 terminal retained; real-provider sanitizer compiler prerequisite remains held'))
