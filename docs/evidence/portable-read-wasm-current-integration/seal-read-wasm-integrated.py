...
repo=pathlib.Path('/home/jkh/Src/nanolang-portable-read-wasm-ready');dest=repo/'docs/evidence/portable-read-wasm-current-integration';dest.mkdir(exist_ok=False);store=pathlib.Path('/tmp/nanolang-read-wasm-integrated-artifacts');store.mkdir(exist_ok=False)
remote=pathlib.Path('/tmp/nanolang-read-wasm-integrated-puck-downloaded');remote.mkdir(exist_ok=False)
with tarfile.open('/tmp/nanolang-read-wasm-integrated-puck-seal.tar.gz') as t:t.extractall(remote,filter='data')
def sha(p):
 h=hashlib.sha256()
 ...
 return h.hexdigest()
art={};summary={}
for host,root in [('linux',pathlib.Path('/tmp/nanolang-read-wasm-integrated-linux-seal')),('puck',remote/'nanolang-read-wasm-integrated-puck-seal')]:
...
for p in ['/tmp/run-wasm-read-integrated-query.py','/tmp/supervise-read-wasm-neighbors.py','/tmp/collect-read-wasm-seal.py','/tmp/seal-read-wasm-integrated.py']:shutil.copyfile(p,dest/pathlib.Path(p).name)
archive=pathlib.Path('/tmp/nanolang-read-wasm-integrated-puck-seal.tar.gz')
