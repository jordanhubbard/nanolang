...
h.dump('configuration.json',{'tools':extra,'env':{k:v for k,v in h.env.items() if k.startswith('PORTABLE_') or k in ['CC','PATH','SDKROOT','ASAN_OPTIONS','UBSAN_OPTIONS','LSAN_OPTIONS']},'scope':'ordinary integrated declaration query only; no native/engine repeats, no new sanitizer or installed CLI/package claim'})
phases=[]
def phase(name,fn):
