...
def tools():
 result={}
 for name in ('cc','gcc','clang','make','python3','ar','ld','pkg-config','brew',cc,nativecc):
...
write('environment.json',{k:v for k,v in os.environ.items() if k in ('PATH','CC','NANO_VM','NANO_NVM2C','NANO_AOT_RUNTIME','NANO_MODULE_PATH','NANO_BUILD_CACHE','PYTHONPATH','SDKROOT','ASAN_OPTIONS')})
(out/'runner.py').write_bytes(pathlib.Path(__file__).read_bytes())
if not linux:
 prefix=subprocess.check_output(['/opt/homebrew/bin/brew','--prefix','openssl'],text=True).strip()
 deps={'openssl_prefix':prefix,'sdk':os.environ['SDKROOT'],'libffi_flags':subprocess.check_output(['/opt/homebrew/bin/pkg-config','--cflags','--libs','libffi'],text=True).strip()}
 paths=[pathlib.Path(prefix)/'include/openssl/sha.h',pathlib.Path(prefix)/'lib/libcrypto.dylib',pathlib.Path(os.environ['SDKROOT'])/'usr/include/ffi/ffi.h']
 deps['files']={str(p):{'resolved':str(p.resolve()),'sha256':sha(p)} for p in paths}
...
try:
 if phase=='bootstrap':
  ...
  command('build',[make,'-j2','-f','Makefile.gnu','CC='+cc,'nano_virt','nano_vm','obj/test_affine_c_frontend'])
  write('fresh-stages.json',{str(root/'bin'/n):sha(root/'bin'/n) for n in ('nanoc_c','nanoc_stage1','nanoc_stage2')})
 else:raise RuntimeError('Only bootstrap and preparation authorized in this driver')
 status['success']=True
