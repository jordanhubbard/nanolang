...
root=pathlib.Path('/home/jkh/Src/nanolang-mixed-source-corrected');out=pathlib.Path('/tmp/nanolang-mixed-source-qualified-70c511dad');out.mkdir(exist_ok=False)
...
for path in (pathlib.Path(__file__),pathlib.Path('/tmp/nanolang-mixed-source-tests-corrected-run.py')):(out/path.name).write_bytes(path.read_bytes())
env=os.environ.copy()
...
cmd=['python3','/tmp/nanolang-mixed-source-tests-corrected-run.py',str(out/'tests')];status=1;start=time.monotonic();report={'pin':pin,'command':cmd}
try:
