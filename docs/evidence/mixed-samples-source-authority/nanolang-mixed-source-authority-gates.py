...
root=p.Path('/home/jkh/Src/nanolang-mixed-source-authority');old=p.Path('/home/jkh/Src/nanolang-mixed-source-provider');out=p.Path('/tmp/nanolang-mixed-source-authority-29ab099b5');out.mkdir(exist_ok=False)
sha=lambda f:hashlib.sha256(f.read_bytes()).hexdigest()
git=lambda *a:s.check_output(['git',*a],cwd=root,text=True).strip()
assert git('rev-parse','HEAD').startswith('29ab099b5') and not git('status','--porcelain')
...
commands=[('provider-tools',['make','-j2','bin/nanoc_c','nano_virt','nano_vm','nvm2c','nvm2llvm','nvm2hl','nanoisa_dump','test-local-binding-metadata']),('mixed-conjunction',['make','-j2','test-mixed-samples','test-mixed-samples-admission']),('source-smoke',['python3','/tmp/nanolang-mixed-source-final-smoke.py'])]
