...
pin=git('rev-parse','HEAD');assert pin.startswith('2ad92f457');assert not git('status','--porcelain','--untracked-files=no')
...
executables=[cc,san,shutil.which('python3'),shutil.which('make'),shutil.which('ar'),shutil.which('ld')]
def tool_map():
...
steps=[('bootstrap',['make','-j2','bootstrap',*flags],1800),('prepare',['make','-j2','nano_virt','nano_vm','nvm2c',*flags],900),('guards',[sys.executable,'-m','unittest','-v','tests.test_canonical_match_guards'],2400),('shared-policy',[sys.executable,'-m','unittest','-v','tests.test_shared_match_policy_docs','tests.test_cseed_match_totality'],1200),('owned-refusals',[sys.executable,'-m','unittest','-v','tests.test_selected_variant_ownership.SelectedVariantOwnership.test_guarded_owned_match_remains_rejected','tests.test_generic_selected_ownership.GenericSelectedOwnership.test_guarded_generic_match_remains_rejected'],1200)]
status=1
try:
