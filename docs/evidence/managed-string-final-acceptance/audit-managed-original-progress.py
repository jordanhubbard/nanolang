...
import signal
signal.alarm(300)
start=time.monotonic(); base=pathlib.Path(sys.argv[1]); problems=[]; refs={}; counters=collections.Counter()
...
for digest,size in refs.items():
 print('archive',digest,flush=True,file=sys.stderr)
...
phase=load('status.json');check(phase['passed'] and phase['tests_run']==23 and phase['commands']==456 and not phase['errors'] and not phase['failures'] and not phase['skipped'] and not phase['exception'] and not phase['retention_failures'],'phase result')
selected=load('selected-tests.json');check(len(selected)==23 and len(set(selected))==23,'selected test inventory')
log=(base/'unittest.log').read_text();check('Ran 23 tests' in log and log.rstrip().endswith('OK'),'unittest log');check(sum(' ... ok' in line for line in log.splitlines())==23,'individual test outcomes')
nonzero=[]
for i in range(1,457):
 ...
 req=load(prefix+'request.json');env=req['environment'];check(env['NMS_EMITTED_OPTIMIZATION']=='none' and env['LSAN_OPTIONS']=='' and 'detect_leaks=1' in env['ASAN_OPTIONS'],'command environment '+str(i))
