...
root=pathlib.Path('/home/jkh/Src/nanolang-shared-match-backstop-registration');old=pathlib.Path('/home/jkh/Src/nanolang-shared-match-ordering-controls');prior=pathlib.Path('/tmp/nanolang-match-855-linux-first');out=pathlib.Path('/tmp/nanolang-match-855-linux-corrected');out.mkdir(exist_ok=False)
def sha(p):
...
pin=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip();assert pin.startswith('cd25b66cd')
