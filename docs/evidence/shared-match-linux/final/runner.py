...
root=pathlib.Path('/home/jkh/Src/nanolang-shared-match-qualified');old=pathlib.Path('/home/jkh/Src/nanolang-shared-match-ordering-controls');prior=pathlib.Path('/tmp/nanolang-match-855-linux-first');out=pathlib.Path('/tmp/nanolang-match-855-linux-final');out.mkdir(exist_ok=False)
def sha(p):
...
pin=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip();assert pin.startswith('7a21096f8')
...
def run(name,args,limit):
 ...
 with (out/(name+'.log')).open('wb') as f:
  p=subprocess.Popen(args,cwd=(old if name=='evaluator' else root),stdout=f,stderr=subprocess.STDOUT,start_new_session=True)
  try:code=p.wait(timeout=limit)
   ...
   code=124
 save(name+'.json',{'argv':args,'cwd':str(old if name=='evaluator' else root),'exit_code':code,'seconds':time.monotonic()-start});print(name,code,flush=True)
