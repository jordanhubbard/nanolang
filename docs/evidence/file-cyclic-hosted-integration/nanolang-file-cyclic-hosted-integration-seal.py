...
root=Path('/home/jkh/Src/nanolang-file-cyclic-hosted-ready');out=root/'docs/evidence/file-cyclic-hosted-integration';out.mkdir(parents=True,exist_ok=False)
store=Path('/tmp/nanolang-file-cyclic-hosted-integration-artifacts');store.mkdir(exist_ok=False)
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
def write(p,value):p.write_text(json.dumps(value,sort_keys=True,indent=2)+'\n')
groups={'linux':Path('/tmp/nanolang-file-cyclic-hosted-integration-881fc-linux'),'darwin':Path('/tmp/nanolang-file-cyclic-hosted-integration-puck-downloaded/reports/darwin')}
...
for group,source in groups.items():
 ...
 assert len(statuses[group])==4 and all(x['status']==0 for x in statuses[group])
 ...
 for pattern in ('*-artifacts.json','*-inputs-before.json'):
  for p in dst.glob(pattern):
   for record in json.loads(p.read_text()).values():
    references+=1;digest=record['sha256'];src=Path(record['artifact']) if group.startswith('linux') else Path('/tmp/nanolang-file-cyclic-hosted-integration-puck-downloaded/artifacts')/digest
...
current=[]
for group,tree in [('linux',Path('/home/jkh/Src/nanolang-file-cyclic-hosted-integration-881fc'))]:
...
for name in ('/tmp/nanolang-file-cyclic-hosted-integration-seal.py','/tmp/nanolang-file-cyclic-hosted-integration-package-puck.py','/tmp/nanolang-file-cyclic-hosted-integration-driver.py','/tmp/nanolang-file-cyclic-hosted-integration-881fc-puck-launch.py','/tmp/nanolang-file-cyclic-hosted-integration-puck-packaging.json'):
 p=Path(name);shutil.copyfile(p,out/p.name)
archive=Path('/tmp/nanolang-file-cyclic-hosted-integration-artifacts.tar.gz')
...
write(out/'qualification-summary.json',{'production':'bae2badf9f566b0a2124bf395602c2e70d2390b5','qualified_fixture':'881fcf0a71e8a3f08a6fd3d2962dca6d1719fed7','cyclic_execution':False,'acyclic_scalar_control':True,'integrated_sanitizers':False,'runtime_admitted':False,'whole_program_sanitizer':False,'phases':statuses,'source_tool_pairs':pairs,'provider_pairs':provider_pairs,'current_linux':current,'unique_artifacts':len(index),'artifact_references':references,'artifact_bytes':sum(x['bytes'] for x in index.values())})
