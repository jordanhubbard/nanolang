...
save(base/'identity.json',dict(pin='1ac5e91df01b3cffadbbe97b61f0c9581fae52fb',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
