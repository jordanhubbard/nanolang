...
def run(label,cmd,extra=None):
 ...
 with (report/(label+'.log')).open('w') as out:
  ...
  completed=subprocess.CompletedProcess(cmd,124 if timed_out else proc.returncode)
  write(label+'-terminal.json',{'timeout':timed_out,'bound':1800,'returncode':proc.poll()})
...
config=subprocess.check_output(['make','-s','-f','Makefile.gnu','-f',str(probe),'carrier-config','CC='+os.environ['CC']],text=True,timeout=60)
...
run('normal',py+['tests.test_file_private_native'],dict(base,NANO_FILE_RUNTIME_CC=os.environ['CC'],NANO_FILE_RUNTIME_SANITIZERS='0'))
run('sanitizer',py+[os.environ.get('CARRIER_RUNTIME_SAN_TEST','tests.test_file_private_native')],dict(base,NANO_FILE_RUNTIME_CC=os.environ['CARRIER_SAN_CC'],NANO_FILE_RUNTIME_SANITIZERS='1'))
if not darwin:run('clang',py+['tests.test_file_private_native'],dict(base,NANO_FILE_RUNTIME_CC=shutil.which('clang'),NANO_FILE_RUNTIME_CFLAGS='--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13',NANO_FILE_RUNTIME_SANITIZERS='1'))
run('vm',py+['tests.test_file_private_vm'],dict(base,NANO_FILE_RUNTIME_CC=os.environ['CC'],NANO_FILE_RUNTIME_SANITIZERS='0'))
run('frames',py+['tests.test_file_runtime_frames'],dict(base,NANO_FILE_RUNTIME_CC=os.environ['CC'],NANO_FILE_RUNTIME_SANITIZERS='0'))
