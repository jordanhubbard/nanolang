...
for root in roots:
 ...
 for p in root.rglob('*'):
  ...
  if p.parent.name in ('objects','artifacts') and len(p.name)==64:
   ...
   continue
  if 'objects' in rel.parts or 'artifacts' in rel.parts or 'products' in rel.parts:continue
 ...
 for p in list(root.glob('*-status.json'))+list(root.glob('*-terminal.json')):
...
for p in reports.rglob('*.json'):
 def walk(x):
  ...
  if isinstance(x,dict):
   if 'sha256' in x and ('archive' in x or 'artifact' in x):
    assert x['sha256'] in store,(p,x.get('archive',x.get('artifact')))
    if 'bytes' in x:assert store[x['sha256']]['bytes']==x['bytes']
    refs+=1
   for v in x.values():walk(v)
  elif isinstance(x,list):
   for v in x:walk(v)
 walk(json.loads(p.read_text()))
summary={'roots':list(map(str,roots)),'artifacts':len(store),'artifact_bytes':sum(v['bytes'] for v in store.values()),'references':refs,'pairs':pairs,'terminals':terminals,'scope':'Freshbc45 ordinary constructors/admission/real neighbors/emitter and separate public cyclic package; public provider products deliberately rebuilt, only their source/tool pairs claimed equal'}
