...
def inventory(extra=()):
 ...
 return entries
write('environment.json',{k:v for k,v in os.environ.items() if k in ('CC','CARRIER_SAN_CC','SDKROOT','LIBRARY_PATH','NMS_NATIVE_CLANG_FLAGS','NMS_RUNTIME_CLANG','NMS_RUNTIME_OPT','PATH','CARRIER_PHASES','NANO_FILE_RUNTIME_CFLAGS','CARRIER_EXTRA_TOOLS','LSAN_OPTIONS','ASAN_OPTIONS','UBSAN_OPTIONS')})
...
write('terminal.json',{'status':'PASS','scope':'Fresh ordinary public cyclic corpus and real package in separate tree; source/tools equal; provider rebuild products recorded, not immutable'})
