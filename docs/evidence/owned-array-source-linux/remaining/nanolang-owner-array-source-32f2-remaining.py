...
out=pathlib.Path('/tmp/nanolang-owner-array-source-32f2-remaining');out.mkdir(exist_ok=False);work=out/'work';work.mkdir()
...
prior_groups=('optional-local','optional-add','optional-negate','bound-at')
write('prior-evidence.json',{'excluded_groups':prior_groups,'source':'/tmp/nanolang-owner-array-source-b9ecc-remaining.log','new_pass_claim':False})
original_refused=cls.refused
executed_groups=[]
def remaining_refused(self,name,text,shadows=False):
 if name in prior_groups:
  print('PRIOR EVIDENCE ONLY:',name,flush=True);return
 executed_groups.append(name)
 return original_refused(self,name,text,shadows)
cls.refused=remaining_refused
...
try:
 ...
 write('status.json',{'executed_refusal_groups':executed_groups,'seconds':time.monotonic()-start,'testsRun':result.testsRun if result else 0,'errors':len(result.errors) if result else None,'failures':len(result.failures) if result else None,'success':result.wasSuccessful() if result else False})
