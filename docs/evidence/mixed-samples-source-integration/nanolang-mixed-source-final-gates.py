...
root=p.Path('/home/jkh/Src/nanolang-mixed-source-final');old=p.Path('/home/jkh/Src/nanolang-mixed-source-corrected');out=p.Path('/tmp/nanolang-mixed-source-integration-6a492a7aa');out.mkdir(exist_ok=False)
sha=lambda f:hashlib.sha256(f.read_bytes()).hexdigest()
git=lambda *a:s.check_output(['git',*a],cwd=root,text=True).strip()
assert git('rev-parse','HEAD').startswith('6a492a7aa') and not git('status','--porcelain')
...
commands=[('tools',['make','-j2','bin/nanoc_c','nano_virt','nano_vm','nvm2c','nanoisa_dump','test-local-binding-metadata']),('mixed',['make','-j2','test-mixed-layout-view','test-mixed-float-proof','test-mixed-samples','test-mixed-samples-admission','test-mixed-samples-runtime']),('service',['make','-j2','test-service-module']),('source-smoke',['python3','/tmp/nanolang-mixed-source-final-smoke.py'])]
