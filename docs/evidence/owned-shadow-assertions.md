...
`make test-owned-assertions` passes 617 lifecycle checks and four paired
artifacts. I exercise true and false entry/helper assertions through
`vm_invoke`, `vm_execute` and `vm_call_function`, repeat each path eight times,
...
Both passing and failing serialized artifacts go through normal VM execution
...
All 617 checks and paired artifacts pass. Generated native harnesses also pass
...
Logs are `/tmp/nanolang-owned-assertions-gates.log`,
`/tmp/nanolang-owned-assertions-sanitizers.log`,
`/tmp/nanolang-owned-assertions-clang.log`,
`/tmp/nanolang-owned-assertions-clang18.log` and
