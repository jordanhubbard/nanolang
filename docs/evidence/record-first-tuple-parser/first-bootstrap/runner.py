...
root=pathlib.Path('/home/jkh/Src/nanolang-record-first-tuple-parser')
out=pathlib.Path('/tmp/nanolang-record-first-bbafb-bootstrap');out.mkdir(exist_ok=False)
...
try:
 for name,cmd in [('bootstrap',['make','-j2','bootstrap','NMS_NATIVE_CLANG_FLAGS=--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13'])]:
  print('START',name,flush=True);start=time.monotonic()
  with (out/(name+'.log')).open('wb') as log:result=subprocess.run(cmd,cwd=root,env=env,stdout=log,stderr=subprocess.STDOUT,timeout=1800)
