...
for key in ('CFLAGS','CPPFLAGS','LDFLAGS','SDKROOT','LD_PRELOAD','DYLD_INSERT_LIBRARIES'):env.pop(key,None)
def run(label,args):
...
try:
 for mode,cc in zip(['ordinary','sanitized'],compilers):
  ...
  for fixture in ('signature','module'):
   exe=base/(mode+'-'+fixture)
   run(mode+'-'+fixture+'-build',[cc,*flags,'tests/test_sdk_'+fixture+'_snapshot.c','-o',str(exe)])
   before=ident(exe);save(base/(mode+'-'+fixture+'-before.json'),before)
   try:run(mode+'-'+fixture,[str(exe)])
   finally:
    after_product=ident(exe);save(base/(mode+'-'+fixture+'-after.json'),after_product);assert before==after_product
