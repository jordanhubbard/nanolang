...
root=pathlib.Path('/home/jkh/Src/nanolang-owned-array-source-final');out=pathlib.Path('/tmp/nanolang-owner-array-source-1c4e-gates');out.mkdir(exist_ok=False)
...
assert all(x['status']==0 for x in json.loads(pathlib.Path('/tmp/nanolang-owner-array-source-1c4e-bootstrap/status.json').read_text())['steps'])
for k,v in json.loads(pathlib.Path('/tmp/nanolang-owner-array-source-1c4e-bootstrap/environment.json').read_text()).items():os.environ[k]=v
