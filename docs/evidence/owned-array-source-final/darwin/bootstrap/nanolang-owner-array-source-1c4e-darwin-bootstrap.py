...
root=pathlib.Path('/private/tmp/nanolang-owner-array-source-1c4e29067')
out=pathlib.Path('/private/tmp/nanolang-owner-array-source-1c4e-darwin-bootstrap');out.mkdir(exist_ok=False)
...
''')
env=os.environ.copy();env['SDKROOT']=subprocess.check_output(['/usr/bin/xcrun','--sdk','macosx','--show-sdk-path'],text=True).strip()
for k in ('NANOC','NANO_VM','NANO_NVM2C','NANO_AOT_RUNTIME','NANO_BUILD_CACHE','NANO_MODULE_PATH'):env.pop(k,None)
env.update(CC='/usr/bin/clang',NANO_VM=str(root/'bin/nano_vm'),NANO_NVM2C=str(root/'bin/nvm2c'),NANO_AOT_RUNTIME=str(root/'bin/nano_aot_runtime.o'),NANO_MODULE_PATH=str(root/'modules'),NANO_BUILD_CACHE=str(root/'obj/module_cache'),PYTHONPATH=str(root))
write('environment.json',{k:env[k] for k in ('SDKROOT','CC','NANO_VM','NANO_NVM2C','NANO_AOT_RUNTIME','NANO_MODULE_PATH','NANO_BUILD_CACHE','PYTHONPATH')})
report={'pin':pin,'steps':[]};status=1
try:
 for name,cmd in [('bootstrap',['/usr/bin/make','-j2','-f','Makefile.gnu','CC=/usr/bin/clang','bootstrap']),('setup',['/usr/bin/make','-j2','-f','Makefile.gnu','-f',str(setup),'CC=/usr/bin/clang','owned-array-source-setup'])]:
