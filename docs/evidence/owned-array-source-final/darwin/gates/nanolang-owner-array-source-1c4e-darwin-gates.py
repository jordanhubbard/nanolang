...
root=pathlib.Path('/private/tmp/nanolang-owner-array-source-1c4e29067');out=pathlib.Path('/private/tmp/nanolang-owner-array-source-1c4e-darwin-gates');out.mkdir(exist_ok=False)
os.chdir(root);sys.path.insert(0,str(root))
pin='1c4e29067aa21ee843e00997d42851877f7082fe'
def git(*a):return subprocess.check_output(['git',*a],text=True).strip()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
assert git('rev-parse','HEAD')==pin and not git('status','--porcelain')
assert all(x['status']==0 for x in json.loads(pathlib.Path('/private/tmp/nanolang-owner-array-source-1c4e-darwin-bootstrap/status.json').read_text())['steps'])
for k,v in json.loads(pathlib.Path('/private/tmp/nanolang-owner-array-source-1c4e-darwin-bootstrap/environment.json').read_text()).items():os.environ[k]=v
