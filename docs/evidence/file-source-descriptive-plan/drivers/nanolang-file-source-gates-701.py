...
if platform=='linux':
 ...
 configs=[('gcc-ordinary',cc,[],False,True),('clang-ordinary',other,extra,False,True),('gcc-sanitizer',cc,[],True,False),('clang-sanitizer',other,extra,True,False)]
else:
 cc='/usr/bin/clang';other='/opt/homebrew/opt/llvm/bin/clang';extra=[]
 env['PATH']='/opt/homebrew/bin:'+env.get('PATH','')
 configs=[('apple-ordinary',cc,[],False,True),('homebrew-ordinary',other,[],False,True),('homebrew-sanitizer',other,[],True,False)]
...
dump('driver.json',hashfile(__file__,True));dump('configuration.json',{'pin':'701dca50f','platform':platform,'configs':configs,'source_count':len(source_paths),'source_scope':'all tracked files at the frozen pin','sanitizer_scope':'fresh three C providers and C fixtures only; ordinary generated Nano products','tool_scope':'explicit drivers, selected subtools, sanitizer libraries and Darwin ffi headers/library; not complete transitive system toolchain'})
def phase(name,args,phase_env,timeout):
 ...
 return ok
dump('bootstrap-reuse.json',json.loads(pathlib.Path(sys.argv[5]).read_text()))
...
dump('terminal.json',{'status':'PASS','pin':'701dca50f','configs':[x[0] for x in configs],'all_input_pairs_equal':True})
