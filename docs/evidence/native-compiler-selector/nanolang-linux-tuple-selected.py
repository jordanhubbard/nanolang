...
root=Path('/home/jkh/Src/nanolang-file-cyclic-public-qualified-4c85');report=Path('/tmp/nanolang-linux-tuple-selected');report.mkdir(exist_ok=False);os.chdir(root);sys.path.insert(0,str(root))
selected=Path('/home/jkh/Src/nanolang-capture-flags/tests/test_nanoisa_tuple_values.py');spec=importlib.util.spec_from_file_location('selected_tuple',selected);suite=importlib.util.module_from_spec(spec);spec.loader.exec_module(suite);suite.ROOT=root
os.environ.pop('NANO_NATIVE_TEST_CC',None);index=0
paths=[selected,Path(__file__),root/'bin/nanoc_c',root/'bin/nano_virt',root/'bin/nano_vm',root/'bin/nanoisa',root/'bin/nvm2c',Path(sys.executable),Path('/usr/bin/gcc'),Path(subprocess.check_output(['/usr/bin/gcc','-print-file-name=libasan.so'],text=True).strip())]
