...
root=Path('/private/tmp/nanolang-file-cyclic-public-final');report=Path('/tmp/nanolang-darwin-projected-selected');report.mkdir(exist_ok=False)
os.chdir(root);sys.path.insert(0,str(root))
import importlib.util
selected=Path('/tmp/nanolang-darwin-aggregate-selectors/tests/test_nanoisa_flat_records.py')
...
index=0
paths=[root/'tests/test_nanoisa_flat_records.py',root/'bin/nanoisa',root/'bin/nano_vm',root/'bin/nvm2c',Path('/usr/bin/cc'),Path(sys.executable),Path(__file__),selected,Path('/opt/homebrew/opt/llvm/bin/clang'),Path('/opt/homebrew/opt/llvm/lib/clang/23/lib/darwin/libclang_rt.asan_osx_dynamic.dylib')]
