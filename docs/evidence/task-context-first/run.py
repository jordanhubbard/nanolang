...
save(base/'identity.json',dict(pin='1ff96ffe2c81fe39eac6e2abe33ca15c1746b995',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
