...
roots={'darwin':Path('/tmp/nanolang-file-cyclic-04d58-puck')}
base=Path('/tmp/nanolang-file-cyclic-puck-package');base.mkdir(exist_ok=False);reports=base/'reports';reports.mkdir();objects=base/'artifacts';objects.mkdir()
...
for group,root in roots.items():
 ...
 for p in root.glob('*-inputs-before.json'):
  for d in json.loads(p.read_text()).values():
   ...
   if sha not in index:shutil.copyfile(source,objects/sha);index[sha]={'bytes':source.stat().st_size}
current=[]
for group,root,tree,phase in [('darwin',roots['darwin'],Path('/tmp/nanolang-file-cyclic-04d58'),'ordinary')]:
...
for name,source in [('reports',reports),('artifacts',objects)]:
 archive=Path('/tmp/nanolang-file-cyclic-puck-'+name+'.tar.gz')
...
Path('/tmp/nanolang-file-cyclic-puck-packaging.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary,indent=2))
