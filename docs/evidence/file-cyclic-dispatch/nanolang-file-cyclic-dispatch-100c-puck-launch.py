...
root=Path('/tmp/nanolang-file-cyclic-dispatch-100c'); archive=Path('/tmp/nanolang-file-cyclic-dispatch-100c-source.tar.gz')
manifest=json.loads(Path('/tmp/nanolang-file-cyclic-dispatch-100c-source.json').read_text())
...
subprocess.run(['/opt/homebrew/bin/python3','/tmp/nanolang-file-cyclic-dispatch-100c-driver.py',str(root),'/tmp/nanolang-file-cyclic-dispatch-100c-puck'],check=True,env=env)
