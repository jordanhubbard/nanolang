...
def sha(p):
 ...
 return h.hexdigest()
groups={pin:Path('/tmp/nanolang-file-cyclic-dispatch-'+pin+'-puck') for pin in ('9284','7913','100c')}
records={};current=[]
for pin,report in groups.items():
 results=json.loads((report/'results.json').read_text())
 if pin=='100c':assert len(results)==14 and all(r['status']==0 for r in results),results
...
archive=Path('/tmp/nanolang-file-cyclic-dispatch-puck-evidence.tar.gz')
...
Path('/tmp/nanolang-file-cyclic-dispatch-puck-packaging.json').write_text(json.dumps(result,indent=2)+'\n')
