...
report={'head':pin,'host':platform.node(),'platform':platform.platform(),'scope':'fresh C seed, unchanged example regressions, four changed example compilations and full available strict examples; no interactive windows run; deleted inner temporary products not reconstructed','phases':[]};save('manifest.json',report)
...
phases=[('build',['make','-j2','bin/nanoc_c',*flags],1800),('regressions',['make','test-examples-regressions',*flags],180)]
for source in ('language/nl_random_sentence','graphics/sdl_fire','graphics/sdl_starfield','terminal/ncurses_matrix_rain'):
 phases.append((source.split('/')[-1],[str(root/'bin/nanoc_c'),'examples/'+source+'.nano','-o',str(out/source.split('/')[-1]),'--keep-c'],120))
