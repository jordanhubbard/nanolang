...
pin=git('rev-parse','HEAD');assert pin=='28ba6c21373988f09de3fa6e5ba38e3131f184ce'
assert not git('status','--porcelain','--untracked-files=no')
assert (root/'bin/nanoc_c').exists()
# I retain the existing stage1 alias from the previous qualification.
...
selected=[shutil.which('cc'),shutil.which('c++'),cc,native,sys.executable,shutil.which('python3'),shutil.which('make'),shutil.which('ar'),shutil.which('ld'),shutil.which('perl'),shutil.which('git')]
...
report={'head':pin,'host':platform.node(),'platform':platform.platform(),'scope':'retained qualified C seed plus actual CI stage1 prerequisite without global CC overriding module C++ selection, unchanged example regressions, four changed example compilations and full available strict examples; no interactive windows run; deleted inner temporary products not reconstructed','phases':[]};save('manifest.json',report)
...
env.pop('CC',None)
flags=['NMS_RUNTIME_CLANG='+llvm+'/clang','NMS_RUNTIME_OPT='+llvm+'/opt','NMS_NATIVE_CLANG_FLAGS='+env['NMS_NATIVE_CLANG_FLAGS']]
phases=[('build',['make','-j2','stage1',*flags],1800),('regressions',['make','test-examples-regressions',*flags],180)]
for example_source in ('language/nl_random_sentence','graphics/sdl_fire','graphics/sdl_starfield','terminal/ncurses_matrix_rain'):
 phases.append((example_source.split('/')[-1],[str(root/'bin/nanoc_c'),'examples/'+example_source+'.nano','-o',str(out/example_source.split('/')[-1]),'--keep-c'],120))
phases.append(('strict-examples',['make','examples','EXAMPLES_TIMEOUT=2400',*flags],2500))
for name,args,bound in phases:
 if not command(name,args,bound):sys.exit(1)
save('retained-example-products.json',snapshot(p for p in out.iterdir() if p.is_file() and (p.name.startswith('nl_') or p.name.startswith('sdl_') or p.name.startswith('ncurses_'))))
