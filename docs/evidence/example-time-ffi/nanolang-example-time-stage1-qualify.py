...
assert not (root/'bin/nanoc').exists()
...
report={'head':pin,'host':platform.node(),'platform':platform.platform(),'scope':'retained qualified C seed plus actual CI stage1 prerequisite, unchanged example regressions, four changed example compilations and full available strict examples; no interactive windows run; deleted inner temporary products not reconstructed','phases':[]};save('manifest.json',report)
