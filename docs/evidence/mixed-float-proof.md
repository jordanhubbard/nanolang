# I qualify closed FLOAT provenance without admission

My [sealed reports](mixed-float-proof/manifest.json) preserve first query pin830d8bd4
and corrected pinae07f575 separately. The first339-check suite did not complete:
an ordinary branch query rejected pc29, and the C fixture assertion terminated.
No pending bytecode executed. Recorded branch correctionae07 uses opcode PC plus
signed displacement, matching existing decoder/verifier semantics.

The corrected focused GCC suite passed339 checks in0.916s; Clang passed339 checks
in1.367s with ASan/UBSan and leak detection. Five adjacent suites passed16.508s:
...
instrumented in Clang. Other linked objects used their ordinary builds. These are
query/allocation checks, not instrumented VM execution. The suite covers weak
repeated-site field unions, both predecessor orders, receiver alternatives,
VOID-local joins, FLOAT-or-VOID read facts and unsatisfied typed-F64 obligations,
wrong parent nominal, unused invalid functions/opcodes, bounded origin/instruction/
