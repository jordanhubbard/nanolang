...
compilers=['/usr/bin/clang','/opt/homebrew/opt/llvm/bin/clang'] if darwin else ['/usr/bin/gcc','/usr/bin/gcc'];tools={c:ident(pathlib.Path(c).resolve()) for c in compilers};save(base/'identity.json',{'pin':'1ff96ffe2c81fe39eac6e2abe33ca15c1746b995','archive':ident(archive),'driver':ident(pathlib.Path(__file__)),'host':platform.uname()._asdict(),'sdk':sdk,'tools':tools})
...
try:
 for mode,cc in zip(['ordinary','sanitized'],compilers):
  ...
  exe=base/(mode+'-codec');run(mode+'-build',[cc,*flags,'tests/test_sdk_signature_snapshot.c','-o',str(exe)]);run(mode+'-codec',[str(exe)])
