...
ccs=[('gcc','/bin/gcc',{}),('clang','/usr/local/bin/clang',{'NANO_FILE_PLAN_TEST_CFLAGS':'--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13'})] if sys.platform!='darwin' else [('darwin','/opt/homebrew/opt/llvm/bin/clang',{})]
...
steps=[(name,['python3','-m','unittest','-f','-v','tests.test_nsi_file_plan'],{'NANO_FILE_PLAN_TEST_CC':cc,**extra}) for name,cc,extra in ccs]
steps.append(('normal',['make','CC='+('/usr/bin/gcc' if sys.platform!='darwin' else '/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/clang'),'test-nsi-file-plan'],{}))
