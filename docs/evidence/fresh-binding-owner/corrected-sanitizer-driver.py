import hashlib,json,os,pathlib,platform,shlex,shutil,signal,subprocess,sys,tarfile,time,tempfile
...
assert selected['pin']=='ead442cf9a53a406385e6f4e0c188b67c5abbdd5'
assert len(selected['files'])==4017
with tarfile.open(archive) as tf:
 members=tf.getmembers()
 assert len(members)==4017 and {m.name for m in members}==set(selected['files'])
...
def verify_linked_terminal():
 ...
 for fixture in (root/'obj/test_native_nominal_context',root/'obj/test_env_symbol_index'):
  if fixture.is_file():
...
try:
 for name,compiler,san in [('sanitizer',sancc,True)]:
  linked_before=None
  copied_links={}
  ...
  tmp=pathlib.Path(tempfile.mkdtemp(prefix='nl4-',dir='/tmp'));env['TMPDIR']=str(tmp);env['NANOLANG_SDK_ROOT']=str(root)
  save(out/'private-tmpdir.json',dict(path=str(tmp),owner_uid=tmp.stat().st_uid,mode=oct(tmp.stat().st_mode & 0o777),config=name,run=str(base)))
  ...
  run('fresh-providers', [*args,'bin/nanoc_c','bin/nanoc-ffi','nano_virt','nano_vm','nano_vmd','nano_cop','nvm2c','nanoisa_dump','nvm2c-runtime'],root,out,1800)
  linked_before=linked_closure(root);save(out/'linked-providers-before.json',linked_before)
  args += ['--assume-old='+target for target in ('nano_virt','nano_vm','nano_vmd','nano_cop','nvm2c','nanoisa_dump','nvm2c-runtime')]
  ...
  run('symbol-index', [*args,'test-env-symbol-index'],root,out,1200)
  run('nominal-context', [*args,'test-native-nominal-context'],root,out,1200)
  run('original-tco', [sys.executable,'-m','unittest','-f','-v','tests.test_native_tco'],root,out,1200)
  run('parameter-metadata', [sys.executable,'-m','unittest','-f','-v','tests.test_parameter_nominal_metadata'],root,out,1200)
 ...
 save(base/'terminal.json',dict(status=0,scope='fresh ordinary and sanitizer direct nominal binding metadata, original TCO suite plus additive mixed-name case, and original parameter metadata controls'))
