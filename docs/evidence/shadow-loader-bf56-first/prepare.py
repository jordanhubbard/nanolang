...
save(base/'identity.json',dict(pin='bf56ca5f8',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitizer',sancc,True)]:
  ...
  run('fresh-providers',[*args,'bin/nanoc_c','nano_virt','nano_vm','nvm2c','nanoisa_dump','nvm2c-runtime'],root,out,1800)
  ...
  run('fork-failure-controls', [*args,'test-shadow-loader-fork-failure'],root,out,1200)
  run('exact-contracts',[sys.executable,'-m','unittest','-f','-v','tests.test_one_ir_compiler.OneIrCompiler.test_nanoisa_artifact_contracts_remain_exact'],root,out,1200)
  run('borrowed-string-controls',[sys.executable,'-m','unittest','-f','-v','tests.test_compiler_support_artifact_adapters.CompilerSupportArtifactAdapters.test_mutable_borrowed_root_is_snapshotted','tests.test_compiler_support_artifact_adapters.CompilerSupportArtifactAdapters.test_borrowed_string_arguments_and_isolated_snapshots'],root,out,1200)
  run('loader-admission-controls',[sys.executable,'-m','unittest','-f','-v','tests.test_compiler_support_artifact_adapters.CompilerSupportArtifactAdapters.test_loader_prepared_fork_contract'],root,out,1200)
 ...
 save(base/'terminal.json',dict(status=0,scope='exact catalog and borrowed zero/one/two string VM/COP/native plus refusal/allocation, prepared-loader and original SDK probe controls, both actual shadow-fork failures and complete callback-shadow suite with constructor/thread child placement; full real-provider compiler prerequisite and original full typecheck remain separate'))
