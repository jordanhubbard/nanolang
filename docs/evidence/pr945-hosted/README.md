...
The reviewed one-line correction is source `585a6bc12` on this separate branch. My fresh actual Make gate supplies `CC` and `LDFLAGS` only as command-line assignments, removing inherited values first. Its original 90-method ordinary suite passes; coverage and sanitizer qualification are still running. Their eventual results will not relabel this hosted run. This correction does not modify runtime or fixture assertions.
