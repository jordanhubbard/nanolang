...
base={'FILE_INDIRECT_HOSTED_OBJECTS':values['ISA'],'FILE_INDIRECT_HOSTED_LDFLAGS':values['LDFLAGS']}
run('discovery',[sys.executable,'-c',"import unittest; s=unittest.defaultTestLoader.loadTestsFromName('tests.test_file_indirect_hosted'); print(s.countTestCases()); assert s.countTestCases()==2"])
run('ordinary',py+['tests.test_file_indirect_hosted'],dict(base,NANO_FILE_INDIRECT_HOSTED_CC=os.environ['CC'],NANO_FILE_INDIRECT_HOSTED_CFLAGS=normal_flags,NANO_FILE_INDIRECT_HOSTED_SANITIZERS='0'))
run('clang-ordinary',py+['tests.test_file_indirect_hosted'],dict(base,NANO_FILE_INDIRECT_HOSTED_CC=shutil.which('clang'),NANO_FILE_INDIRECT_HOSTED_CFLAGS=clang_flags,NANO_FILE_INDIRECT_HOSTED_SANITIZERS='0'))
run('sanitizer',py+['tests.test_file_indirect_hosted'],dict(base,NANO_FILE_INDIRECT_HOSTED_CC=os.environ['CARRIER_SAN_CC'],NANO_FILE_INDIRECT_HOSTED_CFLAGS=normal_flags,NANO_FILE_INDIRECT_HOSTED_SANITIZERS='1'))
if not darwin:run('clang-sanitizer',py+['tests.test_file_indirect_hosted'],dict(base,NANO_FILE_INDIRECT_HOSTED_CC=shutil.which('clang'),NANO_FILE_INDIRECT_HOSTED_CFLAGS=clang_flags,NANO_FILE_INDIRECT_HOSTED_SANITIZERS='1'))
...
run('hosted',py+['tests.test_file_hosted'],dict(FILE_HOSTED_OBJECTS=values['ISA'],FILE_HOSTED_LDFLAGS=values['LDFLAGS'],NANO_FILE_HOSTED_CC=compiler,NANO_FILE_HOSTED_CFLAGS=normal_flags,NANO_FILE_HOSTED_SANITIZERS='1'))
...
for name in ('indirect_targets','indirect_flow'):
 opts={f'FILE_{name.upper()}_OBJECTS':filtered,f'FILE_{name.upper()}_LDFLAGS':values['LDFLAGS'],f'NANO_FILE_{name.upper()}_CC':compiler,f'NANO_FILE_{name.upper()}_CFLAGS':normal_flags+' -fsanitize=address,undefined -fno-omit-frame-pointer'}
 run(name,py+[f'tests.test_file_{name}'],opts)
