...
roots={'darwin-first':Path('/tmp/nanolang-file-indirect-hosted-225b-puck'),'darwin':Path('/tmp/nanolang-file-indirect-hosted-5c26-puck')}
base=Path('/tmp/nanolang-file-indirect-hosted-puck-package');base.mkdir(exist_ok=False);reports=base/'reports';reports.mkdir();objects=base/'artifacts';objects.mkdir()
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest();index={};references=0
for group,root in roots.items():
 results=json.loads((root/'results.json').read_text());assert (len(results)==4 and [x['status'] for x in results]==[0,0,0,1]) if group.endswith('first') else (len(results)==15 and all(x['status']==0 for x in results))
 ...
 for p in root.glob('*-inputs-before.json'):
  for d in json.loads(p.read_text()).values():
   ...
   if sha not in index:shutil.copyfile(source,objects/sha);index[sha]={'bytes':source.stat().st_size}
current=[]
for group,root,tree,phase in [(g,r,Path('/tmp/nanolang-file-indirect-hosted-'+('225b' if g.endswith('first') else '5c26')),'ordinary') for g,r in roots.items()]:
...
for name,source in [('reports',reports),('artifacts',objects)]:
 archive=Path('/tmp/nanolang-file-indirect-hosted-puck-'+name+'.tar.gz')
...
Path('/tmp/nanolang-file-indirect-hosted-puck-packaging.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary,indent=2))
