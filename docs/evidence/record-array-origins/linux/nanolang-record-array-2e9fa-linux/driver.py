...
py=[sys.executable,'-m','unittest','-f','-v','tests.test_record_array_origins']
clang=shutil.which('clang');clang_flags='' if darwin else '--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13'
sanitize=' -fsanitize=address,undefined -fno-omit-frame-pointer'
run('discovery',[sys.executable,'-c',"import unittest;from tests import test_record_array_origins as m;s=unittest.defaultTestLoader.loadTestsFromModule(m);print(s.countTestCases());assert s.countTestCases()==1"])
