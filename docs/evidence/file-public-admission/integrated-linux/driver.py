...
run('setup',['make','-f','Makefile.gnu','-f',str(probe),'-j2','CC='+env['PUBLIC_CC'],'public-providers','nvm2c','nvm2llvm','nvm2hl'],env,1200)
...
base=dict(env,FILE_RUNTIME_OBJECTS=values['ALL'],FILE_RUNTIME_LDFLAGS=values['LDFLAGS'])
matrix=[('ordinary-integrated',env['PUBLIC_CC'],'0')]
for label,cc,san in matrix:
 flags='--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13' if not darwin and 'clang' in label else ''
 if darwin:flags=os.environ.get('PUBLIC_DARWIN_CFLAGS','')
 selected_cc=shlex.join([cc,'--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13']) if not darwin and 'clang' in label else cc
 phase=dict(base,NANO_FILE_RUNTIME_CC=selected_cc,NANO_FILE_RUNTIME_CFLAGS=flags,NANO_FILE_RUNTIME_SANITIZERS=san)
 run(label,[sys.executable,'-m','unittest','-f','-v','tests.test_file_public'],phase)

cyclic_objects=' '.join(p for p in values['ISA'].split() if not p.endswith(('/nanoisa/file_flow.o','/nanoisa/service_file_nominal.o','/nanoisa/service_file_nominal_plan.o','/nsi_file_plan.o')))
flags=os.environ.get('PUBLIC_DARWIN_CFLAGS','') if darwin else ''
neighbor=dict(base,FILE_CYCLIC_OBJECTS=cyclic_objects,FILE_CYCLIC_LDFLAGS=values['LDFLAGS'],NANO_FILE_CYCLIC_CC=env['PUBLIC_CC'],NANO_FILE_CYCLIC_CFLAGS=flags,FILE_OPCODE_OBJECTS=values['OPCODES'],FILE_OPCODE_LDFLAGS=values['LDFLAGS'],NANO_FILE_OPCODE_TEST_CC=env['PUBLIC_CLANG'] if darwin else env['PUBLIC_CC'],NANO_FILE_OPCODE_TEST_CFLAGS=flags)
run('cyclic-query',[sys.executable,'-m','unittest','-f','-v','tests.test_file_cyclic'],neighbor)
run('opcode-refusal',[sys.executable,'-m','unittest','-f','-v','tests.test_file_opcodes'],neighbor)
