...
run('setup',['make','-f','Makefile.gnu','-f',str(probe),'-j2','CC='+env['PUBLIC_CC'],'public-providers'],env,1200)
...
if darwin:matrix=[('apple-normal',env['PUBLIC_CC'],'0'),('clang-normal',env['PUBLIC_CLANG'],'0'),('clang-sanitizer',env['PUBLIC_CLANG'],'1')]
else:matrix=[('gcc-normal',env['PUBLIC_CC'],'0'),('gcc-sanitizer',env['PUBLIC_CC'],'1'),('clang-normal',env['PUBLIC_CLANG'],'0'),('clang-sanitizer',env['PUBLIC_CLANG'],'1')]
for label,cc,san in matrix:
 ...
 phase=dict(base,NANO_FILE_RUNTIME_CC=cc,NANO_FILE_RUNTIME_CFLAGS=flags,NANO_FILE_RUNTIME_SANITIZERS=san)
