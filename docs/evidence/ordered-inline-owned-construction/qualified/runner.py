...
root=pathlib.Path('/home/jkh/Src/nanolang-ordered-inline-owned-construction')
out=pathlib.Path('/tmp/nanolang-inline-owned-qualified');out.mkdir(exist_ok=False)
...
before=sources();write('source-before.json',before);(out/'runner.py').write_bytes(pathlib.Path(__file__).read_bytes())
...
steps=[('prepare',['make','-j2','--eval='+prep,'inline-prepare'],{}),('focused-gcc',['python3','-m','unittest','-v','tests.test_inline_owned_construction'],{'CC':'gcc'}),('focused-clang',['python3','-m','unittest','-v','tests.test_inline_owned_construction'],{'CC':'/tmp/nanolang-inline-clang'}),('source-regression',['python3','-m','unittest','-v','tests.test_source_borrow_emission'],{}),('runtime',['make','-j2','test-affine-state','test-affine-bytecode','test-owned-transfers','test-owned-runtime','test-owned-value-graphs','test-owned-value-results','test-owned-assertions'],{})]
try:
