...
def tools():
 ...
 if sys.platform=='darwin':paths['actual_apple_clang']=subprocess.check_output(['xcrun','--find','clang'],text=True).strip()
...
provider=['make','-j4','nano_virt']
if sys.platform=='darwin':provider.append('CC='+subprocess.check_output(['xcrun','--find','clang'],text=True).strip())
steps=[('provider',provider),('wrapper',['bin/nano_virt','tests/test_minimal.nano','-o',str(report/'wrapper')]),('wrapper-run',[str(report/'wrapper')])]
results=[]
