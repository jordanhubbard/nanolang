...
save(base/'identity.json',dict(pin='d480b0c40',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitizer',sancc,True)]:
  ...
  run('actual-and-borrowed-provider',[sys.executable,'-m','unittest','-f','-v','tests.test_compiler_support_artifact_adapters.CompilerSupportArtifactAdapters.test_mutable_borrowed_root_is_snapshotted'],root,out,1200)
 ...
 save(base/'terminal.json',dict(status=0,scope='corrected mutable borrowed-root VM/native control only; prior actual-provider ordinary proof separate and known real-provider sanitizer compiler prerequisite held'))
