...
save(base/'identity.json',dict(pin='8c93ebf26',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitizer',sancc,True)]:
  ...
  run('fresh-providers',[*args,'nano_vm','nvm2c','nanoisa_dump','nvm2c-runtime'],root,out,1800)
 ...
 save(base/'terminal.json',dict(status=0,scope='exact catalog and borrowed zero/one/two string VM/COP/native plus refusal/allocation, prepared-loader and original SDK probe controls; real-provider sanitizer compiler prerequisite remains held'))
