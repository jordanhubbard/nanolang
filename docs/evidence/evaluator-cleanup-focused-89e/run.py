...
save(base/'identity.json',dict(pin='89e41b8fa0558a4bb631eff371fa71f2c2090954',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  (root/'tests/focused_eval_cleanup.c').write_text('#define main retained_full_eval_main\n#include "test_eval.c"\n#undef main\nint main(void) {\n TEST(eval_handler_return_async_calls);\n TEST(eval_callable_projection_lifetimes);\n TEST(eval_struct_creation_and_access);\n TEST(eval_tuple_types);\n TEST(eval_effect_argument_lists);\n puts("I completed the bounded evaluator cleanup controls.");\n return 0;\n}\n')
  (root/'focused-eval.mk').write_text('include Makefile.gnu\n.PHONY: focused-eval\nfocused-eval: stage1 $(OBJ_DIR)/test_interpreter_ffi_native.so $(OBJ_DIR)/eval_io_faults.o $(OBJ_DIR)/eval_clock_test.o\n\t$(CC) $(CFLAGS) -o tests/focused_eval_cleanup tests/focused_eval_cleanup.c $(filter-out $(OBJ_DIR)/eval.o $(OBJ_DIR)/eval/eval_io.o,$(COMMON_OBJECTS)) $(OBJ_DIR)/eval_clock_test.o $(OBJ_DIR)/eval_io_faults.o $(RUNTIME_OBJECTS) $(LDFLAGS)\n\t./tests/focused_eval_cleanup\n')
  source_names += ['tests/focused_eval_cleanup.c','focused-eval.mk']
  ...
  args=[make,'-f','focused-eval.mk','-j2','CC='+compiler,'CFLAGS='+shlex.join(flags),'LDFLAGS='+shlex.join(ld),'PYTHON_WITH_YAML='+sys.executable]
  run(name+'-build',[*args,'stage1'],root,out,1200)
  run(name+'-eval',[*args,'focused-eval'],root,out,1200)
