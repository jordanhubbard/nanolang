...
run('normal',py+['tests.test_file_runtime_frames'],dict(base,NANO_FILE_RUNTIME_CC=os.environ['CC'],NANO_FILE_RUNTIME_SANITIZERS='0'))
run('sanitizer',py+[os.environ.get('CARRIER_RUNTIME_SAN_TEST','tests.test_file_runtime_frames')],dict(base,NANO_FILE_RUNTIME_CC=os.environ['CARRIER_SAN_CC'],NANO_FILE_RUNTIME_SANITIZERS='1'))
if not darwin:run('clang',py+['tests.test_file_runtime_frames'],dict(base,NANO_FILE_RUNTIME_CC=shutil.which('clang'),NANO_FILE_RUNTIME_CFLAGS='--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13',NANO_FILE_RUNTIME_SANITIZERS='1'))
