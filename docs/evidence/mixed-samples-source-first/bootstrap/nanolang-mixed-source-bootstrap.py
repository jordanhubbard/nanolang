...
root=pathlib.Path('/home/jkh/Src/nanolang-mixed-samples-source-integrated'); out=pathlib.Path('/tmp/nanolang-mixed-source-bootstrap-5ccfe04b');out.mkdir(exist_ok=False)
def git(*a):return subprocess.check_output(['git',*a],cwd=root,text=True).strip()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
pin=git('rev-parse','HEAD');assert pin=='5ccfe04b6c0af9145a78523742e03499feb99a47' and not git('status','--porcelain','--untracked-files=no')
