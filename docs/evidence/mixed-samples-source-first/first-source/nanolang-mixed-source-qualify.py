...
root=pathlib.Path('/home/jkh/Src/nanolang-mixed-samples-source-integrated');out=pathlib.Path('/tmp/nanolang-mixed-source-qualified-b92753166');out.mkdir(exist_ok=False)
def git(*a):return subprocess.check_output(['git',*a],cwd=root,text=True).strip()
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
pin=git('rev-parse','HEAD');assert pin=='b92753166931a5ba75030d629b9abc762fb9c94f' and not git('status','--porcelain','--untracked-files=no')
...
def inputs():
 ...
 paths += [pathlib.Path('/tmp/nanolang-projection-clang'),pathlib.Path('/usr/bin/gcc').resolve(),pathlib.Path(shutil.which('clang')).resolve(),pathlib.Path(shutil.which('python3')).resolve()]
...
for path in (pathlib.Path(__file__),pathlib.Path('/tmp/nanolang-mixed-source-tests-run.py')):(out/path.name).write_bytes(path.read_bytes())
env=os.environ.copy()
for key in ('NANOC','NANO_VM','NANO_NVM2C','NANO_AOT_RUNTIME','NANO_BUILD_CACHE','NANO_MODULE_PATH'):env.pop(key,None)
env.update(NANO_BUILD_CACHE=str(root/'obj/module_cache'),NANO_VM=str(root/'bin/nano_vm'),NANO_NVM2C=str(root/'bin/nvm2c'),NANO_AOT_RUNTIME=str(root/'bin/nano_aot_runtime.o'),NANO_MODULE_PATH=str(root/'modules'),PYTHONPATH=str(root),CC='/usr/bin/gcc')
cmd=['python3','/tmp/nanolang-mixed-source-tests-run.py',str(out/'tests')];status=1;start=time.monotonic();report={'pin':pin,'command':cmd}
try:
