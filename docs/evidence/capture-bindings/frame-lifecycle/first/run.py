...
if platform.system() == 'Darwin':
    ...
    compilers=[('apple','/usr/bin/clang',False),('brew','/opt/homebrew/opt/llvm/bin/clang',False),('brew-sanitized','/opt/homebrew/opt/llvm/bin/clang',True)]
else:
    compilers=[('gcc','/usr/bin/gcc',False),('clang','/usr/local/bin/clang',False),('gcc-sanitized','/usr/bin/gcc',True),('clang-sanitized','/usr/local/bin/clang',True)]
for name,compiler,sanitized in compilers:
    p=Path(compiler).resolve()
    (REPORT/(name+'-identity.json')).write_text(json.dumps({'path':compiler,'realpath':str(p),'sha256':hashlib.sha256(p.read_bytes()).hexdigest()},indent=2)+'\n')
    command(name+'-version',[compiler,'--version'])
    flags=[]
    if platform.system()=='Darwin': flags += ['-isysroot',sdk]
    elif 'clang' in name: flags += ['--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13']
    if sanitized: flags += ['-fsanitize=address,undefined','-fno-omit-frame-pointer']
    command(name+'-private-syntax',[compiler,*flags,'-std=c99','-Wall','-Wextra','-Werror','-D_GNU_SOURCE','-DNANO_RECORD_ARRAY_PRIVATE_RUNTIME','-Isrc','-fsyntax-only','src/nanovm/vm.c'])
    command(name+'-vm',['/usr/bin/make','-j2','test-nanovm','OBJ_DIR=obj-'+name,'CC='+ ' '.join([compiler,*flags])],timeout=600)
after=inventory()
