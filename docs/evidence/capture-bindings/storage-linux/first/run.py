...
for name, compiler, sanitized in compilers:
    ...
    command(name + '-build', [compiler, *flags, 'tests/nanovm/test_binding_state.c', 'src/nanovm/heap.c',
        'src/nanovm/heap_cycles.c', 'src/nanovm/value.c', '-lm', '-o', str(output)])
    command(name + '-run', [str(output)])
    text = (REPORT / (name + '-run.stdout')).read_text()
    assert 'binding storage checks, including shared cell lifetime and cycle reclamation.' in text
after = inventory()
