...
for name, compiler, sanitized in compilers:
    ...
    command(name + '-build', [compiler, *flags, 'tests/nanoisa/test_capture_bindings.c',
        'src/nanoisa/nvm_v2_cursor.c', 'src/nanoisa/isa.c', '-o', str(output)])
    command(name + '-run', [str(output)])
    text = (REPORT / (name + '-run.stdout')).read_text()
    assert 'reader and writer allocation failures recover without live storage.' in text
command('schema-current', [__import__('sys').executable, 'scripts/gen_nanoisa_schema.py', '--check'])
command('schema-tests', [__import__('sys').executable, '-m', 'unittest', 'discover', '-s', 'tests', '-p', 'test_nanoisa_schema.py'])
after = inventory()
