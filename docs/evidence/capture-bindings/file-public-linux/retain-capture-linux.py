...
for n,p in selected.items():
 entries[n]={'sha256':sha(p),'bytes':p.stat().st_size,'mode':stat.S_IMODE(p.lstat().st_mode),'type':'symlink' if p.is_symlink() else 'file'}
 if p.is_symlink():entries[n]['linkname']=str(p.readlink())
...
verifier=Path('/home/jkh/nanolang-qualification/verify-capture-retention.py')
