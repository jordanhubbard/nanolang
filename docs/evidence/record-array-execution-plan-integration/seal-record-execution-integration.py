...
for short in ['f093b','b87bb']:
 ...
 for kind in ['source','tools']:
  phase='complete-declarations' if short=='b87bb' else 'ordinary';p=root/(phase+'-'+kind+'-after.json');rows=json.loads(p.read_text());n=0
 ...
 p=root/(('complete-declarations' if short=='b87bb' else 'ordinary')+'-artifacts.json');rows=json.loads(p.read_text())
...
summary={'host':host,'reports':len(index),'objects':len(store),'bytes':sum(v['bytes'] for v in store.values()),'references':count,'equal_pairs':len(pairs),'pairs':pairs,'terminals':terms,'scope':'post-PR932 copied plan ordinary integration only; firstf093 stale92-vs93 Python output predicate retained; correctedb87bb no production delta'}
