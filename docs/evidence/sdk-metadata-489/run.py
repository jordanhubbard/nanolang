...
save(base/'identity.json',dict(pin='489e45c7330ab2fe2263a06c5672d31c0e8360e6',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  try:
   run(name+'-metadata',[*args,'test-module-sdk-abi'],root,out,1200)
  finally:
  ...
  save(out/'owning-products.json',{str(p.relative_to(out)):digest(p) for p in tmp.rglob('owned') if p.is_file()})
