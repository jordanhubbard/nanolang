...
commands=subprocess.check_output(['make','-n','test-owned-scalar-global-runtime'],text=True).splitlines()
recipe=next(shlex.split(x) for x in commands if ' -o obj/test_owned_scalar_global_runtime ' in x)
...
with tempfile.TemporaryDirectory(prefix='pr522-runtime-instrument-') as d:
 ...
 for line in commands:
  if not line.startswith('cc ') or ' -o obj/test_owned_' not in line:continue
  ...
  if original.endswith('_alloc.o'):
   replacements[original]=args[out];continue
  target=Path(args[out]);run=[str(target)]
  if target.name=='test_owned_scalar_global_runtime':
