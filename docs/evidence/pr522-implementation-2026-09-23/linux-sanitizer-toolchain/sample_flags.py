...
work=Path('/work/profile-flags');work.mkdir(exist_ok=True)
...
with (work/'shadow-sampled.log').open('w') as log:
 ...
 if child:
  for i in range(3):
   ...
   result=subprocess.run(['gdb','-q','-batch','-ex','set pagination off','-ex',f'attach {child}','-ex','bt 12','-ex','frame 0','-ex','info args','-ex','info locals','-ex','p *this','-ex','detach'],capture_output=True,text=True,timeout=10,env={**env,'DEBUGINFOD_URLS':''})
