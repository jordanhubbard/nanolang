...
work=Path('/work/profile');work.mkdir(exist_ok=True)
...
with (work/'shadow-sampled.log').open('w') as log:
 ...
 if child:
  for i in range(30):
   ...
   result=subprocess.run(['gdb','-q','-batch','-ex','set pagination off','-ex',f'attach {child}','-ex','bt 24','-ex','detach'],capture_output=True,text=True,timeout=10,env={**env,'DEBUGINFOD_URLS':''})
