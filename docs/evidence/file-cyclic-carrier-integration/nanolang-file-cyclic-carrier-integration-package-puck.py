...
roots={'darwin':Path('/tmp/nanolang-file-cyclic-carrier-5c71-puck')}
base=Path('/tmp/nanolang-file-cyclic-carrier-integration-puck-package');base.mkdir(exist_ok=False);reports=base/'reports';reports.mkdir();objects=base/'artifacts';objects.mkdir()
h=lambda p:hashlib.sha256(p.read_bytes()).hexdigest();index={};references=0
for group,root in roots.items():
 results=json.loads((root/'results.json').read_text());assert len(results)==6 and all(x['status']==0 for x in results)
 ...
 for p in root.glob('*-inputs-before.json'):
  for d in json.loads(p.read_text()).values():
   ...
   if sha not in index:shutil.copyfile(source,objects/sha);index[sha]={'bytes':source.stat().st_size}
current=[]
for group,root,tree,phase in [('darwin',roots['darwin'],Path('/tmp/nanolang-file-cyclic-carrier-5c71'),'ordinary')]:
...
for name,source in [('reports',reports),('artifacts',objects)]:
 archive=Path('/tmp/nanolang-file-cyclic-carrier-integration-puck-'+name+'.tar.gz')
...
Path('/tmp/nanolang-file-cyclic-carrier-integration-puck-packaging.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary,indent=2))
