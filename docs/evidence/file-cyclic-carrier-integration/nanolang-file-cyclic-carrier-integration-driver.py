...
def tools():
 paths={n:shutil.which(n) for n in ('make','python3','ar','nm','ld','cc','gcc','clang')}
...
carrier-config:
\t@printf '%s\\n' 'ISA=$(NANOISA_OBJECTS) $(NANOISA_UTF8)' 'ALL=$(NANOISA_OBJECTS) $(NANOVM_OBJECTS) $(COMMON_OBJECTS) $(RUNTIME_OBJECTS) $(OBJ_DIR)/nsi.o' 'LDFLAGS=$(LDFLAGS)' 'PUBLIC=$(FILE_PUBLIC_OBJECTS)' 'OPCODES=$(OBJ_DIR)/nanovirt/wrapper_gen.o $(OBJ_DIR)/nanoisa/nvm2llvm.o $(NANOISA_OBJECTS) $(NANOVM_OBJECTS) $(COMMON_OBJECTS) $(RUNTIME_OBJECTS) $(OBJ_DIR)/nsi.o'
''')
...
run('carrier',py+['tests.test_file_cyclic_runtime'],dict(FILE_RUNTIME_OBJECTS=values['ALL'],FILE_RUNTIME_LDFLAGS=values['LDFLAGS'],NANO_FILE_RUNTIME_CC=os.environ['CC'],NANO_FILE_RUNTIME_CFLAGS=normal_flags,NANO_FILE_RUNTIME_SANITIZERS='0'))
run('indirect',py+['tests.test_file_indirect_targets'],dict(FILE_INDIRECT_TARGETS_OBJECTS=filtered,FILE_INDIRECT_TARGETS_LDFLAGS=values['LDFLAGS'],NANO_FILE_INDIRECT_TARGETS_CC=os.environ['CC'],NANO_FILE_INDIRECT_TARGETS_CFLAGS=normal_flags))
base={'FILE_CYCLIC_HOSTED_OBJECTS':values['ISA'],'FILE_CYCLIC_HOSTED_LDFLAGS':values['LDFLAGS']}
run('ordinary',py+['tests.test_file_cyclic_hosted'],dict(base,NANO_FILE_CYCLIC_HOSTED_CC=os.environ['CC'],NANO_FILE_CYCLIC_HOSTED_CFLAGS=normal_flags,NANO_FILE_CYCLIC_HOSTED_SANITIZERS='0'))
...
run('archive-boundary',py+['tests.test_file_cyclic_hosted_integration'],dict(base,FILE_CYCLIC_HOSTED_OBJECTS=shlex.join(linked),FILE_CYCLIC_HOSTED_ARCHIVE=str(archive),NANO_FILE_CYCLIC_HOSTED_CC=os.environ['CC'],NANO_FILE_CYCLIC_HOSTED_CFLAGS=normal_flags))
