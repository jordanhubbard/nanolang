...
save(base/'identity.json',dict(pin='29a3b7c4ab18a54fb6368610bcdea155b60a325c',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  if name=='ordinary':
   prior_base=pathlib.Path(sys.argv[3]).resolve();prior=prior_base/'ordinary/source'
   original=json.loads((prior_base/'ordinary/source-before.json').read_text())
   assert original==json.loads((prior_base/'ordinary/source-after-failure.json').read_text())
   assert {n:file_identity(prior/n) for n in original}==original
   assert json.loads((prior_base/'identity.json').read_text())['tools']==json.loads((base/'identity.json').read_text())['tools']
   before=mapping(prior);shutil.copytree(prior,root);assert mapping(root)==before
   save(out/'copied-providers.json',before)
  else:root.mkdir()
  ...
  if name=='ordinary':
   changes=[n for n,h in before.items() if file_identity(root/n)!=h]
   assert set(changes)=={'docs/CANONICAL_SDK_TYPED_PROVIDER_CONTRACT.md','docs/ROADMAP.md','tests/test_native_byte_array_identity.py'},changes
   assert mapping(prior)==before;save(out/'reuse-changes.json',changes)
  ...
  if name!='ordinary':run(name+'-build',[*args,'nvm2c','nanoisa_dump','nano_vm','nano_virt'],root,out,1200)
  if name!='ordinary':run(name+'-shapes',[*args,'test-nvm2c-shapes'],root,out,240)
