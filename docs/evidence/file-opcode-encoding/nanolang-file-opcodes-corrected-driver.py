...
def sources():return {p:digest(p) for p in (Path('/tmp/nanolang-file-opcodes-tracked.txt').read_text().splitlines() if not Path('.git').exists() else subprocess.check_output(['git','ls-files','-z']).decode().split('\0')) if p and Path(p).is_file()}
def tools():
...
steps=[('focused',['make','-j4','test-file-opcodes']),('adjacent',['make','test-nanoisa','test-disasm-roundtrip','test-nvm-v2-convert','test-service-module','test-file-nominal-module']),('wrapper',['make','-j4','test-wrapper-gen'])]
results=[]
