...
def sources():return {p:digest(p) for p in (Path('/tmp/nanolang-file-opcodes-ff5-tracked.txt').read_text().splitlines() if not Path('.git').exists() else subprocess.check_output(['git','ls-files','-z']).decode().split('\0')) if p and Path(p).is_file()}
def tools():
...
write('source-before.json',sources());write('tools-before.json',tools());write('selected-environment.json',{k:v for k,v in os.environ.items() if k in ['CC','SDKROOT','LIBRARY_PATH','NANO_FILE_OPCODE_TEST_CC','NANO_SERVICE_MODULE_TEST_CC','NMS_NATIVE_CLANG_FLAGS']})
steps=[('adjacent',['make','test-nanoisa','test-disasm-roundtrip','test-nvm-v2-convert','test-service-module','test-file-nominal-module']),('wrapper',['make','-j4','test-wrapper-gen'])]
results=[]
