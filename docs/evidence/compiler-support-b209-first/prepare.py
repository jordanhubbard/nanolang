...
save(base/'identity.json',dict(pin='b2094af7e',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitizer',sancc,True)]:
  ...
  run('fresh-providers',[*args,'bin/nanoc_c','nano_vm','nvm2c','nanoisa_dump','nvm2c-runtime'],root,out,1800)
  ...
  run('actual-and-borrowed-provider',[sys.executable,'-m','unittest','-f','-v','tests.test_compiler_support_artifact_adapters'],root,out,1200)
 ...
 save(base/'terminal.json',dict(status=0,scope='focused exact artifact contracts and actual/synthetic provider lifetimes only; full bootstrap and original suites pending'))
