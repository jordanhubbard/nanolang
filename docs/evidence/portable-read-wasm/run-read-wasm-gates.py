...
for name in phases:
 ...
 argv=[sys.executable,'-m','unittest','-f','-v','tests.test_portable_read_wasm'];write(name+'-command.json',{'argv':argv,'cwd':str(root),'env':{k:v for k,v in env.items() if k.startswith('PORTABLE_') or k in ['SDKROOT','ASAN_OPTIONS','UBSAN_OPTIONS','LSAN_OPTIONS']}})
