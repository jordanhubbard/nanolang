...
extra={'cc':cc,'clang':clang,'python':sys.executable,'make':'/usr/bin/make','ld':shutil.which('ld'),'ar':shutil.which('ar'),'pkg-config':shutil.which('pkg-config')}
if darwin:
...
h.dump('configuration.json',{'tools':extra,'env':{k:v for k,v in h.env.items() if k.startswith('PORTABLE_') or k in ['CC','SDKROOT','ASAN_OPTIONS','UBSAN_OPTIONS','LSAN_OPTIONS']},'scope':'ordinary native adapter and query; GCC/HB ASan UBSan LSan native core/module controls only; no installed CLI/package claim'})
phases=[]
def phase(name,fn):
