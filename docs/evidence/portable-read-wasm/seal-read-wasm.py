...
repo=pathlib.Path('/home/jkh/Src/nanolang-portable-read-wasm');dest=repo/'docs/evidence/portable-read-wasm';dest.mkdir(exist_ok=False);store=pathlib.Path('/tmp/nanolang-read-wasm-artifacts');store.mkdir(exist_ok=False)
remote=pathlib.Path('/tmp/nanolang-read-wasm-puck-downloaded');remote.mkdir(exist_ok=False)
with tarfile.open('/tmp/nanolang-read-wasm-puck-seal.tar.gz') as t:t.extractall(remote,filter='data')
def sha(p):
 h=hashlib.sha256()
 ...
 return h.hexdigest()
art={};summary={}
for host,root in [('linux',pathlib.Path('/tmp/nanolang-read-wasm-linux-seal')),('puck',remote/'nanolang-read-wasm-puck-seal')]:
...
for p in ['/tmp/run-read-wasm-gates.py','/tmp/run-wasm-read-neighbors.py','/tmp/run-wasm-read-neighbors-puck.py','/tmp/supervise-read-wasm-neighbors.py','/tmp/collect-read-wasm-seal.py','/tmp/seal-read-wasm.py']:shutil.copyfile(p,dest/pathlib.Path(p).name)
archive=pathlib.Path('/tmp/nanolang-read-wasm-puck-seal.tar.gz')
