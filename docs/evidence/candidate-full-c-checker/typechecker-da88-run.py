...
save(base/'identity.json',dict(pin='da88d2003264eb989fc9972913842e28e40cdcb3',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  try:
   ...
   run(name+'-checker-build',[compiler,*flags,'-o',str(exe),'tests/test_typechecker.c',*[str(p) for p in providers],*ld],root,out,1200)
