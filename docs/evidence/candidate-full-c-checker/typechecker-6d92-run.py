...
save(base/'identity.json',dict(pin='6d92a2d53cb330117731ab310e97b6a088731ccf',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  try:
   exe=out/'test_typechecker'
   run(name+'-checker-build',[compiler,*shlex.split((out/'cflags.txt').read_text()),'-o',str(exe),'tests/test_typechecker.c',*[str(p) for p in providers],*shlex.split((out/'ldflags.txt').read_text())],root,out,1200)
   product_before=digest(exe);save(out/'checker-product-before.json',{'path':str(exe),'sha256':product_before})
   try:
    run(name+'-checker',[str(exe)],root,out,180)
   finally:
    product_after=digest(exe);save(out/'checker-product-after.json',{'path':str(exe),'sha256':product_after})
