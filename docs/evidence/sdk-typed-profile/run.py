...
save(base/'identity.json',dict(pin='8954a54c6538c0ac750392866790e5a2c6cf0477',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
