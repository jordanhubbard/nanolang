...
root=pathlib.Path('/private/tmp/nanolang-mixed-source-70c511dad'); out=pathlib.Path('/private/tmp/nanolang-mixed-source-darwin-bootstrap-70c511dad');out.mkdir(exist_ok=False)
...
(out/'setup.mk').write_bytes(pathlib.Path('/private/tmp/nanolang-mixed-source-darwin-setup.mk').read_bytes())
env=os.environ.copy()
env.update(CC='/usr/bin/clang',SDKROOT=subprocess.check_output(['/usr/bin/xcrun','--show-sdk-path'],text=True).strip())
(out/'environment.json').write_text(json.dumps({'CC':env['CC'],'SDKROOT':env['SDKROOT'],'PATH':env.get('PATH')},indent=2)+'\n')
...
steps=[('bootstrap',['make','-j2','CC=/usr/bin/clang','bootstrap']),('setup',['make','-j2','CC=/usr/bin/clang','-f','Makefile.gnu','-f',str(out/'setup.mk'),'transitive-wrapper-setup','bin/nano'])]
