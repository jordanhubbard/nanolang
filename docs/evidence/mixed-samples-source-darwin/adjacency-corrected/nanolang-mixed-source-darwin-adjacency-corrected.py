...
root=pathlib.Path('/private/tmp/nanolang-mixed-source-adjacency-70c511dad');out=pathlib.Path('/private/tmp/nanolang-mixed-source-darwin-adjacency-corrected-70c511dad')
...
commands=[('owned-regressions',['make','-j2','CC=/opt/homebrew/opt/llvm/bin/clang','test-owned-value-graphs','test-owned-value-results']),('service',['make','-j2','CC=/opt/homebrew/opt/llvm/bin/clang','test-service-module'])]
report={'pin':git('rev-parse','HEAD'),'steps':[],'compiler':{'path':'/opt/homebrew/opt/llvm/bin/clang','resolved':str(pathlib.Path('/opt/homebrew/opt/llvm/bin/clang').resolve()),'sha256':sha(pathlib.Path('/opt/homebrew/opt/llvm/bin/clang'))}};status=1
try:
