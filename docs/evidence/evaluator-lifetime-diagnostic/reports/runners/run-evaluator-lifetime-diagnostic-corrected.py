...
out=pathlib.Path('/tmp/nanolang-evaluator-lifetime-4abf-linux');out.mkdir(exist_ok=False)
toolroot=out/'compiler-root';toolroot.mkdir();(toolroot/'bin').mkdir()
for name in ('src','src_nano','modules','stdlib','include','lib'):
...
assert subprocess.check_output(['git','rev-parse','HEAD'],cwd=source,text=True).strip()=='4abfa9dae9321d1c63d7ae6cabd163d3548f4620'
