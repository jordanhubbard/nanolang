...
out=pathlib.Path('/tmp/nanolang-evaluator-lifetime-c1df-linux');out.mkdir(exist_ok=False)
...
assert subprocess.check_output(['git','rev-parse','HEAD'],cwd=source,text=True).strip()=='c1df52326b185334a17c09730428ab8ca349dfd8'
...
env=dict(os.environ,CC='/bin/gcc',NANO_BUILD_CACHE=str(out/'module-cache'),NANO_MODULE_PATH=str(root/'modules'),ASAN_OPTIONS='detect_leaks=1:halt_on_error=1',LSAN_OPTIONS='',UBSAN_OPTIONS='halt_on_error=1:print_stacktrace=1')
for k in ('LD_PRELOAD','DYLD_INSERT_LIBRARIES','NANOC','NANOLANG_COMPILER'):env.pop(k,None)
...
linked=[out/'env.o' if p=='obj/env.o' else out/'eval.o' if p=='obj/eval.o' else root/p for p in objects]
compiler=out/'nanoc-diagnostic'
...
result=command('full-parser',[compiler,root/'src_nano/parser_driver.nano','-o',out/'parser'],root,120)
...
assert after==json.loads((out/'inputs-before.json').read_text())
