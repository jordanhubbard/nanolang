...
env={'NANO_SERVICE_MODULE_TEST_CC':selected,'OWNED_ARRAY_AUTHORITY_ARTIFACTS':str(out/'authority-fixture'),'OWNED_ARRAY_ORIGIN_ARTIFACTS':str(out/'origin-fixture')}
if platform=='darwin':env['LIBRARY_PATH']='/opt/homebrew/opt/openssl@3/lib'
steps=[('setup',['make','-j8','nano_virt','nano_vm','nvm2c','nvm2llvm','nvm2hl'],{}),('integrated',['make','test-file-nominal-module','test-owned-array-origins','test-owned-array-authority'],env)]
try:
 for name,cmd,env in steps:
  ...
  paths=list((root/'bin').glob('*'))+list((root/'obj').glob('test_*'))+list((out/'authority-fixture').rglob('*'))+list((out/'origin-fixture').rglob('*'))
