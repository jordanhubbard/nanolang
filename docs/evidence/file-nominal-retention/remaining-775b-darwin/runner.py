...
steps=[('remaining',['make','test-owned-array-layouts','test-owned-array-origins'],{'LIBRARY_PATH':'/opt/homebrew/opt/openssl@3/lib','OWNED_ARRAY_LAYOUT_ARTIFACTS':str(out/'layout-fixture'),'OWNED_ARRAY_ORIGIN_ARTIFACTS':str(out/'origin-fixture')})]
try:
 for name,cmd,env in steps:
  ...
  paths=list((root/'bin').glob('*'))+list((root/'obj').glob('test_*'))+list((out/'layout-fixture').rglob('*'))+list((out/'origin-fixture').rglob('*'))
