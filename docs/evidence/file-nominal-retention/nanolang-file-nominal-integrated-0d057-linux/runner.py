...
if platform=='darwin':
 ...
 baseenv['NANO_SERVICE_MODULE_TEST_CC']='/opt/homebrew/opt/llvm/bin/clang'
 baseenv['NANO_FILE_NOMINAL_CC']='/opt/homebrew/opt/llvm/bin/clang'
 tools.append(pathlib.Path('/opt/homebrew/opt/openssl@3/lib/libcrypto.3.dylib').resolve())
...
selected = '/opt/homebrew/opt/llvm/bin/clang' if platform=='darwin' else '/bin/gcc'
env={'NANO_SERVICE_MODULE_TEST_CC':selected,'OWNED_ARRAY_AUTHORITY_ARTIFACTS':str(out/'authority-fixture')}
if platform=='darwin':env['LIBRARY_PATH']='/opt/homebrew/opt/openssl@3/lib'
steps=[('setup',['make','-j8','nano_virt','nano_vm','nvm2c','nvm2llvm','nvm2hl'],{}),('integrated',['make','test-file-nominal-module','test-service-module','test-owned-array-authority','test-wrapper-gen'],env)]
try:
 for name,cmd,env in steps:
  ...
  paths=list((root/'bin').glob('*'))+list((root/'obj').glob('test_*'))+list((out/'authority-fixture').rglob('*'))
