...
save(base/'identity.json',dict(pin='f2866596f8ece4a61df58ab559bac5ebe2c50795',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  try:
   run(name+'-owning-method',[*args,'test-evaluator-owned-lifetimes'],root,out,1200)
  finally:
