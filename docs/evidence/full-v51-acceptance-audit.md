...
| 6 | I implement the required LLVM and WebAssembly translators, then run one pinned module corpus on VM, AOT C and every shipped translator. AST PTX/OpenCL product routes must become translators or leave the product. | The user-selected full roadmap makes the unconditional Phase 20 LLVM/Wasm implementation rows required for v5.1. I reconciled the architecture table and acceptance wording accordingly; generic architectural optionality does not narrow this release. GPU/JVM evaluation is required; shipping every evaluated target is not. |
