...
base=Path('/home/jkh/nanolang-qualification');out=Path('/home/jkh/Src/nanolang-evaluator-dynamic-child-owners/docs/evidence/collection-22fb');out.mkdir(parents=True,exist_ok=True)
def sha(p):return hashlib.file_digest(Path(p).open('rb'),'sha256').hexdigest()
scope=json.loads((base/'collection-22fb-drivers/scope.json').read_text());expected={p:v['sha256'] for p,v in scope['files'].items()}
seal={'source':scope['pin'],'lanes':{},'objects':{}}
for label,r,status in [('linux-ordinary',base/'collection-22fb-ordinary',0),('darwin-ordinary',base/'collection-22fb-puck-reports/collection-22fb-ordinary',0),('linux-sanitizer',base/'collection-22fb-sanitizer',0),('darwin-sanitizer',base/'collection-22fb-puck-reports/collection-22fb-sanitizer',0)]:
 objects={}
 ...
 results=json.loads((r/'results.json').read_text());assert len(results)==2 and all(x['status']==status for x in results)
 assert 'All typechecker tests passed!' in (r/'test-typechecker.log').read_text()
 log=(r/'test-evaluator-collection-ownership.log').read_text();assert 'I retain dynamic string and record children across mutation, callbacks, arithmetic and completed tasks.' in log;assert 'I passed evaluator collection ownership controls.' in log
