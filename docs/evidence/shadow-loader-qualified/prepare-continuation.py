...
save(base/'identity.json',dict(pin='bafadf615',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
def verify_linked_terminal():
 ...
 fixture=root/'obj/test_shadow_loader_fork_failure'
 if fixture.is_file():
  retained=out/'retained';retained.mkdir(exist_ok=True)
  shutil.copy2(fixture,retained/fixture.name)
  save(out/'fork-failure-executable.json',dict(path=str(fixture),sha256=digest(fixture),retained_sha256=digest(retained/fixture.name)))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitizer',sancc,True)]:
  ...
  prior=pathlib.Path(sys.argv[3]).resolve()/name;oldroot=prior/'source'
  baseline=json.loads((prior/'source-before.json').read_text())
  assert {n:digest(oldroot/n) for n in baseline}==baseline
  differences=sorted(n for n in set(sources)|set(baseline) if sources.get(n)!=baseline.get(n))
  assert differences and all(n.startswith('docs/') or n=='tests/test_callback_shadows.py' for n in differences),differences
  oldphase=json.loads((prior/'fresh-providers.json').read_text())
  assert oldphase['returncode']==0 and oldphase['argv']==[*args,'bin/nanoc_c','nano_virt','nano_vm','nvm2c','nanoisa_dump','nvm2c-runtime']
  oldtools=json.loads((prior.parent/'tools-after-terminal.json').read_text())
  assert oldtools['unchanged'] and oldtools['tools']==json.loads((base/'identity.json').read_text())['tools']
  oldlinks=json.loads((prior/'linked-providers-after-terminal.json').read_text())
  assert not oldlinks['missing'] and not oldlinks['changed']
  original_products={}
  for folder in ['bin','obj','lib']:
   if not (oldroot/folder).exists():continue
   closure=mapping(oldroot/folder);original_products[folder]=closure
   links=symlink_map(oldroot/folder)
   shutil.copytree(oldroot/folder,root/folder,copy_function=shutil.copy2,symlinks=True)
   assert symlink_map(root/folder)==links and symlink_map(oldroot/folder)==links
   copied_links[folder]=links
   assert mapping(root/folder)==closure and mapping(oldroot/folder)==closure
  for n,h in sources.items():
   if baseline.get(n)==h:
    st=(oldroot/n).stat();os.utime(root/n,ns=(st.st_atime_ns,st.st_mtime_ns))
  save(out/'product-symlinks-before.json',copied_links)
  linked_before=linked_closure(root)
  assert all(linked_before.get(n)==h for n,h in oldlinks['files'].items())
  copied_extras=sorted(set(linked_before)-set(oldlinks['files']))
  save(out/'copied-generated-providers.json',{n:linked_before[n] for n in copied_extras})
  save(out/'linked-providers-before.json',linked_before)
  save(out/'reuse-proof.json',dict(prior=str(prior),differences=differences,products=original_products,prior_argv=oldphase['argv']))
  ...
  run('callback-and-interpreter-shadows',[sys.executable,'-m','unittest','-f','-v','tests.test_callback_shadows'],root,out,1200)
 ...
 save(base/'terminal.json',dict(status=0,scope='complete six-method callback suite with observable sanitizer refusals and retained compiler streams; independently copied and hash-verified2039 products; full original typecheck separate'))
