...
if platform=='darwin':
 ...
 settings=pathlib.Path(sdk)/'SDKSettings.json'
 write('sdk.json',{'path':sdk,'version':subprocess.check_output(['xcrun','--sdk','macosx','--show-sdk-version'],text=True).strip(),'settings_sha256':sha(settings),'xcrun_sha256':sha(pathlib.Path('/usr/bin/xcrun'))})
