...
summary={'roots':list(map(str,roots)),'artifacts':len(store),'artifact_bytes':sum(v['bytes'] for v in store.values()),'references':refs,'pairs':pairs,'terminals':terminals,'scope':'focused VM and exact observed service predicate plus modeled hooks; separate unchanged actual host/ownership neighbors and original emitter; statuses in terminals'}
