...
save(base/'identity.json',dict(pin='435c8e72f6dce5c8f3f9d7d3bf0e7d1ed5959d0f',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  try:
   run(name+'-declarations',[*args,'test-ownership-declaration-projection'],root,out,1200)
  finally:
  ...
  fixture=out/'codec'
  run(name+'-codec-build',[compiler,*[('-std=c11' if f=='-std=c99' else f) for f in flags],'tests/test_sdk_provider_codec.c','-o',str(fixture),*ld],root,out,60)
  run(name+'-codec',[str(fixture)],root,out,60)
