...
source.extend([pathlib.Path('/tmp/run_owned_array_authority_033.py'),pathlib.Path('/tmp/test_owned_array_authority_033.py')])
...
import re
old_log=pathlib.Path('/tmp/nanolang-affine-variants-660-puck-neighbors/owner-authority.log').read_text()
match=re.search(r'OWNED_ARRAY_AUTHORITY_LINK_OBJECTS="([^"]+)"',old_log);assert match
owner_objects=shlex.split(match.group(1));assert all((root/p).is_file() for p in owner_objects)
phase('owner-authority',[python,'/tmp/run_owned_array_authority_033.py','/tmp/test_owned_array_authority_033.py',root],dict(baseenv,OWNED_ARRAY_AUTHORITY_LINK_OBJECTS=shlex.join(owner_objects),OWNED_ARRAY_AUTHORITY_LDFLAGS=shlex.join(libs),OWNED_ARRAY_AUTHORITY_ARTIFACTS=str(evidence/'products'/'owner-authority')))
...
dump('terminal.json',{'status':'PASS','scope':'Corrected033 owner authority then previously unreached mixed lifecycle, owner ARRAY, contracts, affine, emitter; actual callback/FFI and mixed admission/query remain original660 passes'})
