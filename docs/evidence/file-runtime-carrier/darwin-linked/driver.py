...
def inventory(extra=()):
 ...
 for p in paths:
  ...
  if p.is_relative_to(root) and p.suffix not in ('.o','.a','.so','.dylib','.ll','.bc') and not os.access(p,os.X_OK):continue
  digest=sha(p);dest=artifacts/digest
  if not dest.exists():shutil.copyfile(p,dest)
  entries[str(p)]={'sha256':digest,'artifact':str(dest)}
 return entries
write('environment.json',{k:v for k,v in os.environ.items() if k in ('CC','CARRIER_SAN_CC','SDKROOT','LIBRARY_PATH','NMS_NATIVE_CLANG_FLAGS','PATH','CARRIER_PHASES','NANO_FILE_RUNTIME_CFLAGS','CARRIER_EXTRA_TOOLS')})
...
def run(label,cmd,extra=None):
 ...
 write(label+'-command.json',{'argv':cmd,'selectors':{k:v for k,v in env.items() if k.startswith(('NANO_','FILE_','WRAPPER_')) or k in ('CC','SDKROOT','LIBRARY_PATH','NMS_NATIVE_CLANG_FLAGS')}})
...
carrier-providers: $(NANOISA_OBJECTS) $(NANOVM_OBJECTS) $(COMMON_OBJECTS) $(RUNTIME_OBJECTS) $(OBJ_DIR)/nsi.o $(OBJ_DIR)/nanovirt/wrapper_gen.o
carrier-config:
...
''')
run('setup',['make','-f','Makefile.gnu','-f',str(probe),'-j4','CC='+os.environ['CC'],'carrier-providers','nvm2c','nvm2llvm','nvm2hl','nano_virt'])
...
run('normal',py+['tests.test_file_runtime'],dict(base,NANO_FILE_RUNTIME_CC=os.environ['CC'],NANO_FILE_RUNTIME_SANITIZERS='0'))
run('sanitizer',py+[os.environ.get('CARRIER_RUNTIME_SAN_TEST','tests.test_file_runtime')],dict(base,NANO_FILE_RUNTIME_CC=os.environ['CARRIER_SAN_CC'],NANO_FILE_RUNTIME_SANITIZERS='1'))
if not darwin:run('clang',py+['tests.test_file_runtime'],dict(base,NANO_FILE_RUNTIME_CC=shutil.which('clang'),NANO_FILE_RUNTIME_CFLAGS='--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13',NANO_FILE_RUNTIME_SANITIZERS='1'))
...
run('wrapper-provider',['make','-f','Makefile.gnu','CC='+os.environ['CC'],'obj/nanovm/vmd_protocol.o','obj/nanovm/vmd_client.o'])
