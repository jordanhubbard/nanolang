...
probe=report/'passive-comparison.mk'
...
run('comparison-build',['make','-f','Makefile.gnu','-f',str(probe),'-j2','CC='+os.environ['CC'],'nanoisa_emit','passive-comparator'])
run('comparison',[sys.executable,'-m','unittest','-f','-v','tests.test_nanoisa_flat_records.FlatRecordEmitter.test_passive_flow_bytecode_and_source_order_metadata_match','tests.test_nanoisa_flat_records.FlatRecordEmitter.test_passive_par_records_and_scalar_execution'])
run('scoped-sanitizer',['make','-f','Makefile.gnu','-f',str(probe),'CC='+os.environ['CARRIER_SAN_CC'],'passive-scoped-sanitizer'])

run('float-casts',[sys.executable,'-m','unittest','-f','-v','tests.test_native_floats.NativeFloats.test_cast_float_matches_vm_scalar_conversions','tests.test_native_floats.NativeFloats.test_cast_float_through_polymorphic_calls_and_local_loop'],{'NANO_NATIVE_TEST_CC':os.environ['CARRIER_SAN_CC']})
