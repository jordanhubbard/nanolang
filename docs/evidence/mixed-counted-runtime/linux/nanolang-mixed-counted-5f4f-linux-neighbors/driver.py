...
run('setup',['make','-f','Makefile.gnu','-f',str(probe),'-j2','CC='+os.environ['CC'],'array-providers','nanoisa_dump','nano_vm','nvm2llvm','nvm2wasm'])
...
for name in ('managed_records','managed_record_adapters','managed_array_copy_runtime','managed_array_graphs','managed_string_arrays'):
 run('neighbor-'+name,[sys.executable,str(neighbor),str(root),str(report/(name+'-products')),'tests.test_'+name+('.RecordAdapters.test_private_adapters_native_and_wasm' if name=='managed_record_adapters' else '')],dict(CC=os.environ.get('MC_NEIGHBOR_CC',clang),NMS_RUNTIME_CLANG=clang))
