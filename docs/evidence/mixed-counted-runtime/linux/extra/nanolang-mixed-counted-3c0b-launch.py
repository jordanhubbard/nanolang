...
root=Path('/tmp/nanolang-mixed-counted-qualified-3c0b');archive=Path('/tmp/nanolang-mixed-counted-3c0b-source.tar.gz');manifest=json.loads(Path('/tmp/nanolang-mixed-counted-3c0b-source.json').read_text())
...
if darwin:
 env.update(CC='/Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/clang',CARRIER_SAN_CC='/opt/homebrew/opt/llvm/bin/clang',PATH='/opt/homebrew/opt/llvm/bin:/opt/homebrew/opt/lld/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin',LIBRARY_PATH='/opt/homebrew/opt/openssl@3/lib',NMS_RUNTIME_CLANG='/opt/homebrew/opt/llvm/bin/clang',NMS_RUNTIME_OPT='/opt/homebrew/opt/llvm/bin/opt')
...
subprocess.run([sys.executable,'/tmp/nanolang-mixed-counted-driver.py',str(root),'/tmp/nanolang-mixed-counted-3c0b-'+host],check=True,env=env)
