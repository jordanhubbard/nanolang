...
base=Path('/home/jkh/nanolang-qualification');out=Path('/home/jkh/Src/nanolang-array-arithmetic-result-views/docs/evidence/array-views-653');out.mkdir(parents=True,exist_ok=True)
cas=base/'array-views-653-cas';cas.mkdir(exist_ok=True)
def sha(p):return hashlib.file_digest(p.open('rb'),'sha256').hexdigest()
seal={'pin':'653af65401d8fde9253f5671c96ccc785fae340e','lanes':{},'objects':{}}
for host,r in [('linux',base/'array-views-653-linux'),('darwin',base/'array-views-653-puck-copy/array-views-653-puck')]:
 phases=json.loads((r/'phases.json').read_text());assert len(phases)==6
 ...
 for config in ('ordinary','sanitized'):
  ...
  for target in ('test-typechecker','test-eval'):assert providers==json.loads((o/(target+'-providers-after.json')).read_text())
  ...
  executables={}
  for binary,target,marker in [('test_typechecker','test-typechecker','All typechecker tests passed!'),('test_eval','test-eval','All eval tests passed!')]:
  ...
  if host=='darwin' and config=='sanitized':
   for binary in ('test_eval','test_typechecker'):
...
(out/'seal.json').write_text(json.dumps(seal,indent=2)+'\n');shutil.copy2(__file__,out/'seal.py');shutil.copy2(base/'array-views-653-drivers/run.py',out/'run.py');print(seal['totals'])
