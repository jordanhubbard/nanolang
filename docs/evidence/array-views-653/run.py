...
save(base/'identity.json',dict(pin='653af65401d8fde9253f5671c96ccc785fae340e',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
assert shutil.disk_usage(base).free>2*1024**3
scope=json.loads(pathlib.Path(__file__).with_name('scope.json').read_text());assert scope['pin']=='653af65401d8fde9253f5671c96ccc785fae340e'
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  for target,binary in [('test-typechecker','test_typechecker'),('test-eval','test_eval')]:
   ...
   env['FULL_EVAL_BINARY']=str(root/('tests/'+binary));env['FULL_EVAL_RETAIN']=str(out/(binary+'-retained'))
   try:run(name+'-'+target,[*args,target],root,out,1200)
    ...
    if failed.is_file():
     shutil.copy2(failed,out/(binary+'-retained'));save(out/(binary+'-retained.json'),file_identity(failed))
