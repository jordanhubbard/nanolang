...
save(base/'identity.json',dict(pin='88669142f',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitizer',sancc,True)]:
  ...
  prior=base.parent/('cop-opaque-07c-puck' if darwin else 'cop-opaque-07c-linux')/name
  oldroot=prior/'source'
  baseline=json.loads((prior/'source-before.json').read_text())
  assert all((oldroot/n).is_file() and digest(oldroot/n)==h for n,h in baseline.items())
  differences=sorted(n for n in set(sources)|set(baseline) if sources.get(n)!=baseline.get(n))
  assert differences and all(n.startswith('docs/') or n in ('tests/nanovm/test_cop_protocol.c','tests/nanovm/test_vm_ffi.c') for n in differences),differences
  oldphase=json.loads((prior/'fresh-providers.json').read_text())
  assert oldphase['returncode']==0 and oldphase['argv'][:len(args)]==args
  ...
  # Same bytes keep the original source timestamps, preserving Make freshness;
  # the corrected fixture itself is rebuilt by its unchanged phony recipe.
  for n,h in sources.items():
   if baseline.get(n)==h:
    st=(oldroot/n).stat();os.utime(root/n,ns=(st.st_atime_ns,st.st_mtime_ns))
  retained={n:h for folder,items in original_products.items() for rel,h in items.items()
            if folder in ['bin','lib'] or rel.endswith('.o') for n in [folder+'/'+rel]}
  save(out/'reuse-proof.json',dict(prior=str(prior),differences=differences,products=original_products,retained=retained))
  ...
  assert all(digest(root/n)==h for n,h in retained.items())
 ...
 save(base/'terminal.json',dict(status=0,scope='corrected original protocol both configs; remaining original sanitizer fuzz/FFI/lifecycle; exact07cc token provider reuse; no threaded fork or exec acceptance'))
