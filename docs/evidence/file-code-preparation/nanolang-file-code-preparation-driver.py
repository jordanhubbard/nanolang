...
def sources():return {p:digest(p) for p in (Path('/tmp/nanolang-file-code-tracked.txt').read_text().splitlines() if not Path('.git').exists() else subprocess.check_output(['git','ls-files','-z']).decode().split('\0')) if p and Path(p).is_file()}
def tools():
 names=['make','cc','gcc','clang','python3','ar','ld']
 paths={n:shutil.which(n) for n in names}
 paths['sanitizer']=shutil.which(os.environ.get('NANO_FILE_CODE_CC','gcc'))
...
write('source-before.json',sources());write('tools-before.json',tools());write('selected-environment.json',{k:v for k,v in os.environ.items() if k in ['CC','SDKROOT','LIBRARY_PATH','NANO_FILE_CODE_CC','NANO_SERVICE_MODULE_TEST_CC','NMS_NATIVE_CLANG_FLAGS']})
steps=[('normal',['make','-j4','test-file-code']),('sanitizers',['make','test-file-code-sanitizers'])]
if sys.platform!='darwin':steps.append(('clang',['env','NANO_FILE_CODE_CC=clang','NANO_FILE_CODE_CFLAGS=--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13','make','test-file-code-sanitizers']))
steps += [('adjacent',['make','test-file-flow','test-file-nominal','test-service-bindings','test-nanoisa']),('provider',['make','-j4','nano_virt'])]
results=[]
for label,cmd in steps:
 ...
 for directory in re.findall(r'I retain File CODE artifacts at (\S+)',text):
