...
dest=root/'docs/evidence/typecheck-lifetime-729ce';dest.mkdir(parents=True,exist_ok=False)
cas=Path('/tmp/nanolang-typecheck-lifetime-729ce-artifacts');cas.mkdir(exist_ok=False)
source=Path('/tmp/nanolang-typecheck-lifetime-729ce-linux')
manifest=dict(source_pin='729ce12a874b9fa2ba0ad8950b00dd90c34d3a3d',production_pin='40870d07805c85c2a6600c7d02b8440cd547f57a',scope='One private full typecheck_driver diagnostic, original10s shadow deadline and120s outer bound. No production acceptance. Final unmatched interval is unmeasured; recursive view timing is inclusive and nonadditive.',reports={},artifacts={},commands=[])
def report(p,label):
...
for p in ('/tmp/run-typecheck-lifetime-729ce.py',__file__):report(Path(p),'scripts')
