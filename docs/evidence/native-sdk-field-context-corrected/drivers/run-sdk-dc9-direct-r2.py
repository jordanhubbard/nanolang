...
root=pathlib.Path('/tmp/nanolang-native-sdk-dc9-linux-direct-r2-source');root.mkdir()
report=pathlib.Path('/tmp/nanolang-native-sdk-dc9-linux-direct-r2');report.mkdir()
...
for entry in subprocess.check_output(['git','ls-tree','-r',pin],cwd=repo,text=True).splitlines():
 mode,kind,rest=entry.split(' ',2);_,name=rest.split('\t',1);path=root/name
 if path.is_file() and mode!='120000':path.chmod(int(mode,8)&0o7777)
