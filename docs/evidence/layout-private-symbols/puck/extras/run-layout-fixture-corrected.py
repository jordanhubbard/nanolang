...
pin=git('rev-parse','HEAD');assert pin=='e7614728b60b479dd1040bbf89268ed259bd456e'
assert not git('status','--porcelain','--untracked-files=no')
assert (root/'obj').is_dir() and (root/'bin').is_dir()
...
report={'head':pin,'host':platform.node(),'platform':platform.platform(),'scope':'hash-verified ca377 build/bootstrap reuse, corrected layout target and unchanged complete make test; named tools and phase endpoint products retained; deleted inner temporary products are not reconstructed','phases':[]};save('manifest.json',report)
...
for name,args,bound in [('layout',['make','test-nvm-v2-layouts',*flags],600),('full-test',['make','test',*flags],5400)]:
