...
old=pathlib.Path(sys.argv[1]).resolve();new=pathlib.Path(sys.argv[2]).resolve();reports=pathlib.Path(sys.argv[3]).resolve();out=pathlib.Path(sys.argv[4]);out.mkdir(exist_ok=False);store=out/'objects';store.mkdir();shutil.copyfile(__file__,out/'runner.py')
...
for src in sorted(old.rglob('*'), key=lambda p:(p.is_symlink(),str(p))):
