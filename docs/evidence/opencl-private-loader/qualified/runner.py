...
root=pathlib.Path('/home/jkh/Src/nanolang-opencl-private-loader-yaml');out=pathlib.Path('/tmp/nanolang-opencl-private-7f0');out.mkdir(exist_ok=False)
...
head=check(['git','rev-parse','HEAD'],root);assert head.startswith('7f0d01d4c');assert not check(['git','status','--porcelain'],root)
...
yamlroot=out/'yaml';yamlroot.mkdir()
yamlrows=[]
for package_name in ['libyaml-dev','libyaml-0-2']:
 package_file=pathlib.Path('/tmp')/(package_name+'_0.2.5-1build1_arm64.deb')
 metadata=check(['/usr/bin/apt-cache','show',package_name+'=0.2.5-1build1'])
 expected=next(line.split(': ',1)[1] for line in metadata.splitlines() if line.startswith('SHA256: '));assert digest(package_file)==expected
 subprocess.run(['/usr/bin/dpkg-deb','-x',str(package_file),str(yamlroot)],check=True)
 yamlrows.append({'package':str(package_file),'sha256':expected,'metadata':metadata})
save('yaml-packages.json',yamlrows)
psych=out/'psych';shutil.copytree('/home/jkh/Src/RubyOS/build/ruby-build/ruby-4.0.6/ext/psych',psych)
save('psych-source-before.json',filesmap(psych))
toolpaths.extend(str(p) for p in yamlroot.rglob('*') if p.is_file())
env['RUBYLIB']=str(psych/'lib')+':'+str(psych)
save('psych-environment.json',{'RUBYLIB':env['RUBYLIB'],'source':'/home/jkh/Src/RubyOS/build/ruby-build/ruby-4.0.6/ext/psych'})
...
try:
 phase('psych-configure',[str(ruby),'extconf.rb','--with-libyaml-include='+str(yamlroot/'usr/include'),'--with-libyaml-lib='+str(yamlroot/'usr/lib/aarch64-linux-gnu')],psych)
 phase('psych-make',['/usr/bin/make','-j2'],psych)
 save('psych-built-files.json',filesmap(psych))
 phase('psych-dynamic-libraries',['/usr/bin/ldd',str(psych/'psych.so')],psych,120)
 phase('yaml-preflight',[str(ruby),'-e',"require 'yaml'; data={'gate'=>[1,true,'byte']}; abort 'YAML mismatch' unless YAML.safe_load(YAML.dump(data))==data; puts RUBY_DESCRIPTION; puts Psych::VERSION; puts $LOADED_FEATURES.grep(/psych/)"] ,psych,120)
