#include "list_ASTStmtRef_get.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_ASTStmtRef_get definition must be included */
...
static void ensure_capacity_ASTStmtRef_get(List_ASTStmtRef_get *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTStmtRef_get)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTStmtRef_get)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTStmtRef_get)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTStmtRef_get *new_data = realloc(list->data, sizeof(ASTStmtRef_get) * new_capacity);
    if (!new_data) {
...
List_ASTStmtRef_get* nl_list_ASTStmtRef_get_new(void) {
    return nl_list_ASTStmtRef_get_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTStmtRef_get* nl_list_ASTStmtRef_get_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTStmtRef_get)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_ASTStmtRef_get *list = malloc(sizeof(List_ASTStmtRef_get));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(ASTStmtRef_get) * (size_t)capacity) : NULL;
...
void nl_list_ASTStmtRef_get_push(List_ASTStmtRef_get *list, ASTStmtRef_get value) {
    ...
    ensure_capacity_ASTStmtRef_get(list, list->length + 1);
...
void nl_list_ASTStmtRef_get_insert(List_ASTStmtRef_get *list, int index, ASTStmtRef_get value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTStmtRef_get) * (list->length - index));
...
ASTStmtRef_get nl_list_ASTStmtRef_get_remove(List_ASTStmtRef_get *list, int index) {
    ...
    ASTStmtRef_get value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTStmtRef_get) * (list->length - index - 1));
