#include "list_ASTNumber_new.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_ASTNumber_new definition must be included */
...
static void ensure_capacity_ASTNumber_new(List_ASTNumber_new *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTNumber_new)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTNumber_new)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTNumber_new)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTNumber_new *new_data = realloc(list->data, sizeof(ASTNumber_new) * new_capacity);
    if (!new_data) {
...
List_ASTNumber_new* nl_list_ASTNumber_new_new(void) {
    return nl_list_ASTNumber_new_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTNumber_new* nl_list_ASTNumber_new_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTNumber_new)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_ASTNumber_new *list = malloc(sizeof(List_ASTNumber_new));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(ASTNumber_new) * (size_t)capacity) : NULL;
...
void nl_list_ASTNumber_new_push(List_ASTNumber_new *list, ASTNumber_new value) {
    ...
    ensure_capacity_ASTNumber_new(list, list->length + 1);
...
void nl_list_ASTNumber_new_insert(List_ASTNumber_new *list, int index, ASTNumber_new value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTNumber_new) * (list->length - index));
...
ASTNumber_new nl_list_ASTNumber_new_remove(List_ASTNumber_new *list, int index) {
    ...
    ASTNumber_new value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTNumber_new) * (list->length - index - 1));
