#include "list_ASTStmtRef_new.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_ASTStmtRef_new definition must be included */
...
static void ensure_capacity_ASTStmtRef_new(List_ASTStmtRef_new *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTStmtRef_new)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTStmtRef_new)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTStmtRef_new)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTStmtRef_new *new_data = realloc(list->data, sizeof(ASTStmtRef_new) * new_capacity);
    if (!new_data) {
...
List_ASTStmtRef_new* nl_list_ASTStmtRef_new_new(void) {
    return nl_list_ASTStmtRef_new_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTStmtRef_new* nl_list_ASTStmtRef_new_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTStmtRef_new)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_ASTStmtRef_new *list = malloc(sizeof(List_ASTStmtRef_new));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(ASTStmtRef_new) * (size_t)capacity) : NULL;
...
void nl_list_ASTStmtRef_new_push(List_ASTStmtRef_new *list, ASTStmtRef_new value) {
    ...
    ensure_capacity_ASTStmtRef_new(list, list->length + 1);
...
void nl_list_ASTStmtRef_new_insert(List_ASTStmtRef_new *list, int index, ASTStmtRef_new value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTStmtRef_new) * (list->length - index));
...
ASTStmtRef_new nl_list_ASTStmtRef_new_remove(List_ASTStmtRef_new *list, int index) {
    ...
    ASTStmtRef_new value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTStmtRef_new) * (list->length - index - 1));
