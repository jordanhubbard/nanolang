...
static void ensure_capacity_ASTBlock(List_ASTBlock *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTBlock)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTBlock)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTBlock)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTBlock *new_data = realloc(list->data, sizeof(ASTBlock) * new_capacity);
    if (!new_data) {
...
List_ASTBlock* nl_list_ASTBlock_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTBlock)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTBlock) * (size_t)capacity) : NULL;
...
void nl_list_ASTBlock_push(List_ASTBlock *list, ASTBlock value) {
...
ASTBlock nl_list_ASTBlock_pop(List_ASTBlock *list) {
    if (list->length == 0) {
...
void nl_list_ASTBlock_insert(List_ASTBlock *list, int index, ASTBlock value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTBlock) * (list->length - index));
...
ASTBlock nl_list_ASTBlock_remove(List_ASTBlock *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTBlock value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTBlock) * (list->length - index - 1));
...
void nl_list_ASTBlock_set(List_ASTBlock *list, int index, ASTBlock value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTBlock nl_list_ASTBlock_get(List_ASTBlock *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
