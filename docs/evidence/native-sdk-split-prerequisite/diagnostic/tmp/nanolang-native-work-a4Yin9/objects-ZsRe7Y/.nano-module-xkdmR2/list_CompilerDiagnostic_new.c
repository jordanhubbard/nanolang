#include "list_CompilerDiagnostic_new.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_CompilerDiagnostic_new definition must be included */
...
static void ensure_capacity_CompilerDiagnostic_new(List_CompilerDiagnostic_new *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(CompilerDiagnostic_new)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(CompilerDiagnostic_new)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(CompilerDiagnostic_new)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    CompilerDiagnostic_new *new_data = realloc(list->data, sizeof(CompilerDiagnostic_new) * new_capacity);
    if (!new_data) {
...
List_CompilerDiagnostic_new* nl_list_CompilerDiagnostic_new_new(void) {
    return nl_list_CompilerDiagnostic_new_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_CompilerDiagnostic_new* nl_list_CompilerDiagnostic_new_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(CompilerDiagnostic_new)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_CompilerDiagnostic_new *list = malloc(sizeof(List_CompilerDiagnostic_new));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(CompilerDiagnostic_new) * (size_t)capacity) : NULL;
...
void nl_list_CompilerDiagnostic_new_push(List_CompilerDiagnostic_new *list, CompilerDiagnostic_new value) {
    ...
    ensure_capacity_CompilerDiagnostic_new(list, list->length + 1);
...
void nl_list_CompilerDiagnostic_new_insert(List_CompilerDiagnostic_new *list, int index, CompilerDiagnostic_new value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(CompilerDiagnostic_new) * (list->length - index));
...
CompilerDiagnostic_new nl_list_CompilerDiagnostic_new_remove(List_CompilerDiagnostic_new *list, int index) {
    ...
    CompilerDiagnostic_new value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(CompilerDiagnostic_new) * (list->length - index - 1));
