...
static void ensure_capacity_ASTMatch(List_ASTMatch *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTMatch)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTMatch)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTMatch)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTMatch *new_data = realloc(list->data, sizeof(ASTMatch) * new_capacity);
    if (!new_data) {
...
List_ASTMatch* nl_list_ASTMatch_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTMatch)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTMatch) * (size_t)capacity) : NULL;
...
void nl_list_ASTMatch_push(List_ASTMatch *list, ASTMatch value) {
...
ASTMatch nl_list_ASTMatch_pop(List_ASTMatch *list) {
    if (list->length == 0) {
...
void nl_list_ASTMatch_insert(List_ASTMatch *list, int index, ASTMatch value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTMatch) * (list->length - index));
...
ASTMatch nl_list_ASTMatch_remove(List_ASTMatch *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTMatch value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTMatch) * (list->length - index - 1));
...
void nl_list_ASTMatch_set(List_ASTMatch *list, int index, ASTMatch value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTMatch nl_list_ASTMatch_get(List_ASTMatch *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
