#include "list_CompilerDiagnostic_length.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_CompilerDiagnostic_length definition must be included */
...
static void ensure_capacity_CompilerDiagnostic_length(List_CompilerDiagnostic_length *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(CompilerDiagnostic_length)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(CompilerDiagnostic_length)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(CompilerDiagnostic_length)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    CompilerDiagnostic_length *new_data = realloc(list->data, sizeof(CompilerDiagnostic_length) * new_capacity);
    if (!new_data) {
...
List_CompilerDiagnostic_length* nl_list_CompilerDiagnostic_length_new(void) {
    return nl_list_CompilerDiagnostic_length_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_CompilerDiagnostic_length* nl_list_CompilerDiagnostic_length_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(CompilerDiagnostic_length)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_CompilerDiagnostic_length *list = malloc(sizeof(List_CompilerDiagnostic_length));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(CompilerDiagnostic_length) * (size_t)capacity) : NULL;
...
void nl_list_CompilerDiagnostic_length_push(List_CompilerDiagnostic_length *list, CompilerDiagnostic_length value) {
    ...
    ensure_capacity_CompilerDiagnostic_length(list, list->length + 1);
...
void nl_list_CompilerDiagnostic_length_insert(List_CompilerDiagnostic_length *list, int index, CompilerDiagnostic_length value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(CompilerDiagnostic_length) * (list->length - index));
...
CompilerDiagnostic_length nl_list_CompilerDiagnostic_length_remove(List_CompilerDiagnostic_length *list, int index) {
    ...
    CompilerDiagnostic_length value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(CompilerDiagnostic_length) * (list->length - index - 1));
