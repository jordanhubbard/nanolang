...
static void ensure_capacity_ASTBinaryOp(List_ASTBinaryOp *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTBinaryOp)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTBinaryOp)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTBinaryOp)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTBinaryOp *new_data = realloc(list->data, sizeof(ASTBinaryOp) * new_capacity);
    if (!new_data) {
...
List_ASTBinaryOp* nl_list_ASTBinaryOp_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTBinaryOp)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTBinaryOp) * (size_t)capacity) : NULL;
...
void nl_list_ASTBinaryOp_push(List_ASTBinaryOp *list, ASTBinaryOp value) {
...
ASTBinaryOp nl_list_ASTBinaryOp_pop(List_ASTBinaryOp *list) {
    if (list->length == 0) {
...
void nl_list_ASTBinaryOp_insert(List_ASTBinaryOp *list, int index, ASTBinaryOp value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTBinaryOp) * (list->length - index));
...
ASTBinaryOp nl_list_ASTBinaryOp_remove(List_ASTBinaryOp *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTBinaryOp value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTBinaryOp) * (list->length - index - 1));
...
void nl_list_ASTBinaryOp_set(List_ASTBinaryOp *list, int index, ASTBinaryOp value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTBinaryOp nl_list_ASTBinaryOp_get(List_ASTBinaryOp *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
