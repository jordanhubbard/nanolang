...
static void ensure_capacity_ASTNumber(List_ASTNumber *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTNumber)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTNumber)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTNumber)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTNumber *new_data = realloc(list->data, sizeof(ASTNumber) * new_capacity);
    if (!new_data) {
...
List_ASTNumber* nl_list_ASTNumber_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTNumber)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTNumber) * (size_t)capacity) : NULL;
...
void nl_list_ASTNumber_push(List_ASTNumber *list, ASTNumber value) {
...
ASTNumber nl_list_ASTNumber_pop(List_ASTNumber *list) {
    if (list->length == 0) {
...
void nl_list_ASTNumber_insert(List_ASTNumber *list, int index, ASTNumber value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTNumber) * (list->length - index));
...
ASTNumber nl_list_ASTNumber_remove(List_ASTNumber *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTNumber value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTNumber) * (list->length - index - 1));
...
void nl_list_ASTNumber_set(List_ASTNumber *list, int index, ASTNumber value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTNumber nl_list_ASTNumber_get(List_ASTNumber *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
