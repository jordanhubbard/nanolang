#include "list_ASTStmtRef_push.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_ASTStmtRef_push definition must be included */
...
static void ensure_capacity_ASTStmtRef_push(List_ASTStmtRef_push *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTStmtRef_push)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTStmtRef_push)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTStmtRef_push)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTStmtRef_push *new_data = realloc(list->data, sizeof(ASTStmtRef_push) * new_capacity);
    if (!new_data) {
...
List_ASTStmtRef_push* nl_list_ASTStmtRef_push_new(void) {
    return nl_list_ASTStmtRef_push_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTStmtRef_push* nl_list_ASTStmtRef_push_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTStmtRef_push)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_ASTStmtRef_push *list = malloc(sizeof(List_ASTStmtRef_push));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(ASTStmtRef_push) * (size_t)capacity) : NULL;
...
void nl_list_ASTStmtRef_push_push(List_ASTStmtRef_push *list, ASTStmtRef_push value) {
    ...
    ensure_capacity_ASTStmtRef_push(list, list->length + 1);
...
void nl_list_ASTStmtRef_push_insert(List_ASTStmtRef_push *list, int index, ASTStmtRef_push value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTStmtRef_push) * (list->length - index));
...
ASTStmtRef_push nl_list_ASTStmtRef_push_remove(List_ASTStmtRef_push *list, int index) {
    ...
    ASTStmtRef_push value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTStmtRef_push) * (list->length - index - 1));
