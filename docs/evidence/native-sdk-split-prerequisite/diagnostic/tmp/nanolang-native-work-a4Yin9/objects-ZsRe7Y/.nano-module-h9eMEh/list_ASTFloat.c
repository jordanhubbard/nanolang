...
static void ensure_capacity_ASTFloat(List_ASTFloat *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTFloat)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTFloat)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTFloat)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTFloat *new_data = realloc(list->data, sizeof(ASTFloat) * new_capacity);
    if (!new_data) {
...
List_ASTFloat* nl_list_ASTFloat_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTFloat)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTFloat) * (size_t)capacity) : NULL;
...
void nl_list_ASTFloat_push(List_ASTFloat *list, ASTFloat value) {
...
ASTFloat nl_list_ASTFloat_pop(List_ASTFloat *list) {
    if (list->length == 0) {
...
void nl_list_ASTFloat_insert(List_ASTFloat *list, int index, ASTFloat value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTFloat) * (list->length - index));
...
ASTFloat nl_list_ASTFloat_remove(List_ASTFloat *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTFloat value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTFloat) * (list->length - index - 1));
...
void nl_list_ASTFloat_set(List_ASTFloat *list, int index, ASTFloat value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTFloat nl_list_ASTFloat_get(List_ASTFloat *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
