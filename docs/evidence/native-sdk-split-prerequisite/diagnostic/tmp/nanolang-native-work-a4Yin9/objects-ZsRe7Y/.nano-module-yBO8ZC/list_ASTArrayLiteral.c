...
static void ensure_capacity_ASTArrayLiteral(List_ASTArrayLiteral *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTArrayLiteral)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTArrayLiteral)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTArrayLiteral)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTArrayLiteral *new_data = realloc(list->data, sizeof(ASTArrayLiteral) * new_capacity);
    if (!new_data) {
...
List_ASTArrayLiteral* nl_list_ASTArrayLiteral_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTArrayLiteral)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTArrayLiteral) * (size_t)capacity) : NULL;
...
void nl_list_ASTArrayLiteral_push(List_ASTArrayLiteral *list, ASTArrayLiteral value) {
...
ASTArrayLiteral nl_list_ASTArrayLiteral_pop(List_ASTArrayLiteral *list) {
    if (list->length == 0) {
...
void nl_list_ASTArrayLiteral_insert(List_ASTArrayLiteral *list, int index, ASTArrayLiteral value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTArrayLiteral) * (list->length - index));
...
ASTArrayLiteral nl_list_ASTArrayLiteral_remove(List_ASTArrayLiteral *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTArrayLiteral value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTArrayLiteral) * (list->length - index - 1));
...
void nl_list_ASTArrayLiteral_set(List_ASTArrayLiteral *list, int index, ASTArrayLiteral value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTArrayLiteral nl_list_ASTArrayLiteral_get(List_ASTArrayLiteral *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
