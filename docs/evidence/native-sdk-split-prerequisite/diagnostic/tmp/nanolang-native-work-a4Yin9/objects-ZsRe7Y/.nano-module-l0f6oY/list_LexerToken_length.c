#include "list_LexerToken_length.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_LexerToken_length definition must be included */
...
static void ensure_capacity_LexerToken_length(List_LexerToken_length *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(LexerToken_length)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(LexerToken_length)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(LexerToken_length)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    LexerToken_length *new_data = realloc(list->data, sizeof(LexerToken_length) * new_capacity);
    if (!new_data) {
...
List_LexerToken_length* nl_list_LexerToken_length_new(void) {
    return nl_list_LexerToken_length_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_LexerToken_length* nl_list_LexerToken_length_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(LexerToken_length)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_LexerToken_length *list = malloc(sizeof(List_LexerToken_length));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(LexerToken_length) * (size_t)capacity) : NULL;
...
void nl_list_LexerToken_length_push(List_LexerToken_length *list, LexerToken_length value) {
    ...
    ensure_capacity_LexerToken_length(list, list->length + 1);
...
void nl_list_LexerToken_length_insert(List_LexerToken_length *list, int index, LexerToken_length value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(LexerToken_length) * (list->length - index));
...
LexerToken_length nl_list_LexerToken_length_remove(List_LexerToken_length *list, int index) {
    ...
    LexerToken_length value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(LexerToken_length) * (list->length - index - 1));
