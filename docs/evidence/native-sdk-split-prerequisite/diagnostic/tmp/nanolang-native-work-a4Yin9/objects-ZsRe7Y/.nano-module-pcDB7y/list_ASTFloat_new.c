#include "list_ASTFloat_new.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_ASTFloat_new definition must be included */
...
static void ensure_capacity_ASTFloat_new(List_ASTFloat_new *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTFloat_new)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTFloat_new)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTFloat_new)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTFloat_new *new_data = realloc(list->data, sizeof(ASTFloat_new) * new_capacity);
    if (!new_data) {
...
List_ASTFloat_new* nl_list_ASTFloat_new_new(void) {
    return nl_list_ASTFloat_new_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTFloat_new* nl_list_ASTFloat_new_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTFloat_new)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_ASTFloat_new *list = malloc(sizeof(List_ASTFloat_new));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(ASTFloat_new) * (size_t)capacity) : NULL;
...
void nl_list_ASTFloat_new_push(List_ASTFloat_new *list, ASTFloat_new value) {
    ...
    ensure_capacity_ASTFloat_new(list, list->length + 1);
...
void nl_list_ASTFloat_new_insert(List_ASTFloat_new *list, int index, ASTFloat_new value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTFloat_new) * (list->length - index));
...
ASTFloat_new nl_list_ASTFloat_new_remove(List_ASTFloat_new *list, int index) {
    ...
    ASTFloat_new value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTFloat_new) * (list->length - index - 1));
