...
static void ensure_capacity_ASTSet(List_ASTSet *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTSet)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTSet)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTSet)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTSet *new_data = realloc(list->data, sizeof(ASTSet) * new_capacity);
    if (!new_data) {
...
List_ASTSet* nl_list_ASTSet_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTSet)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTSet) * (size_t)capacity) : NULL;
...
void nl_list_ASTSet_push(List_ASTSet *list, ASTSet value) {
...
ASTSet nl_list_ASTSet_pop(List_ASTSet *list) {
    if (list->length == 0) {
...
void nl_list_ASTSet_insert(List_ASTSet *list, int index, ASTSet value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTSet) * (list->length - index));
...
ASTSet nl_list_ASTSet_remove(List_ASTSet *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTSet value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTSet) * (list->length - index - 1));
...
void nl_list_ASTSet_set(List_ASTSet *list, int index, ASTSet value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTSet nl_list_ASTSet_get(List_ASTSet *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
