#include "list_ASTStmtRef_length.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_ASTStmtRef_length definition must be included */
...
static void ensure_capacity_ASTStmtRef_length(List_ASTStmtRef_length *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTStmtRef_length)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTStmtRef_length)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTStmtRef_length)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTStmtRef_length *new_data = realloc(list->data, sizeof(ASTStmtRef_length) * new_capacity);
    if (!new_data) {
...
List_ASTStmtRef_length* nl_list_ASTStmtRef_length_new(void) {
    return nl_list_ASTStmtRef_length_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTStmtRef_length* nl_list_ASTStmtRef_length_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTStmtRef_length)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_ASTStmtRef_length *list = malloc(sizeof(List_ASTStmtRef_length));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(ASTStmtRef_length) * (size_t)capacity) : NULL;
...
void nl_list_ASTStmtRef_length_push(List_ASTStmtRef_length *list, ASTStmtRef_length value) {
    ...
    ensure_capacity_ASTStmtRef_length(list, list->length + 1);
...
void nl_list_ASTStmtRef_length_insert(List_ASTStmtRef_length *list, int index, ASTStmtRef_length value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTStmtRef_length) * (list->length - index));
...
ASTStmtRef_length nl_list_ASTStmtRef_length_remove(List_ASTStmtRef_length *list, int index) {
    ...
    ASTStmtRef_length value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTStmtRef_length) * (list->length - index - 1));
