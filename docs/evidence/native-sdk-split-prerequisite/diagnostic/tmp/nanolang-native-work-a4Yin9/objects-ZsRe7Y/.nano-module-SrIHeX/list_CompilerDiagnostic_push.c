#include "list_CompilerDiagnostic_push.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_CompilerDiagnostic_push definition must be included */
...
static void ensure_capacity_CompilerDiagnostic_push(List_CompilerDiagnostic_push *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(CompilerDiagnostic_push)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(CompilerDiagnostic_push)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(CompilerDiagnostic_push)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    CompilerDiagnostic_push *new_data = realloc(list->data, sizeof(CompilerDiagnostic_push) * new_capacity);
    if (!new_data) {
...
List_CompilerDiagnostic_push* nl_list_CompilerDiagnostic_push_new(void) {
    return nl_list_CompilerDiagnostic_push_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_CompilerDiagnostic_push* nl_list_CompilerDiagnostic_push_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(CompilerDiagnostic_push)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_CompilerDiagnostic_push *list = malloc(sizeof(List_CompilerDiagnostic_push));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(CompilerDiagnostic_push) * (size_t)capacity) : NULL;
...
void nl_list_CompilerDiagnostic_push_push(List_CompilerDiagnostic_push *list, CompilerDiagnostic_push value) {
    ...
    ensure_capacity_CompilerDiagnostic_push(list, list->length + 1);
...
void nl_list_CompilerDiagnostic_push_insert(List_CompilerDiagnostic_push *list, int index, CompilerDiagnostic_push value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(CompilerDiagnostic_push) * (list->length - index));
...
CompilerDiagnostic_push nl_list_CompilerDiagnostic_push_remove(List_CompilerDiagnostic_push *list, int index) {
    ...
    CompilerDiagnostic_push value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(CompilerDiagnostic_push) * (list->length - index - 1));
