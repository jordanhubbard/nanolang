#include "list_ASTFloat_length.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_ASTFloat_length definition must be included */
...
static void ensure_capacity_ASTFloat_length(List_ASTFloat_length *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTFloat_length)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTFloat_length)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTFloat_length)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTFloat_length *new_data = realloc(list->data, sizeof(ASTFloat_length) * new_capacity);
    if (!new_data) {
...
List_ASTFloat_length* nl_list_ASTFloat_length_new(void) {
    return nl_list_ASTFloat_length_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTFloat_length* nl_list_ASTFloat_length_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTFloat_length)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_ASTFloat_length *list = malloc(sizeof(List_ASTFloat_length));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(ASTFloat_length) * (size_t)capacity) : NULL;
...
void nl_list_ASTFloat_length_push(List_ASTFloat_length *list, ASTFloat_length value) {
    ...
    ensure_capacity_ASTFloat_length(list, list->length + 1);
...
void nl_list_ASTFloat_length_insert(List_ASTFloat_length *list, int index, ASTFloat_length value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTFloat_length) * (list->length - index));
...
ASTFloat_length nl_list_ASTFloat_length_remove(List_ASTFloat_length *list, int index) {
    ...
    ASTFloat_length value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTFloat_length) * (list->length - index - 1));
