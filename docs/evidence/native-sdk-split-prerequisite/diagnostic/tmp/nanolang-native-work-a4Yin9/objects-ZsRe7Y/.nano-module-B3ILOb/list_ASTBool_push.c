#include "list_ASTBool_push.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <limits.h>

/* Note: The actual struct nl_ASTBool_push definition must be included */
...
static void ensure_capacity_ASTBool_push(List_ASTBool_push *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTBool_push)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTBool_push)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTBool_push)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTBool_push *new_data = realloc(list->data, sizeof(ASTBool_push) * new_capacity);
    if (!new_data) {
...
List_ASTBool_push* nl_list_ASTBool_push_new(void) {
    return nl_list_ASTBool_push_with_capacity(INITIAL_CAPACITY);
}

/* Create a new list with specified initial capacity */
List_ASTBool_push* nl_list_ASTBool_push_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTBool_push)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    List_ASTBool_push *list = malloc(sizeof(List_ASTBool_push));
    if (!list) {
        fprintf(stderr, "Error: Failed to allocate memory for list\n");
        exit(1);
    }
    
    list->data = capacity ? malloc(sizeof(ASTBool_push) * (size_t)capacity) : NULL;
...
void nl_list_ASTBool_push_push(List_ASTBool_push *list, ASTBool_push value) {
    ...
    ensure_capacity_ASTBool_push(list, list->length + 1);
...
void nl_list_ASTBool_push_insert(List_ASTBool_push *list, int index, ASTBool_push value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTBool_push) * (list->length - index));
...
ASTBool_push nl_list_ASTBool_push_remove(List_ASTBool_push *list, int index) {
    ...
    ASTBool_push value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTBool_push) * (list->length - index - 1));
