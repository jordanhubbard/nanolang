...
static void ensure_capacity_ASTStmtRef(List_ASTStmtRef *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTStmtRef)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTStmtRef)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTStmtRef)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTStmtRef *new_data = realloc(list->data, sizeof(ASTStmtRef) * new_capacity);
    if (!new_data) {
...
List_ASTStmtRef* nl_list_ASTStmtRef_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTStmtRef)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTStmtRef) * (size_t)capacity) : NULL;
...
void nl_list_ASTStmtRef_push(List_ASTStmtRef *list, ASTStmtRef value) {
...
ASTStmtRef nl_list_ASTStmtRef_pop(List_ASTStmtRef *list) {
    if (list->length == 0) {
...
void nl_list_ASTStmtRef_insert(List_ASTStmtRef *list, int index, ASTStmtRef value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTStmtRef) * (list->length - index));
...
ASTStmtRef nl_list_ASTStmtRef_remove(List_ASTStmtRef *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTStmtRef value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTStmtRef) * (list->length - index - 1));
...
void nl_list_ASTStmtRef_set(List_ASTStmtRef *list, int index, ASTStmtRef value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTStmtRef nl_list_ASTStmtRef_get(List_ASTStmtRef *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
