...
static void ensure_capacity_ASTLet(List_ASTLet *list, int min_capacity) {
    if (min_capacity < 0 || list->capacity < 0 ||
        (size_t)min_capacity > SIZE_MAX / sizeof(ASTLet)) {
    ...
    if ((size_t)new_capacity > SIZE_MAX / sizeof(ASTLet)) {
        new_capacity = min_capacity;
    }
    
    while (new_capacity < min_capacity) {
        if (new_capacity > INT_MAX / GROWTH_FACTOR ||
            (size_t)new_capacity > (SIZE_MAX / sizeof(ASTLet)) / GROWTH_FACTOR) {
            new_capacity = min_capacity;
            break;
        }
    ...
    ASTLet *new_data = realloc(list->data, sizeof(ASTLet) * new_capacity);
    if (!new_data) {
...
List_ASTLet* nl_list_ASTLet_with_capacity(int capacity) {
    if (capacity < 0 || (size_t)capacity > SIZE_MAX / sizeof(ASTLet)) {
        fprintf(stderr, "I cannot represent this list capacity.\n");
        exit(1);
    }
    ...
    list->data = capacity ? malloc(sizeof(ASTLet) * (size_t)capacity) : NULL;
...
void nl_list_ASTLet_push(List_ASTLet *list, ASTLet value) {
...
ASTLet nl_list_ASTLet_pop(List_ASTLet *list) {
    if (list->length == 0) {
...
void nl_list_ASTLet_insert(List_ASTLet *list, int index, ASTLet value) {
    ...
    memmove(&list->data[index + 1], &list->data[index], 
            sizeof(ASTLet) * (list->length - index));
...
ASTLet nl_list_ASTLet_remove(List_ASTLet *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
    ...
    ASTLet value = list->data[index];
    
    /* Shift elements to the left */
    memmove(&list->data[index], &list->data[index + 1], 
            sizeof(ASTLet) * (list->length - index - 1));
...
void nl_list_ASTLet_set(List_ASTLet *list, int index, ASTLet value) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
...
ASTLet nl_list_ASTLet_get(List_ASTLet *list, int index) {
    if (index < 0 || index >= list->length) {
        fprintf(stderr, "Error: Index %d out of bounds for list of length %d\n", 
                index, list->length);
        exit(1);
