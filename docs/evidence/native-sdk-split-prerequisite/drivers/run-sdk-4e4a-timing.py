...
old=Path('/tmp/nanolang-native-sdk-4e4a1-linux-source');root=Path('/tmp/nanolang-native-sdk-4e4a1-linux-diagnostic-source');report=Path('/tmp/nanolang-native-sdk-4e4a1-linux-diagnostic');prep=Path('/tmp/nanolang-native-sdk-4e4a1-linux-preparation');root.mkdir();report.mkdir()
def ident(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
selected=json.loads(Path('/tmp/native-sdk-4e4a1-selected-git.json').read_text());oldsources=json.loads((prep/'source-after.json').read_text());oldproducts=json.loads((prep/'products-after.json').read_text());oldtools=json.loads((prep/'tools-after.json').read_text())
...
with tarfile.open('/tmp/native-sdk-4e4a1-source.tar.gz') as a:a.extractall(root,filter='data')
