...
save(base/'identity.json',dict(pin='2a4caa269d1ea2d08120b9b6c24c995a436ba14b',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
assert shutil.disk_usage(base).free>2*1024**3
scope=json.loads(pathlib.Path(__file__).with_name('scope.json').read_text());assert scope['pin']=='2a4caa269d1ea2d08120b9b6c24c995a436ba14b'
