...
start=time.monotonic();base=Path('/run/user/1000/nanolang-record-llvm-complete-seal');reports=base/'reports';index=json.loads((base/'report-sha256.json').read_text());results=[]
configs=[('linux','nanolang-record-llvm-linux-wasm-seal/nanolang-record-llvm-78042-linux-wasm/wasm-artifacts.json'),('darwin','nanolang-record-llvm-darwin-recovery-seal/nanolang-record-llvm-78042-recovery-puck-backup/wasm-artifacts.json')]
...
out={'status':'PASS','sealed_git_pin':'3d4afccb726e18c8b59813c1de1b710e3b56636f','hosts':results,'seconds':time.monotonic()-start,'scope':'Actual retained Wasm modules have no imports; both engines execute all73 linked/observed products at O0/O2 with exact command products, successful terminals, dynamic memory growth, contiguous two-mode fault recovery and explicit limited-memory refusal. Startup/ABI/emission/package, source correspondence and current-main integration remain separate obligations.'}
Path('/home/jkh/nanolang-qualification/llvm-root-wasm-coverage.json').write_text(json.dumps(out,indent=2)+'\n')
