...
pin=git('rev-parse','HEAD');assert pin.startswith('e75a719bf');assert not git('status','--porcelain','--untracked-files=no')
...
executables=[shutil.which('cc'),cc,san,shutil.which('python3'),shutil.which('make'),shutil.which('ar'),shutil.which('ld')]
def tool_map():
 paths=set(pathlib.Path(p).resolve() for p in executables if p)
 for folder in ('bin','obj'):
  paths.update(p for p in (root/folder).rglob('*') if p.is_file() and (folder=='bin' or p.suffix in ('.o','.a','.so','.dylib')))
...
steps=[('bootstrap',['make','-j2','bootstrap','CC='+cc,*flags],1800),('prepare',['make','-j2','nano_virt','nano_vm','nvm2c','CC='+cc,*flags],900),('guards',[sys.executable,'-m','unittest','-v','tests.test_canonical_match_guards'],2400),('shared-policy',[sys.executable,'-m','unittest','-v','tests.test_shared_match_policy_docs','tests.test_cseed_match_totality'],1200),('owned-refusals',[sys.executable,'-m','unittest','-v','tests.test_selected_variant_ownership.SelectedVariantOwnership.test_guarded_owned_match_remains_rejected','tests.test_generic_selected_ownership.GenericSelectedOwnership.test_guarded_generic_match_remains_rejected'],1200)]
status=1
try:
 for name,command,bound in steps:
  ...
  if status or sa!=before or (name not in ('bootstrap','prepare') and ta!=tb):break
finally:
