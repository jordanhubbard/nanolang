...
for short in (['16767','429da','c032c'] if host=='linux' else ['429da','c032c']):
 root=Path('/tmp/nanolang-record-execution-'+short+'-'+host)
 ...
 for kind in ['source','tools']:
  phase='complete-declarations' if short=='c032c' else 'ordinary';p=root/(phase+'-'+kind+'-after.json');rows=json.loads(p.read_text());n=0
  for key,v in rows.items():
   path=Path('/tmp/nanolang-record-execution-qualified-'+short)/key if kind=='source' else Path(v['path']);digest=v if kind=='source' else v['sha256'];assert sha(path)==digest,(kind,path);n+=1
  current[short+'/'+kind]={'entries':n,'map':str(p),'sha256':sha(p)}
 p=root/(('complete-declarations' if short=='c032c' else 'ordinary')+'-artifacts.json');rows=json.loads(p.read_text())
 ...
 for suffix in ['source.json','launch.py']:
  p=Path('/tmp/nanolang-record-execution-'+short+'-'+suffix);save(p,'extra/'+p.name)
 p=Path('/tmp/nanolang-record-execution-'+short+'-'+host+'-launch.log');save(p,'extra/'+p.name)
 p=Path('/tmp/nanolang-record-execution-'+short+'-source.tar.gz');h=add(p)
 manifest=json.loads(Path('/tmp/nanolang-record-execution-'+short+'-source.json').read_text());assert h==manifest['archive_sha256']
 current[short+'/source-archive']={'sha256':h,'bytes':p.stat().st_size,'artifact':str(objects/h)}
extra=reports/'current-inputs.json';extra.write_text(json.dumps(current,indent=2)+'\n');index['current-inputs.json']={'sha256':sha(extra),'bytes':extra.stat().st_size};count+=3 if host=='linux' else 2
summary={'host':host,'reports':len(index),'objects':len(store),'bytes':sum(v['bytes'] for v in store.values()),'references':count,'equal_pairs':len(pairs),'pairs':pairs,'terminals':terms,'scope':'copied non-admitting execution-plan query; seven selected compiler configurations; first16767 UNRESOLVED allocation classification and both429da missing-header fixture terminals retained'}
