...
root=pathlib.Path('/home/jkh/Src/nanolang-mutation-builtin-corrected');out=pathlib.Path('/tmp/nanolang-mutation-identity-82ce-bootstrap');out.mkdir(exist_ok=False)
def sha(p):return hashlib.sha256(pathlib.Path(p).read_bytes()).hexdigest()
def git(*a):return subprocess.check_output(['git',*a],cwd=root,text=True).strip()
assert git('rev-parse','HEAD')=='82ce8b29d0809649185c8e5dd227c7f2f436466c'
