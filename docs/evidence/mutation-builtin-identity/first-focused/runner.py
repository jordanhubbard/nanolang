...
root=pathlib.Path('/home/jkh/Src/nanolang-mutation-builtin-corrected'); out=pathlib.Path('/tmp/nanolang-mutation-identity-54ea-focused');out.mkdir(exist_ok=False)
def sha(p):return hashlib.sha256(pathlib.Path(p).read_bytes()).hexdigest()
def git(*a):return subprocess.check_output(['git',*a],cwd=root,text=True).strip()
def save(n,v):(out/n).write_text(json.dumps(v,indent=2)+'\n')
assert git('rev-parse','--short=9','HEAD')=='54eab7f3c'
...
commands=[('tools',['make','-j2','nano_virt','nano_vm','nvm2c'],600),('focused',['python3','-m','unittest','-v','tests.test_mutation_builtin_identity'],3600),('adjacent',['python3','-m','unittest','-v','tests.test_canonical_prefix_conversion','tests.test_scalar_reduce_source.ScalarReduce.test_bound_reduce_function_and_local','tests.test_scalar_reduce_source.ScalarReduce.test_checked_signature_refusals_preserve_output'],600)]
results=[]
try:
...
raise SystemExit(0 if len(results)==3 and all(x['status']==0 for x in results) else 1)
