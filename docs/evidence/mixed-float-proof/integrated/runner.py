...
def run(name,cmd,extra):
 extra={'NMS_RUNTIME_CLANG':'clang','NMS_NATIVE_CLANG_FLAGS':'--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13',**extra}
...
for name in ['cc','gcc','clang','make','python3','ld','as','opt']:paths.add(Path(shutil.which(name)).resolve())
...
paths.update(Path('bin')/n for n in ['nano_vm','nvm2c','nvm2llvm','nvm2wasm','nanoisa'])
