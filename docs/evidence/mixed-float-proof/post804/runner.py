...
if run('prepare',['make','-f','Makefile.gnu','-f','/tmp/nanolang-proof804-setup.mk','-j2','proof804-setup'],{'CC':'cc'}):raise SystemExit(1)
...
for name,cmd,extra in [('gcc',['make','test-mixed-float-proof'],{'CC':'cc'}),('clang',['make','test-mixed-float-proof'],{'CC':'/tmp/nanolang-projection-clang','MIXED_CFLAGS':'-fsanitize=address,undefined -fno-sanitize-recover=all','ASAN_OPTIONS':'detect_leaks=1:halt_on_error=1'}),('adjacent',['make','-j2','test-verifier','test-mixed-layout-view'],{'CC':'cc','NMS_NATIVE_CLANG_FLAGS':'--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13','ASAN_OPTIONS':'detect_leaks=1:halt_on_error=1'})]:
