...
base=Path('/home/jkh/nanolang-qualification');out=Path('/home/jkh/Src/nanolang-evaluator-dynamic-child-owners/docs/evidence/collection-c34');out.mkdir(parents=True,exist_ok=True)
def sha(p):return hashlib.file_digest(Path(p).open('rb'),'sha256').hexdigest()
scope=json.loads((base/'collection-c34-drivers/scope.json').read_text());expected={p:v['sha256'] for p,v in scope['files'].items()}
seal={'source':scope['pin'],'lanes':{},'objects':{}}
for label,r,status in [('linux-ordinary',base/'collection-c34-ordinary',2)]:
 objects={}
 ...
 log=(r/'test-evaluator-collection-ownership.log').read_text();assert 'dynamic_child_owners' in log and 'run_ctx_init(&ctx, source)' in log
 # Buffered stdout is not a pass marker on this aborted owning method.
 if status:assert 'Assertion' in log
...
seal['remaining_lanes']='unrun after Linux ordinary checker refusal';seal['totals']={'reports':sum(len(x['reports']) for x in seal['lanes'].values()),'objects':len(seal['objects']),'bytes':sum(x['bytes'] for x in seal['objects'].values())}
