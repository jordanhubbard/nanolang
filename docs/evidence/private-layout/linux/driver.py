...
def run(label,cmd,extra=None):
 ...
 write(label+'-command.json',{'argv':cmd,'selectors':{k:v for k,v in env.items() if k.startswith(('NANO_','FILE_','WRAPPER_','OWNERSHIP_')) or k in ('CC','SDKROOT','LIBRARY_PATH','NMS_NATIVE_CLANG_FLAGS','LSAN_OPTIONS')}})
...
py=[sys.executable,'-m','unittest','-f','-v','tests.test_ownership_layouts_private']
clang=shutil.which('clang')
normal=''
clang_flags='' if darwin else '--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13'
sanitize=' -fsanitize=address,undefined -fno-omit-frame-pointer'
run('ordinary',py,dict(OWNERSHIP_LAYOUT_CC=os.environ['CC'],OWNERSHIP_LAYOUT_CFLAGS=normal))
run('clang-ordinary',py,dict(OWNERSHIP_LAYOUT_CC=clang,OWNERSHIP_LAYOUT_CFLAGS=clang_flags))
run('sanitizer',py,dict(OWNERSHIP_LAYOUT_CC=os.environ['CARRIER_SAN_CC'],OWNERSHIP_LAYOUT_CFLAGS=(clang_flags if darwin else normal)+sanitize))
if not darwin:run('clang-sanitizer',py,dict(OWNERSHIP_LAYOUT_CC=clang,OWNERSHIP_LAYOUT_CFLAGS=clang_flags+sanitize))
