...
assert selected['pin']=='91cd9dcfe5944744f825dfbb16bfb9d8eb3733b8'
assert len(selected['files'])==4008
with tarfile.open(archive) as tf:
 members=tf.getmembers()
 assert len(members)==4008 and {m.name for m in members}==set(selected['files'])
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitizer',sancc,True)]:
  ...
  if not san:
   ...
   assert differences and all(n.startswith('docs/') or n in ('tests/nanovm/test_cop_exec.c','tests/test_array_abi_loader.py') for n in differences),differences
   oldphase=json.loads((prior/'fresh-providers.json').read_text())
   assert oldphase['returncode']==0 and oldphase['argv']==[*args,'bin/nanoc_c','nano_virt','nano_vm','nano_vmd','nano_cop','nvm2c','nanoisa_dump','nvm2c-runtime']
