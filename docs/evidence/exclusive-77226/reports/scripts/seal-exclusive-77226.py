...
dest=root/'docs/evidence/exclusive-77226';dest.mkdir(parents=True,exist_ok=False)
cas=Path('/tmp/nanolang-exclusive-77226-artifacts');cas.mkdir(exist_ok=False)
source=Path('/tmp/nanolang-exclusive-77226-linux')
manifest=dict(source_pin='77226ef6e078145e217170bce0d2bef9b24a6732',production_pin='40870d07805c85c2a6600c7d02b8440cd547f57a',scope='One private full typecheck_driver diagnostic, original10s shadow deadline and120s outer bound. No production acceptance. Final unmatched interval is unmeasured; recursive view timing is inclusive and nonadditive.',reports={},artifacts={},commands=[])
def report(p,label):
...
for p in sorted(Path('/tmp/nanolang-typecheck-profile-preflight').iterdir()):
 if p.is_file():report(p,'profiler-preflight')
...
for p in ('/tmp/run-exclusive-77226.py','/tmp/summarize-typecheck-timing.py',__file__):report(Path(p),'scripts')
