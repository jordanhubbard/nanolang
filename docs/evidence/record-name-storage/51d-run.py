...
save(base/'identity.json',dict(pin='51d0b9a998e3cfae5d255d31b9e9ae14f1519e14',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  shimdir=out/'shims';shimdir.mkdir()
  shim=shimdir/'rm'
  shim.write_text('#!'+sys.executable+'\n'+"import os,sys,shutil\nfrom pathlib import Path\np=Path(os.environ['NAME_GATE_BINARY']).resolve()\nif p.is_file() and any(Path(a).resolve()==p for a in sys.argv[1:] if not a.startswith('-')):shutil.copy2(p,os.environ['NAME_GATE_RETAIN'])\nos.execv('/bin/rm',['/bin/rm',*sys.argv[1:]])\n")
  shim.chmod(0o755)
  previous_path=env['PATH'];env['PATH']=str(shimdir)+os.pathsep+previous_path
  env['NAME_GATE_BINARY']=str(root/'tests/test_evaluator_collection_ownership')
  env['NAME_GATE_RETAIN']=str(out/'collection-ownership')
  try:
   run(name+'-owning-method',[*args,'test-evaluator-owned-lifetimes'],root,out,1200)
   run(name+'-collection-method',[*args,'test-evaluator-collection-ownership'],root,out,1200)
  finally:
