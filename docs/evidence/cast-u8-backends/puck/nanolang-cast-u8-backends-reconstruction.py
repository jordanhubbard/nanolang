...
pin=git('rev-parse','HEAD');assert pin.startswith(('163290dda','e0e85441f'))
overlay=['tests/test_nvm2llvm.py','tests/test_nvm2wasm.py']
if pin.startswith('e0e85441f'):
 assert sorted(git('diff','--name-only').splitlines())==overlay
 for name in overlay:assert (root/name).read_bytes()==subprocess.check_output(['git','show','163290dda:'+name],cwd=root)
else:assert not git('status','--porcelain','--untracked-files=no')
...
report={'head':pin,'host':platform.node(),'platform':platform.platform(),'fixture_overlay':'163290dda:tests/test_nvm2llvm.py,tests/test_nvm2wasm.py' if darwin else None,'matrix_mode':mode,'scope':'Selected generated native C ASan/UBSan and emitted native IR ASan when requested; ordinary LLVM/Wasm. Explicit target ABI corrected generated CAST_U8 native/LLVM/Wasm acceptance with inventoried first-build ordinary providers; source and reconstruction remain unimplemented','phases':[]};save('manifest.json',report)
...
for name,args in [('reconstructed-c',[str(root/'bin/nvm2hl'),'--language','c']),('reconstructed-nano',[str(root/'bin/nvm2hl'),'--language','nano'])]:
...
save('terminal.json',{'status':'PASS','head':pin,'scope':'I preserve existing output when C and Nano reconstruction encounter unsupported CAST_U8. This does not establish source conversion support.'})
