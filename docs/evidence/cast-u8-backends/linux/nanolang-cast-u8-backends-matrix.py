...
pin=git('rev-parse','HEAD');assert pin.startswith('252dc2266')
...
if mode=='asan':native=llvm+'/clang'
...
report={'head':pin,'host':platform.node(),'platform':platform.platform(),'matrix_mode':mode,'scope':'Selected generated native C ASan/UBSan and emitted native IR ASan when requested; ordinary LLVM/Wasm. Corrected generated CAST_U8 native/LLVM/Wasm acceptance with inventoried first-build ordinary providers; source and reconstruction remain unimplemented','phases':[]};save('manifest.json',report)
...
phases=[('discovery',[sys.executable,'-c',"import unittest;from tests import test_cast_u8_backends;s=unittest.defaultTestLoader.loadTestsFromModule(test_cast_u8_backends);assert s.countTestCases()==5;print('I discover exactly five generated CAST_U8 methods.')"],30),('build',['make','-j2','nanoisa_dump','nano_vm','nvm2c','nvm2llvm','nvm2wasm',*flags],1800),('generated-byte',[sys.executable,'-m','unittest','-v','tests.test_cast_u8_backends'],1800)]
