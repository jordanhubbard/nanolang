...
root=pathlib.Path('/home/jkh/Src/nanolang-owned-float-locals-unsafe');out=pathlib.Path('/tmp/nanolang-owned-float-locals-qualified');out.mkdir(exist_ok=False)
...
for path in [pathlib.Path(__file__),pathlib.Path('/tmp/nanolang-transitive-wrapper-unittest.py'),pathlib.Path('/tmp/nanolang-owned-float-locals-setup.mk')]: (out/path.name).write_bytes(path.read_bytes())
env=os.environ.copy()
...
methods=['test_float_local_unsafe_original_pattern','test_float_local_unsafe_control_flow','test_float_local_unsafe_refusals_preserve_publication']
...
steps=[('setup',setup,None),('focused-gcc',['python3','/tmp/nanolang-transitive-wrapper-unittest.py',str(out/'focused-gcc-producers.json'),*focus],'/usr/bin/gcc'),('focused-clang',['python3','/tmp/nanolang-transitive-wrapper-unittest.py',str(out/'focused-clang-producers.json'),*focus],'/tmp/nanolang-projection-clang'),('runtime',['make','test-owned-binary64','test-owned-value-graphs','test-owned-value-results','test-consuming-calls','test-multiple-consuming-calls','test-owned-assertions'],None)]
