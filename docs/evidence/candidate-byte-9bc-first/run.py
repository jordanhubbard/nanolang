...
save(base/'identity.json',dict(pin='9bc3cd88e60d57505db1aefbe6087683763b0e1c',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 for name,compiler,san in [('ordinary',cc,False),('sanitized',sancc,True)]:
  ...
  run(name+'-build',[*args,'nvm2c','nanoisa_dump','nano_vm','nano_virt'],root,out,1200)
  run(name+'-shapes',[*args,'test-nvm2c-shapes'],root,out,240)
  run(name+'-native',[sys.executable,'-m','unittest','-f','-v','tests.test_native_byte_array_identity'],root,out,1200)
