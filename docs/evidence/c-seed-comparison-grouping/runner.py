...
pin=git('rev-parse','HEAD');assert pin=='b4ddc1edb5a4509e5e8888fa4834b367f8f49cc7'
...
report={'head':pin,'host':platform.node(),'platform':platform.platform(),'scope':'fresh Make build/bootstrap, full array compatibility module, and existing Boolean precedence through C seed/Stage1/Stage2; deleted inner temporary products are not reconstructed','phases':[]};save('manifest.json',report)
...
for name,args,bound in [('build',['make','-j2','build',*flags],1800),('bootstrap',['make','-j2','bootstrap',*flags],1800),('array-compatibility',[sys.executable,'-m','unittest','-v','tests.test_selfhost_array_compatibility'],900)]:
 if not command(name,args,bound):sys.exit(1)
for compiler in ('nanoc_c','nanoc_stage1','nanoc_stage2'):
 output=out/('boolean-'+compiler)
 if not command('boolean-compile-'+compiler,[str(root/'bin'/compiler),'tests/test_boolean_precedence.nano','-o',str(output),'--keep-c'],60):sys.exit(1)
 if not command('boolean-run-'+compiler,[str(output)],10):sys.exit(1)
save('retained-boolean-products.json',snapshot(out.glob('boolean-*')))
