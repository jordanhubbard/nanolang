...
base=Path(sys.argv[1]).resolve();darwin=sys.platform=='darwin';root=base/'source';report=base/'package-hb-reports'
...
env['CARRIER_PHASES']='configuration,package'
if darwin:
...
assert darwin
env['CC']=clang
extra.update(package_wasmtime=shutil.which('wasmtime',path=env['PATH']),package_node=shutil.which('node',path=env['PATH']),package_opt=shutil.which('opt',path=env['PATH']))
env['CARRIER_EXTRA_TOOLS']=json.dumps(extra)
subprocess.run([sys.executable,str(base/'verify-record-backend-current.py'),'--root',str(root),'--report',str(base/'reports'),'--output',str(base/'package-hb-original-current.json')],check=True,timeout=600)
...
(base/'package-hb-launch.json').write_text(json.dumps(record,indent=2)+'\n')
with (base/'package-hb-outer.log').open('wb') as log:
 p=subprocess.Popen([sys.executable,str(base/'nanolang-record-backend-integration-driver.py'),str(root),str(report)],env=env,stdout=log,stderr=subprocess.STDOUT);record['driver_pid']=p.pid;(base/'package-hb-launch.json').write_text(json.dumps(record,indent=2)+'\n');code=p.wait()
record.update(returncode=code,finished=time.time());(base/'package-hb-outer.json').write_text(json.dumps(record,indent=2)+'\n');sys.exit(code)
