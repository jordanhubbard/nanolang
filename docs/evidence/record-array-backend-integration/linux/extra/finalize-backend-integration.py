...
base=Path(sys.argv[1]).resolve();assert (base/'outer.json').is_file();assert json.loads((base/'outer.json').read_text())['returncode']==0
assert shutil.disk_usage(base).free>=2147483648
python=sys.executable
subprocess.run([python,str(base/'audit-backend-integration-coverage.py'),str(base)],check=True,timeout=300)
subprocess.run([python,str(base/'verify-record-backend-current.py'),'--root',str(base/'source'),'--report',str(base/'reports'),'--output',str(base/'current-inputs.json')],check=True,timeout=600)
seal=base/'seal';args=[python,str(base/'seal-record-llvm-local-copies.py'),'--output',str(seal),'--history',str(base/'reports')]
for name in ('source.json','launch.json','outer.json','outer.log','current-inputs.json','coverage-audit.json','nanolang-backend-integration-launch.py','nanolang-record-backend-integration-driver.py','nanolang-record-package-retain.py','audit-backend-integration-coverage.py','verify-record-backend-current.py','finalize-backend-integration.py','seal-record-llvm-local-copies.py','first-extraction-terminal.json','staged-selector-preflight.json','orchestration-inputs.json','integration-correspondence.json'):
 p=base/name
