...
rows=read(report/'results.json');assert [(r['phase'],r['status']) for r in rows]==[(p,0) for p in expected]
assert read(base/'outer.json')['returncode']==0
result=[]
for phase in expected:
 assert good(read(report/(phase+'-terminal.json'))),phase
...
out={'source':read(base/'source.json')['pin'],'status':'PASS','phases':len(rows),'coverage':result,'scope':'Current integration terminal, exact phase selection and complete 73-case product/fault accounting; original sanitizer attribution remains separate.'}
