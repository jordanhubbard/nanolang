...
## My host ABI boundary

My exact-owner marking relies on generated string values being allocation bases or independently backed borrowed strings, not interior views into an owned argument. `STR_SUBSTR` copies its selected bytes. My builtin copy/file/normalize/capture/mktemp adapters allocate distinct result buffers; `from_char` returns a pool allocation base. Snapshot adapters copy transient facade results before returning.

For the in-tree artifact APIs, `modules/std/fs.c` duplicates the POSIX `basename`/`dirname` result before releasing its temporary input; `path_join` and `path_relpath` allocate their result, `path_canonical` uses `realpath`/`strdup`, and normalization calls the owned-result helper in `src/runtime/path_normalize.h`. `file_read` returns owned text from `src/runtime/file_text.h` or a literal. None returns an interior pointer into an input pool string.

I trust these exact native adapter lifetime contracts. My artifact name/signature/path checks do not inspect a replacement library's implementation and do not prove that it honors independent result storage. General borrowed-input or interior-pointer artifact results are unsupported by this adapter set; adding one requires snapshotting or explicit base-owner retention before it can share early collection. This is not native-code isolation. The host-buffer follow-up must retain this distinction rather than blindly freeing returned pointers.
