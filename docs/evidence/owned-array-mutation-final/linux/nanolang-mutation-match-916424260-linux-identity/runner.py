...
''')
 ...
 elif phase=='eval-final':
  dependency('build')
  if linux:
   prior=pathlib.Path(str(base)+'-eval')
   result=json.loads((prior/'eval.json').read_text())
   log=(prior/'eval.log').read_text()
   assert result['status']==0 and log.count('  Testing ')==123 and 'All eval tests passed!' in log
   assert json.loads((prior/'status.json').read_text())['error']=="FileNotFoundError(2, 'No such file or directory')"
   write('prior-evaluator-pass.json',{'command':result,'log_path':str(prior/'eval.log'),'log_sha256':sha(prior/'eval.log'),'tests':123,'binary_retained':False,'rerun':False})
  else:
   mk=out/'eval-retained.mk'
   mk.write_text('eval-retained: stage1 $(OBJ_DIR)/test_interpreter_ffi_native.so $(OBJ_DIR)/eval_io_faults.o $(OBJ_DIR)/eval_clock_test.o\n\t$(CC) $(CFLAGS) -o '+str(out/'test_eval')+' tests/test_eval.c $(filter-out $(OBJ_DIR)/eval.o $(OBJ_DIR)/eval/eval_io.o,$(COMMON_OBJECTS)) $(OBJ_DIR)/eval_clock_test.o $(OBJ_DIR)/eval_io_faults.o $(RUNTIME_OBJECTS) $(LDFLAGS)\n')
   command('eval-build',[make,'-j2','-f','Makefile.gnu','-f',str(mk),'CC='+cc,'eval-retained'])
   ...
   assert log.count('  Testing ')==123 and 'All eval tests passed!' in log
  ...
  assert 'Ran 8 tests' in (out/'totality.log').read_text()
 else:
  if phase=='identity':
   dependency('eval-final')
