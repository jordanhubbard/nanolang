...
def sources():return {p:digest(p) for p in (Path('/tmp/nanolang-file-body-tracked.txt').read_text().splitlines() if not Path('.git').exists() else subprocess.check_output(['git','ls-files','-z']).decode().split('\0')) if p and Path(p).is_file()}
def tools():
...
write('source-before.json',sources());write('tools-before.json',tools());write('selected-environment.json',{k:v for k,v in os.environ.items() if k in ['CC','SDKROOT','LIBRARY_PATH','NANO_FILE_BODY_CC','NANO_SERVICE_MODULE_TEST_CC','NMS_NATIVE_CLANG_FLAGS']})
steps=[('normal',['make','-j4','test-file-body']),('sanitizers',['make','test-file-body-sanitizers'])]
if sys.platform!='darwin':steps.append(('clang',['env','NANO_FILE_BODY_CC=clang','NANO_FILE_BODY_CFLAGS=--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13','make','test-file-body-sanitizers']))
steps += [('adjacent',['make','test-file-code','test-file-flow','test-file-nominal','test-service-bindings','test-nanoisa']),('provider',['make','-j4','nano_virt'])]
results=[]
