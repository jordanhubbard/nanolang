...
def tools():
 ...
 paths['actual_python']=sys.executable
...
def run(label,cmd,extra=None):
 ...
 write(label+'-command.json',{'argv':cmd,'selectors':{k:v for k,v in env.items() if k.startswith(('NANO_','FILE_','WRAPPER_','ORDINARY_')) or k in ('CC','SDKROOT','LIBRARY_PATH','NMS_NATIVE_CLANG_FLAGS','LSAN_OPTIONS')}})
...
base=dict(ORDINARY_ARRAY_OBJECTS=filtered,ORDINARY_ARRAY_LDFLAGS=values['LDFLAGS'])
py=[sys.executable,'-m','unittest','-f','-v','tests.test_ordinary_array_authority']
clang=shutil.which('clang');clang_flags='' if darwin else '--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13'
sanitize=' -fsanitize=address,undefined -fno-omit-frame-pointer'
run('ordinary',py,dict(base,ORDINARY_ARRAY_CC=os.environ['CC'],ORDINARY_ARRAY_CFLAGS=''))
run('clang-ordinary',py,dict(base,ORDINARY_ARRAY_CC=clang,ORDINARY_ARRAY_CFLAGS=clang_flags))
run('sanitizer',py,dict(base,ORDINARY_ARRAY_CC=os.environ['CARRIER_SAN_CC'],ORDINARY_ARRAY_CFLAGS=sanitize))
if not darwin:run('clang-sanitizer',py,dict(base,ORDINARY_ARRAY_CC=clang,ORDINARY_ARRAY_CFLAGS=clang_flags+sanitize))
for name in ('ordinary_record_authority','nvm_v2_convert','nvm_v2_endtoend'):
 selected=[p for p in objects if name!='ordinary_record_authority' or Path(p).stem not in ('ownership_contracts','nvm_v2_layouts')]
 variants=[('ordinary',os.environ['CC'],[])]
 if name=='ordinary_record_authority':variants.append(('sanitizer',os.environ['CARRIER_SAN_CC'],['-fsanitize=address,undefined','-fno-omit-frame-pointer']))
 for mode,compiler,flags in variants:
  label=name+'-'+mode;exe=controls/label
  run(label+'-build',[compiler,'-std=c11','-D_DEFAULT_SOURCE','-O1','-g','-Wall','-Wextra','-Werror',*flags,'-Isrc/nanoisa','-Imodules/nanoisa','tests/nanoisa/test_'+name+'.c',*selected,*links,'-o',str(exe)])
