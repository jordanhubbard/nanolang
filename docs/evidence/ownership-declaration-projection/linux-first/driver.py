...
def run(label,cmd,extra=None):
 ...
 write(label+'-command.json',{'argv':cmd,'selectors':{k:v for k,v in env.items() if k.startswith(('NANO_','FILE_','WRAPPER_','ORDINARY_','DECLARATION_')) or k in ('CC','SDKROOT','LIBRARY_PATH','NMS_NATIVE_CLANG_FLAGS','LSAN_OPTIONS')}})
...
base=dict(DECLARATION_OBJECTS=filtered,DECLARATION_LDFLAGS=values['LDFLAGS'])
py=[sys.executable,'-m','unittest','-f','-v','tests.test_ownership_declaration_projection']
clang=shutil.which('clang');clang_flags='' if darwin else '--gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13'
sanitize=' -fsanitize=address,undefined -fno-omit-frame-pointer'
run('discovery',[sys.executable,'-c',"import unittest;from tests import test_ownership_declaration_projection as m;s=unittest.defaultTestLoader.loadTestsFromModule(m);print(s.countTestCases());assert s.countTestCases()==1"])
run('ordinary',py,dict(base,DECLARATION_CC=os.environ['CC'],DECLARATION_CFLAGS=''))
run('clang-ordinary',py,dict(base,DECLARATION_CC=clang,DECLARATION_CFLAGS=clang_flags))
run('sanitizer',py,dict(base,DECLARATION_CC=os.environ['CARRIER_SAN_CC'],DECLARATION_CFLAGS=sanitize))
if not darwin:run('clang-sanitizer',py,dict(base,DECLARATION_CC=clang,DECLARATION_CFLAGS=clang_flags+sanitize))
