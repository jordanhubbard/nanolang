...
base=Path('/tmp/nanolang-mixed-declaration-puck-package');base.mkdir(exist_ok=False)
...
def sha(p):
 ...
 return h.hexdigest()
groups={'darwin-corrected':'5a367'};index={};refs=0;current=[]
for label,suffix in groups.items():
 root=Path('/tmp/nanolang-mixed-declaration-'+suffix).resolve();src=Path('/tmp/nanolang-mixed-declaration-'+suffix+'-puck');dst=reports/label;dst.mkdir()
 for p in src.iterdir():
  if p.is_file():shutil.copyfile(p,dst/p.name)
 results=json.loads((src/'results.json').read_text());last=results[-1]['phase'];assert len(results)==17 and all(x['status']==0 for x in results)
...
for name in ('reports','artifacts'):
 dest=Path('/tmp/nanolang-mixed-declaration-puck-'+name+'.tar.gz')
...
Path('/tmp/nanolang-mixed-declaration-puck-packaging.json').write_text(json.dumps(summary,indent=2)+'\n');print(json.dumps(summary,indent=2),flush=True)
