...
root=Path('/tmp/nanolang-mixed-declaration-5a367'); archive=Path('/tmp/nanolang-mixed-declaration-5a367-source.tar.gz')
manifest=json.loads(Path('/tmp/nanolang-mixed-declaration-5a367-source.json').read_text())
...
subprocess.run(['/opt/homebrew/bin/python3','/tmp/nanolang-mixed-declaration-driver.py',str(root),'/tmp/nanolang-mixed-declaration-5a367-puck'],check=True,env=env)
