...
source=Path('/tmp/nanolang-file-service-parser-a2eda.tar');remote='/tmp/nanolang-file-service-parser-a2eda.retained.tar.gz'
script='''from pathlib import Path
import gzip,hashlib,json,os,shutil,sys
p=Path('/tmp/nanolang-file-service-parser-a2eda.retained.tar.gz');tmp=p.with_name(p.name+'.partial')
assert not p.exists() and not tmp.exists()
h=hashlib.sha256();length=0
with tmp.open('xb') as raw:
 with gzip.GzipFile(filename='nanolang-file-service-parser-a2eda.tar',mode='wb',fileobj=raw,compresslevel=1,mtime=0) as out:
  while True:
...
assert length==5709209600 and verified==length and v.digest()==h.digest()
c=hashlib.sha256()
...
assert initial.st_nlink==1 and not source.is_symlink() and initial.st_size==5709209600
...
Path('/tmp/nanolang-archive-a2eda-remote.stdout').write_text(result.stdout);Path('/tmp/nanolang-archive-a2eda-remote.stderr').write_text(result.stderr)
...
row.update({'original_local_path':str(source),'original_mode':initial.st_mode&0o777,'original_mtime_ns':initial.st_mtime_ns,'remote_host':'jkh@puck.local','seconds':time.monotonic()-start,'restore':'scp jkh@puck.local:'+remote+' . ; gzip -dc nanolang-file-service-parser-a2eda.retained.tar.gz > nanolang-file-service-parser-a2eda.tar'})
manifest=source.with_name(source.name+'.storage.json')
with manifest.open('w') as out:out.write(json.dumps(row,indent=2)+'\n');out.flush();os.fsync(out.fileno())
current=source.stat()
assert current.st_ino==initial.st_ino and current.st_nlink==1 and current.st_mtime_ns==initial.st_mtime_ns
source.unlink()
print(json.dumps(row),flush=True)
