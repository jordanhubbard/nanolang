...
pin=git('rev-parse','HEAD');assert pin.startswith('c1409c5b5');assert not git('status','--porcelain','--untracked-files=no')
prior=pathlib.Path(sys.argv[3])
prior_map=json.loads((prior/'full-test-tools-after.json').read_text())
assert all(pathlib.Path(p).is_file() and sha(p)==digest for p,digest in prior_map.items())
save('prior-provider-verification.json',{'prior':str(prior),'count':len(prior_map),'equal':True})
...
def tool_map():
 ...
 for folder in ('bin','obj'):
  paths.update(p for p in (root/folder).rglob('*') if p.is_file() and 'nano_modules' not in p.parts and 'module_cache' not in p.parts and (folder=='bin' or p.suffix in ('.o','.a','.so','.dylib')))
...
archive(before,'source-artifacts.json');save('environment.json',{k:env.get(k) for k in ('CC','NANO_NATIVE_TEST_CC','NANOLANG_GUARD_SAN_CC','NMS_NATIVE_CLANG_FLAGS','PATH','SDKROOT','NANO_BUILD_CACHE','NANO_MODULE_PATH')})
...
steps=[('transport',[sys.executable,'/tmp/run-match-call-unit-retained.py','tests.test_reference_eval_transport'],1200),('full-test',['make','test','CC='+cc,*flags],5400)]
def products():
 return {str(p):sha(p) for folder in ('obj/nano_modules','obj/module_cache','build/userguide','tests/nanovm') for p in (root/folder).rglob('*') if p.is_file()}
status=1
try:
 for name,command,bound in steps:
  pb=products();save(name+'-products-before.json',pb);archive(pb,name+'-product-artifacts-before.json')
  ...
  with (out/(name+'.log')).open('wb') as log:
   phase_env=dict(env)
   if name in ('totality','transport'):phase_env['PYTHONPATH']=str(root)
  ...
  status=124 if timeout else p.returncode;ta=tool_map();sa=source_map();
  if name=='selfhost-bootstrap':
   assert all(sha(root/'bin'/n)==prior_map[str(root/'bin'/n)] for n in ('nanoc_c',))
   assert (root/'bin/nanoc_stage1').is_file() and (root/'bin/nanoc_stage2').is_file()
  pa=products();save(name+'-products-after.json',pa);archive(pa,name+'-product-artifacts-after.json');save(name+'-tools-after.json',ta);save(name+'-sources-after.json',sa);archive(ta,name+'-tool-artifacts.json')
  ...
  if status or sa!=before or (name not in ('vm','full-test') and ta!=tb):break
finally:
