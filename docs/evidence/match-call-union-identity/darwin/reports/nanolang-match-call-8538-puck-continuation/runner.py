...
prior_map=json.loads((prior/'guards-tools-after.json').read_text())
...
steps=[('selfhost-bootstrap',['make','-j2','bootstrap','CC='+cc,*flags],1800),('guards',[sys.executable,'-m','unittest','-v','tests.test_canonical_match_guards'],2400),('userguide',['make','userguide-check','CC='+cc,*flags],1800),('full-test',['make','test','CC='+cc,*flags],5400)]
def products():
 return {str(p):sha(p) for folder in ('obj/nano_modules','obj/module_cache') for p in (root/folder).rglob('*') if p.is_file()}
status=1
try:
 for name,command,bound in steps:
  ...
  if status or sa!=before or (name not in ('selfhost-bootstrap','userguide','full-test') and ta!=tb):break
finally:
