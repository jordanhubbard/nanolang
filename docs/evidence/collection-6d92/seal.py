...
base=Path('/home/jkh/nanolang-qualification');out=Path('/home/jkh/Src/nanolang-evaluator-collection-owners/docs/evidence/collection-6d92');out.mkdir(parents=True,exist_ok=True)
def sha(p):return hashlib.file_digest(Path(p).open('rb'),'sha256').hexdigest()
scope=json.loads((base/'collection-6d92-drivers/scope.json').read_text());expected={p:v['sha256'] for p,v in scope['files'].items()}
seal={'source':scope['pin'],'lanes':{},'objects':{}}
for label,r,status in [('linux-ordinary',base/'collection-6d92-ordinary',0),('darwin-ordinary',base/'collection-6d92-puck-reports/collection-6d92-ordinary',0),('darwin-sanitizer',base/'collection-6d92-puck-reports/collection-6d92-sanitizer',2)]:
 objects={}
...
shutil.copy2(base/'collection-6d92-puck-reports/collection-6d92-drivers/staging-first-terminal.json',out/'staging-first-terminal.json')
seal['linux_sanitizer']='unrun after Darwin first leak terminal';seal['totals']={'reports':sum(len(x['reports']) for x in seal['lanes'].values()),'objects':len(seal['objects']),'bytes':sum(x['bytes'] for x in seal['objects'].values())}
