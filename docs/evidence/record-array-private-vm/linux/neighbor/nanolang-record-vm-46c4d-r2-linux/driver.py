...
base=dict(RECORD_ARRAY_VM_OBJECTS=values['VM'],RECORD_ARRAY_VM_LDFLAGS=values['LDFLAGS'])
py=[sys.executable,'-m','unittest','-f','-v','tests.test_record_array_vm']
clang=shutil.which('clang');clang_flags='' if darwin else ' --gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13'
sanitize=' -fsanitize=address,undefined -fno-omit-frame-pointer'
run('discovery',[sys.executable,'-c',"import unittest;from tests import test_record_array_vm as m;s=unittest.defaultTestLoader.loadTestsFromModule(m);print(s.countTestCases());assert s.countTestCases()==2"])
run('ordinary',py,dict(base,RECORD_ARRAY_VM_CC=os.environ['CC'],RECORD_ARRAY_VM_CFLAGS=flags))
run('clang-ordinary',py,dict(base,RECORD_ARRAY_VM_CC=clang,RECORD_ARRAY_VM_CFLAGS=flags+clang_flags))
run('sanitizer',py,dict(base,RECORD_ARRAY_VM_CC=os.environ['CARRIER_SAN_CC'],RECORD_ARRAY_VM_CFLAGS=flags+sanitize))
if not darwin:run('clang-sanitizer',py,dict(base,RECORD_ARRAY_VM_CC=clang,RECORD_ARRAY_VM_CFLAGS=flags+clang_flags+sanitize))
...
proof=controls/'owned-string-proof'
proof_objects=[p for p in shlex.split(values['VM']) if p!='obj/nanovm/vm.o']
run('owned-string-proof-build',[os.environ['CC'],*shlex.split(flags),'-std=c11','-Isrc/nanoisa','tests/nanoisa/test_owned_string_proof.c',*proof_objects,*links,'-o',str(proof)])
run('owned-string-proof-run',[str(proof)])
for target in ('test-vm-ordinary-admission','test-owned-array-overwrite','test-file-private-vm','test-file-cyclic-public'):
 run(target,['make','-j2','CC='+os.environ['CC'],target],dict(NANO_FILE_RUNTIME_CC=os.environ['CC'],NANO_FILE_RUNTIME_CFLAGS=flags,NANO_FILE_RUNTIME_SANITIZERS='0',OWNED_ARRAY_OVERWRITE_CFLAGS=flags))
