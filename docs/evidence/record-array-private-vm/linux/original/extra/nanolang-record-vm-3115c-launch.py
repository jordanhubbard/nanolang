...
root=Path('/tmp/nanolang-record-vm-qualified-3115c');archive=Path('/tmp/nanolang-record-vm-3115c-source.tar.gz');manifest=json.loads(Path('/tmp/nanolang-record-vm-3115c-source.json').read_text())
...
subprocess.run([sys.executable,'/tmp/nanolang-record-vm-3115c-driver.py',str(root),'/tmp/nanolang-record-vm-3115c-'+host],check=True,env=env)
