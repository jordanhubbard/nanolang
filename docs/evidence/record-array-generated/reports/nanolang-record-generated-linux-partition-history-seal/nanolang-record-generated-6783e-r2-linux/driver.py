...
def run(label,cmd,extra=None):
 ...
 write(label+'-command.json',{'argv':cmd,'selectors':{k:v for k,v in env.items() if k.startswith(('NANO_','FILE_','WRAPPER_','ORDINARY_','DECLARATION_','RECORD_ARRAY_','RECORD_GENERATED_')) or k in ('CC','SDKROOT','LIBRARY_PATH','NMS_NATIVE_CLANG_FLAGS','LSAN_OPTIONS')}})
...
# I reuse only exact original 6783 setup providers after the capacity stop.
original=Path('/tmp/nanolang-record-generated-6783e-linux')
last=json.loads((original/'results.json').read_text())[-1]['phase']
assert source()==json.loads((original/(last+'-source-after.json')).read_text())
assert tools()==json.loads((original/(last+'-tools-after.json')).read_text())
prior=json.loads((original/(last+'-artifacts.json')).read_text());checked={}
for name,row in prior.items():
 p=Path(name)
 if p.is_relative_to(root):assert sha(p)==row['sha256'];checked[name]=row['sha256']
write('original-setup-reuse.json',{'source_pin':(root/'.qualification-pin').read_text().strip(),'original_report':str(original),'providers':checked,'original_configuration_sha256':sha(original/'configuration.log')})
shutil.copyfile(original/'configuration.log',report/'configuration.log')
...
py=[sys.executable,'-m','unittest','-f','-v','tests.test_record_array_generated']
clang=shutil.which('clang');clang_flags='' if darwin else ' --gcc-install-dir=/usr/lib/gcc/aarch64-linux-gnu/13'
sanitize=' -fsanitize=address,undefined -fno-omit-frame-pointer'
run('discovery',[sys.executable,'-c',"import unittest;from tests import test_record_array_generated as m;s=unittest.defaultTestLoader.loadTestsFromModule(m);print(s.countTestCases());assert s.countTestCases()==2"])
run('ordinary',py,dict(base,RECORD_GENERATED_CC=os.environ['CC'],RECORD_GENERATED_CFLAGS=flags))
run('clang-ordinary',py,dict(base,RECORD_GENERATED_CC=clang,RECORD_GENERATED_CFLAGS=flags+clang_flags))
run('sanitizer',py,dict(base,RECORD_GENERATED_CC=os.environ['CARRIER_SAN_CC'],RECORD_GENERATED_CFLAGS=flags+sanitize))
if not darwin:run('clang-sanitizer',py,dict(base,RECORD_GENERATED_CC=clang,RECORD_GENERATED_CFLAGS=flags+clang_flags+sanitize))
run('old-private-vm',[sys.executable,'-m','unittest','-f','-v','tests.test_record_array_vm'],dict(RECORD_ARRAY_VM_OBJECTS=values['VM'],RECORD_ARRAY_VM_LDFLAGS=values['LDFLAGS'],RECORD_ARRAY_VM_CC=os.environ['CC'],RECORD_ARRAY_VM_CFLAGS=flags))
for name in ('record_array_global_flow','record_array_execution','record_array_origins'):
 run(name,[sys.executable,'-m','unittest','-f','-v','tests.test_'+name],dict(RECORD_ARRAY_OBJECTS=values['ISA'],RECORD_ARRAY_LDFLAGS=values['LDFLAGS'],RECORD_ARRAY_CC=os.environ['CC'],RECORD_ARRAY_CFLAGS=flags))
