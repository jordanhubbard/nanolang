...
root=Path('/tmp/nanolang-record-generated-qualified-75979');archive=Path('/tmp/nanolang-record-generated-75979-source.tar.gz');manifest=json.loads(Path('/tmp/nanolang-record-generated-75979-source.json').read_text())
assert hashlib.sha256(archive.read_bytes()).hexdigest()==manifest['archive_sha256']
assert shutil.disk_usage('/tmp').free >= 3*1024**3,'I need 3 GiB free before fresh setup'
...
env=dict(os.environ,LSAN_OPTIONS='');env['CARRIER_PHASES']='setup,configuration,discovery,ordinary' if darwin else 'setup,configuration,discovery,ordinary,sanitizer,clang-sanitizer,old-private-vm,record_array_global_flow,record_array_execution,record_array_origins'
if darwin:
...
subprocess.run([sys.executable,'/tmp/nanolang-record-generated-75979-driver.py',str(root),'/tmp/nanolang-record-generated-75979-'+host],check=True,env=env)
