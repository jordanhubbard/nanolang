...
save(base/'identity.json',dict(pin='780f28636',archive_sha=digest(archive),driver_sha=digest(pathlib.Path(__file__)),host=platform.uname()._asdict(),sdk=sdk,tools={p:digest(pathlib.Path(p).resolve()) for p in [make,cc,sancc,sys.executable]}))
...
try:
 ...
 save(base/'terminal.json',dict(status=0,scope='exact catalog and borrowed zero/one/two string VM/COP/native plus refusal/allocation controls; real-provider sanitizer compiler prerequisite remains held'))
