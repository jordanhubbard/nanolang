...
def tools():
 ...
 for key,value in os.environ.items():
  if key.startswith('RECORD_LLVM_') and key not in ('RECORD_LLVM_C_BASELINE','RECORD_LLVM_IR_ASAN'):
...
configs=[('ordinary',os.environ['CC'],flags,False),('clang-ordinary',clang,flags+clang_flags,False),('sanitizer',os.environ['CARRIER_SAN_CC'],flags+sanitize,True)]
if not darwin:configs.append(('clang-sanitizer',clang,flags+clang_flags+sanitize,True))
