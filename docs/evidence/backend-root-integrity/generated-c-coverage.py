...
import json,re
base=Path('/home/jkh/nanolang-qualification/backend-integration-a1bb-combined-seal')
reports=base/'reports'
phases=[(reports/host/'reports',phase) for host in ['linux','puck'] for phase in ['generated-ordinary-native','generated-clang-native']]
...
for root,phase in phases:
 ...
 for opt in ['O0','O2']:
  for mode in ['linked','observed']:
   for i in range(73):
    for step in ['build', 'run' if mode=='linked' else 'baseline']:assert good(artifact(f'{opt}-{mode}-{i:04d}-{step}-status.json'))
 total=0
 for opt in ['O0','O2']:
  for i in range(73):
...
result={'scope':'Independent four fresh generated-C integration configuration terminal, child status, 73-case O0/O2 linked/observed products and complete fault-range/recovery audit. Current-main integration and production correspondence remain separate.','phases':results,'passed':True}
Path('/home/jkh/nanolang-qualification/backend-root-generated-c-coverage.json').write_text(json.dumps(result,indent=2)+'\n')
