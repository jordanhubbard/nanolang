...
start=time.monotonic();base=Path('/home/jkh/nanolang-qualification/backend-integration-a1bb-combined-seal');reports=base/'reports';index=json.loads((base/'report-sha256.json').read_text());result=[]
configs=[('linux-default', 'linux/reports/ordinary-native-artifacts.json', ('O0', 'O2')), ('linux-clang', 'linux/reports/clang-ordinary-native-artifacts.json', ('O0', 'O2')), ('darwin-default', 'puck/reports/ordinary-native-artifacts.json', ('O0', 'O2')), ('darwin-clang', 'puck/reports/clang-ordinary-native-artifacts.json', ('O0', 'O2'))]
for label,path,opts in configs:
 ...
 for name,row in mapping.items():
  # Mach-O debug companions reuse the executable basename inside .dSYM.
  # They remain in the integrity audit, but are not command products.
  if any(part.endswith('.dSYM') for part in Path(name).parts):continue
  byname.setdefault(Path(name).name,[]).append((name,row))
 def get(name):
...
out={'status':'PASS','qualified_source_pin':'a1bb1cb85e5551f9e5bb10491f1608877ef6b7b5','cases_per_mode_optimization':73,'configurations':result,'seconds':time.monotonic()-start,'scope':'Four fresh ordinary native integration configurations. Checks actual retained products, compile/link/verifier terminals, invoked executable identity, linked observation success, symbol exclusions and exact contiguous two-mode allocation recovery logs. Startup/emission/Wasm/package, normalized corpus equivalence and fresh current-main integration are separate obligations.'}
Path('/home/jkh/nanolang-qualification/backend-root-native-coverage.json').write_text(json.dumps(out,indent=2)+'\n')
