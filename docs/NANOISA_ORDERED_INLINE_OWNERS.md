...
I account for held named owners through the unchanged adjacent `test-affine-state` controls: `tests/nanoisa/test_affine_state.c` creates two shared holds on a nested owner, then requires `nvm_affine_move(s,5,6)` and whole-owner unpack to refuse with unchanged state. The same suite checks exclusive access conflicts and runs its allocation-instrumented form. This is runtime state evidence, not invented source syntax for a stored borrow. My new source path retains exact mode/liveness checks, and verified OWN_MOVE remains the authority for active holds.
