...
A new generator mode named `file-binding` first validates the existing NSI v0 document through `nl_file_plan_build`. It stages the complete binding output and a canonical current-schema NSI companion, then publishes the manifest/module pair without leaving a mixed-generation pair after failure. The source declaration resolves its NSI path relative to the declaring module through the existing canonical module resolver. It reads a bounded snapshot once; later phases use owned validated facts rather than reopening a potentially changed document. I compare full IDs and contents against catalog1; filenames, comments, hashes, sanitized symbols and generated provenance are never authority. A handwritten identical declaration may request the same interface, but still needs all checks and a host grant. Unknown keys remain rejected by the unchanged NSI v0 parser.

The exact companion name and publication manifest format receive a narrow generator/API review before code; I do not add ignored keys to `module.json` or NSI. If the existing manifest cannot express the pair, I use a separately versioned, explicitly consumed binding manifest rather than pretending atomic publication of two independent renames. The first descriptive checkpoint publishes neither file.
